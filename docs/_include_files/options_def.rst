...
- ``=bfloat16``: Disable the SYCL extensions for bfloat16. `sycl_ext_oneapi_bfloat16 <https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/supported/sycl_ext_oneapi_bfloat16.asciidoc>`_
- ``=device_info``: Disable the Intel extensions for device information, if supported
  by the compiler and the backend. `sycl_ext_intel_device_info <https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/supported/sycl_ext_intel_device_info.md>`_
- ``=enqueued_barriers``: Disable the enqueued barriers extension. `sycl_ext_oneapi_enqueue_barrier <https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/supported/sycl_ext_oneapi_enqueue_barrier.asciidoc>`_
- ``=peer_access``: Disable the peer access extension. `sycl_ext_oneapi_peer_access <https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/supported/sycl_ext_oneapi_peer_access.asciidoc>`_
- ``=assert``: Disable the assert extension. `sycl_ext_oneapi_assert <https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/supported/sycl_ext_oneapi_assert.asciidoc>`_
- ``=queue_empty``: Disable the queue empty extension. `sycl_ext_oneapi_queue_empty <https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/supported/sycl_ext_oneapi_queue_empty.asciidoc>`_
...
- ``=c_cxx_standard_library``: Use std functions from the libdevice library
  ...
  ``intel_device_math``, the ``intel_device_math`` functions take precedence. `C-CXX-StandardLibrary <https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/supported/C-CXX-StandardLibrary.rst>`_
- ``=intel_device_math``: Use ``sycl::ext::intel::math`` functions from the libdevice
  ...
  mapping in the SYCL standard. `libdevice <https://github.com/intel/llvm/tree/sycl/libdevice>`_
...
- ``=bfloat16_math_functions``: Experimental extension that allows use of bfloat16 math functions. `sycl_ext_oneapi_bfloat16_math_functions <https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/experimental/sycl_ext_oneapi_bfloat16_math_functions.asciidoc>`_
- ``=bindless_images``: Experimental extension that allows use of bindless images APIs. `sycl_ext_oneapi_bindless_images <https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/experimental/sycl_ext_oneapi_bindless_images.asciidoc>`_
- ``=dpl-experimental-api``: Experimental extension that allows use of experimental
  oneDPL APIs. `dpl_experimental_api <https://github.com/oneapi-src/oneDPL/tree/main/include/oneapi/dpl/pstl/experimental>`_
- ``=free-function-queries``: Experimental extension that allows getting
  ``id``, ``item``, ``nd_item``, ``group``, and ``sub_group`` instances
  globally. `sycl_ext_oneapi_free_function_queries <https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/supported/sycl_ext_oneapi_free_function_queries.asciidoc>`_
- ``=local-memory-kernel-scope-allocation``: Experimental extension that
  allows allocation of local memory objects at the kernel functor scope. `sycl_ext_oneapi_local_memory <https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/supported/sycl_ext_oneapi_local_memory.asciidoc>`_
...
- ``=matrix``: Experimental extension that allows use of matrix extension like class ``joint_matrix``. `sycl_ext_oneapi_matrix <https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/experimental/sycl_ext_matrix/sycl_ext_oneapi_matrix.asciidoc>`_
...
- ``=user-defined-reductions``: Experimental extension that allows user-defined
  reductions. `sycl_ext_oneapi_user_defined_reductions <https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/experimental/sycl_ext_oneapi_user_defined_reductions.asciidoc>`_
- ``=non-uniform-groups``: Experimental extension that allows use of non-uniform groups. `sycl_ext_oneapi_non_uniform_groups <https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/experimental/sycl_ext_oneapi_non_uniform_groups.asciidoc>`_
- ``=device_global``: Experimental extension that allows device scoped memory allocations into SYCL that can
  be accessed within a kernel using syntax similar to C++ global variables. `sycl_ext_oneapi_device_global <https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/experimental/sycl_ext_oneapi_device_global.asciidoc>`_
