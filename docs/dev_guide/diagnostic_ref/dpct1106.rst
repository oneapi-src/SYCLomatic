...
The CUDA\* function ``<function name>`` (for example ``cudaMemGetInfo`` or ``cuMemGetInfo``) was
migrated with the Intel extensions for device information, which may be not supported
by all compilers or runtimes. You may need to adjust the code.
