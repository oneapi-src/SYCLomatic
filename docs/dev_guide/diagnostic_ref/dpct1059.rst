...
.. code-block:: cpp
   ...
   void foo(sycl::uint2 *image_data) {
     // (1) Please allocate memory for the new_image_data with sycl::uint4 type.
     // (2) Copy the 1st and 2nd element from each pixel of the image_data to
     //     the corresponding pixel of the new_image_data.
     // (3) Replace the image_data, the binded memory of the tex object, with the new_image_data.
