...
**WWhy some compile commands for C/C++ files are missing in the compilation database
generated by intercept-build when ccache is enabled?**

When ccache is enabled, the object of C/C++ files created by the previous build is cached, thus some C/C++ compile commands are skipped during the execution of intercept-build.
Therefore, the generated compilation database is not complete.

The fix is to disable ccache by running:
     .. code-block:: bash
         export CCACHE_DISABLE=1
Then all the C/C++ compile command will be triggered and captured by intercept-build tool.
