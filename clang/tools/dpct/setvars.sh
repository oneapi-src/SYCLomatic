...
if [ -n "${ZSH_VERSION:-}" ] && [ -n "${ZSH_EVAL_CONTEXT:-}" ] ; then     # zsh 5.x and later
  ...
  vars_script_name="$( (echo "${.sh.file}") 2>&1 )" || : ;
  if [ "$(printf "%s" "$vars_script_name" | grep -Eq "sh: [0-9]+: .*setvars\.sh: " ; echo $?)" -eq 0 ] ; then # dash as sh
    vars_script_name="$(expr "${vars_script_name:-}" : '^.*sh: [0-9]*: \(.*\):')" ;
  fi
  ...
  # sample error msg shown; assume this file is named "setvars.sh"
  vars_script_name="$( (echo "${.sh.file}") 2>&1 )" || : ;
  if [ "$(printf "%s" "$vars_script_name" | grep -Eq "^.+: [0-9]+: .*setvars\.sh: " ; echo $?)" -eq 0 ] ; then # dash
...
# ############################################################################
component_root=$(get_script_path "${vars_script_name:-}")

# Export required env vars for dpct package.

DPCT_BUNDLE_ROOT=$component_root
