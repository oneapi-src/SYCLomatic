...
def compare_checkpoint_list(cuda_prelog_checkpoint_list, cuda_epilog_checkpoint_list, sycl_prelog_checkpoint_list, sycl_epilog_checkpoint_list):
    global passed_checkpoint_num
    global checkpoint_size
    failed_log  = ""
    is_checkpoint_length_dismatch(cuda_prelog_checkpoint_list, sycl_prelog_checkpoint_list)
    is_checkpoint_length_dismatch(cuda_epilog_checkpoint_list, sycl_epilog_checkpoint_list)
    failed_epilog = []
    for id, cuda_epilog_checkpoint in cuda_epilog_checkpoint_list.items():
        ...
        try:
            ...
            continue
    for id, cuda_prelog_checkpoint in cuda_prelog_checkpoint_list.items():
        checkpoint_size += 1
        if not sycl_prelog_checkpoint_list.get(id):
            failed_log += get_missing_key_log(id)
            continue
        try:
            compare_checkpoint(cuda_prelog_checkpoint, sycl_prelog_checkpoint_list.get(id))
            passed_checkpoint_num += 1
        except comparison_error as e:
            # Check whether the corresponding ID is failed in the epilog,
            # If the epilog is matching, then skip the prelog.
            # if the epilog is dismatching, then check the prelog data.
            fail_id = id.replace(":prelog:", ":epilog:")
            if fail_id in failed_epilog:
                failed_log += get_data_value_mismatch_log(new_id, e.message)
                failed_epilog.append(id)
            continue
    return failed_log
...
def get_checkpoint_list_from_json_file(file_path):
    ...
    prelog_checkpoint_list = {}
    epilog_checkpoint_list = {}
    for item in json_data_list:
        id = item[UUID]
        if "prelog" in id:
            prelog_checkpoint_list[id] = item.get(CHECKPOINT, {})
        elif "epilog" in id:
            epilog_checkpoint_list[id] = item.get(CHECKPOINT, {})
    return prelog_checkpoint_list, epilog_checkpoint_list
...
def main():
    ...
    cuda_prelog_checkpoint_list, cuda_epilog_checkpoint_list = get_checkpoint_list_from_json_file(args.instrumented_cuda_log)
    sycl_prelog_checkpoint_list, sycl_epilog_checkpoint_list = get_checkpoint_list_from_json_file(args.instrumented_sycl_log)
    
    failed_log = compare_checkpoint_list(cuda_prelog_checkpoint_list, cuda_epilog_checkpoint_list, sycl_prelog_checkpoint_list, sycl_epilog_checkpoint_list)
