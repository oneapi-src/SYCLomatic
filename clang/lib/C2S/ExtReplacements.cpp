...
std::shared_ptr<ExtReplacement> ExtReplacements::buildOriginCodeReplacement(
    const SourceLineRange &LineRange, std::shared_ptr<C2SFileInfo> FileInfo) {
  if (!LineRange.SrcBeginLine)
    return std::shared_ptr<ExtReplacement>();
  std::string Text = "/* C2S_ORIG ";
...
// DPC++, LOC migrated to helper functions. While, the old solution only
  ...
  for (auto &R : ReplMap) {
    ...
    if (Repl->getLength()) {
      ...
      if (DuplicateFilter.find(Key) == end(DuplicateFilter)) {
        ...
        if (Repl->getReplacementText().find(std::string("c2s::")) !=
            StringRef::npos) {
    ...
    if (C2SGlobalInfo::isC2SNamespaceTempEnabled()) {
      // When option "--report-type=stats" or option " --report-type=all" is
      // specified to get the migration status report and c2s namespace is
...
void ExtReplacements::addReplacement(std::shared_ptr<ExtReplacement> Repl) {
  auto const &FileInfo = C2SGlobalInfo::getInstance().insertFile(FilePath);
...
void ExtReplacements::emplaceIntoReplSet(tooling::Replacements &ReplSet) {
  std::vector<std::shared_ptr<clang::c2s::ExtReplacement>> ReplsList =
...
void ExtReplacements::processCudaArchMacro() {
  ...
  auto &CudaArchPPInfosMap =
      C2SGlobalInfo::getInstance().getCudaArchPPInfoMap()[FilePath];
  auto &CudaArchDefinedMap =
      C2SGlobalInfo::getInstance().getCudaArchDefinedMap()[FilePath];
  auto &ReplSet = C2SGlobalInfo::getInstance().getCudaArchMacroReplSet();
  // process __CUDA_ARCH__ macro of directive condition in generated host code:
  // if __CUDA_ARCH__ > 800      -->  if !C2S_COMPATIBILITY_TEMP
  // if defined(__CUDA_ARCH__)   -->  if !defined(C2S_COMPATIBILITY_TEMP)
  // if !defined(__CUDA_ARCH__)  -->  if defined(C2S_COMPATIBILITY_TEMP)
  auto processIfMacro = [&](std::shared_ptr<ExtReplacement> Repl,
    ...
    if (CudaArchDefinedMap.count((*Repl).getOffset())) {
      ...
      (*Repl).setReplacementText("!C2S_COMPATIBILITY_TEMP");
    }
  };
  for (auto Repl = ReplSet.begin(); Repl != ReplSet.end();) {
    ...
    for (auto Iterator = CudaArchPPInfosMap.begin();
      ...
      unsigned Pos_a = 0, Len_a = 0, Pos_b = 0, Len_b = 0,
               Round = C2SGlobalInfo::getRunRound();
      if (CudaArchOffset >= Info.IfInfo.ConditionLoc &&
        ...
        //  ifndef__CUDA_ARCH__ --> ifdef C2S_COMPATIBILITY_TEMP
        //  ifdef __CUDA_ARCH__ --> ifndef C2S_COMPATIBILITY_TEMP
...
void ExtReplacements::buildCudaArchHostFunc(
  ...
  auto &HDFDIMap = C2SGlobalInfo::getInstance().getHostDeviceFuncDefInfoMap();
  auto &HDFDeclIMap =
      C2SGlobalInfo::getInstance().getHostDeviceFuncDeclInfoMap();
  auto &HDFCIMap = C2SGlobalInfo::getInstance().getHostDeviceFuncCallInfoMap();
  // process call
  ...
  for (auto &R : ExtraRepl) {
    auto &FileReplCache = C2SGlobalInfo::getFileReplCache();
...
void ExtReplacements::postProcess() {
  auto FileInfo = C2SGlobalInfo::getInstance().insertFile(FilePath);
  for (auto &R : ReplMap) {
    ...
    while (std::regex_search(Begin, End, Result, RE)) {
      ...
      NewText += C2SGlobalInfo::getStringForRegexReplacement(
    ...
    if (NewText.size()) {
      ...
      New->setInsertPosition(
          static_cast<c2s::InsertPosition>(Old->getInsertPosition()));
      Old = std::move(New);
    }
  }

  if (C2SGlobalInfo::isKeepOriginCode())
    buildOriginCodeReplacements(FileInfo);

  processCudaArchMacro();

  if (C2SGlobalInfo::getRunRound() == 1) {
