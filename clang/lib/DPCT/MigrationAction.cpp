...
DpctFrontEndAction::CreateASTConsumer(CompilerInstance &CI, StringRef InFile) {
  return std::make_unique<DpctConsumer>();
}

bool DpctFrontEndAction::BeginSourceFileAction(CompilerInstance &CI) {
  auto &PP = CI.getPreprocessor();
  PP.addPPCallbacks(std::make_unique<IncludesCallbacks>(
      TransformSet, IncludeMap, CI.getSourceManager()));
  if (DpctGlobalInfo::getCheckUnicodeSecurityFlag()) {
    Handler = std::make_unique<MisleadingBidirectionalHandler>(TransformSet);
...
bool DpctToolAction::runInvocation(
  ...
  DpctFrontEndAction FEAction(Info->Transforms, Info->IncludeMapSet);
