  ...
  MF.addMatcher(
      typeLoc(
          loc(qualType(hasDeclaration(namedDecl(anyOf(
              hasAnyName(
                  "cudaError", "curandStatus", "cublasStatus",
  ...
  MF.addMatcher(
      declRefExpr(
          to(enumConstantDecl(matchesName(
              ...
              "IMPLEMENTED|CUFFT_LICENSE_ERROR|CUFFT_NOT_SUPPORTED|CUFFT_R2C|"
              "CUFFT_C2R|CUFFT_C2C|CUFFT_D2Z|CUFFT_Z2D|CUFFT_Z2Z)"))))
          .bind("FFTConstants"),
      this);
}
...
void SPBLASFunctionCallRule::run(const MatchFinder::MatchResult &Result) {
  ...
  if (FuncName == "cusparseCreate" || FuncName == "cusparseDestroy" ||
    ...
    if (FuncName == "cusparseCreate") {
      ...
      int Index = DpctGlobalInfo::getHelperFuncReplInfoIndexThenInc();
      buildTempVariableMap(Index, CE, HelperFuncType::DefaultQueue);
      ReplaceStrs.Repl = LHS + " = &{{NEEDREPLACEQ" + std::to_string(Index) + "}}";
    ...
    ReplaceStrs.PrefixInsertStr =
        ReplaceStrs.PrefixInsertStr + "oneapi::mkl::sparse::init_matrix_handle(&" +
        MatrixHandleName + ");" + getNL() + ReplaceStrs.IndentStr;
    if (DpctGlobalInfo::getUsmLevel() == UsmLevel::none) {
      ReplaceStrs.PrefixInsertStr =
    ...
    ReplaceStrs.SuffixInsertStr = ReplaceStrs.SuffixInsertStr + getNL() +
                              ReplaceStrs.IndentStr +
    ...
    if (DpctGlobalInfo::getUsmLevel() == UsmLevel::none) {
      ReplaceStrs.Repl = "oneapi::mkl::sparse::gemv(*" + CallExprArguReplVec[0] +
                     ", " + TransStr + ", " + "dpct::get_value(" +
      ...
      if (FuncName == "cusparseScsrmv" || FuncName == "cusparseDcsrmv")
        ReplaceStrs.Repl = "oneapi::mkl::sparse::gemv(*" + CallExprArguReplVec[0] +
                       ", " + TransStr + ", " + "dpct::get_value(" +
                       CallExprArguReplVec[5] + ", *" + CallExprArguReplVec[0] +
                       "), " + MatrixHandleName + ", const_cast<" + BufferType +
                       "*>(" + X + "), " + "dpct::get_value(" +
        ...
        ReplaceStrs.Repl = "oneapi::mkl::sparse::gemv(*" + CallExprArguReplVec[0] +
                       ...
                       X + ", " + "dpct::get_value(" + CallExprArguReplVec[11] +
    ...
    if (DpctGlobalInfo::getUsmLevel() == UsmLevel::none) {
      ReplaceStrs.Repl = "oneapi::mkl::sparse::gemm(*" + CallExprArguReplVec[0] +
                     ", " + TransStr + ", " + "dpct::get_value(" +
...
void EventAPICallRule::handleTimeMeasurement() {
    ...
    for (auto Iter = FuncBody->child_begin(); Iter != FuncBody->child_end();
      ...
      if (const CallExpr *RecordCall = dyn_cast<CallExpr>(*Iter)) {
        ...
        if (RecordFuncName == "cudaEventRecord") {
          ...
          if (Arg0 == getStmtSpelling(TimeElapsedCE->getArg(1)))
            RecordBegin = RecordCall;
          else if (Arg0 == getStmtSpelling(TimeElapsedCE->getArg(2)))
  ...
  RecordBeginLoc = RecordBegin->getBeginLoc().getRawEncoding();
  RecordEndLoc = RecordEnd->getBeginLoc().getRawEncoding();
  TimeElapsedLoc = TimeElapsedCE->getBeginLoc().getRawEncoding();
...
void EventAPICallRule::handleTargetCalls(const Stmt *Node) {
  ...
  for (auto It = Node->child_begin(); It != Node->child_end(); ++It) {
    auto Loc = It->getBeginLoc().getRawEncoding();
    ...
    if (Loc > RecordEndLoc && Loc < TimeElapsedLoc) {
      auto Call = dyn_cast<CallExpr>(*It);
      if (!Call)
        continue;
      ...
      if (Callee->getName() == "cudaEventSynchronize") {
        emplaceTransformation(new ReplaceStmt(
            Call, false, std::string("cudaEventSynchronize"), ""));
      }
    }
...
// nullptr.
const Expr *TextureRule::getAssignedBO(const Expr *E,
                                                 ASTContext &Context) {
