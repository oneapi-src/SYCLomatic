...
void RandomFunctionCallRule::runRule(const MatchFinder::MatchResult &Result) {
  ...
  if (IsAssigned) {
    requestFeature(HelperFeatureEnum::Dpct_dpct_check_error, CE);
  ...
  if (MapNames::BLASComputingAPIWithRewriter.find(FuncName) !=
    ...
    if (SM->isMacroArgExpansion(CE->getBeginLoc()) &&
      ...
      if (IsAssigned) {
        requestFeature(HelperFeatureEnum::Dpct_dpct_check_error, CE);
        emplaceTransformation(
            new ReplaceText(SR.getBegin(), Len, "DPCT_CHECK_ERROR(" + Repl + ")"));
      } else {
        emplaceTransformation(
      ...
      if (IsAssigned) {
        requestFeature(HelperFeatureEnum::Dpct_dpct_check_error, CE);
        emplaceTransformation(new ReplaceStmt(CE, true, "DPCT_CHECK_ERROR(" + Repl + ")"));
      } else {
...
void EventAPICallRule::handleEventElapsedTime(bool IsAssigned) {
  if(DpctGlobalInfo::getEnablepProfilingFlag()) {
    ...
    if (IsAssigned) {
      ...
      Repl = std::move(Temp);
      requestFeature(HelperFeatureEnum::Dpct_dpct_check_error, TimeElapsedCE);
    }
...
void KernelFunctionInfoRule::runRule(const MatchFinder::MatchResult &Result) {
  if (auto V = getNodeAsType<VarDecl>(Result, "decl")) {
    ...
    emplaceTransformation(
        new ReplaceToken(C->getBeginLoc(), "DPCT_CHECK_ERROR(" + MapNames::getDpctNamespace() +
