...
bool IncludesCallbacks::isInRoot(SourceLocation Loc) {
  return DpctGlobalInfo::isInRoot(Loc);
}
...
void IncludesCallbacks::MacroExpands(const Token &MacroNameTok,
  ...
  bool IsInRoot = isInRoot(SM.getSpellingLoc(MacroNameTok.getLocation()));
  auto MI = MD.getMacroInfo();
  if (!MI) {
    return;
  }
...
void IncludesCallbacks::InclusionDirective(
  ...
  std::string InRoot = DpctGlobalInfo::getInRoot();
...
REGISTER_RULE(DevicePropVarRule, PassKind::PK_Migration)
...
REGISTER_RULE(GlibcMemoryAPIRule, PassKind::PK_Migration)
...
void MemVarRule::processDeref(const Expr *S, ASTContext &Context) {
  auto Parents = Context.getParents(*S);
  if (Parents.size()) {
    auto &Parent = Parents[0];
    if (auto ICE = Parent.get<ImplicitCastExpr>()) {
      return processDeref(ICE, Context);
    } else if (auto ME = Parent.get<MemberExpr>()) {
      return emplaceTransformation(new ReplaceToken(ME->getOperatorLoc(), "->"));
    } else if (auto CDSME = Parent.get<CXXDependentScopeMemberExpr>()) {
      return emplaceTransformation(new ReplaceToken(CDSME->getOperatorLoc(), "->"));
    } else if (Parent.get<BinaryOperator>() || Parent.get<CallExpr>() ||
               Parent.get<CXXConstructExpr>() || Parent.get<ParenExpr>()) {
      return emplaceTransformation(new InsertBeforeStmt(S, "*"));

    } else if (auto UO = Parent.get<UnaryOperator>()) {
      if (UO->getOpcode() == UnaryOperatorKind::UO_AddrOf)
        return emplaceTransformation(new ReplaceToken(UO->getOperatorLoc(), ""));
    }
    ExprAnalysis EA;
    EA.analyze(S);
    return emplaceTransformation(new ReplaceStmt(S, "(*" + EA.getReplacedString() + ")"));
  }
}
