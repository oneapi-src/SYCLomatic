...
void test(cusparseHandle_t handle, cusparseOperation_t trans, int m,
          const void *alpha, cudaDataType alpha_type,
          const cusparseMatDescr_t desc, const void *value,
          cudaDataType value_type, const int *row_ptr, const int *col_idx,
          cusparseSolveAnalysisInfo_t info, const void *f, cudaDataType f_type,
          void *x, cudaDataType x_type, cudaDataType exec_type) {
  // Start
