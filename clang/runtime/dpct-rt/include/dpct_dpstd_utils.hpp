...
#else
template <typename T>
using device_iterator = pstl::sycl_iterator<sycl::access::mode::read_write, T>;
#endif
...
struct compare_key_fun {
  ...
  compare_key_fun(Compare _comp = __pstl::internal::pstl_less())
      : comp(_comp) {}
...
// Used by: copy_if, remove_copy_if, stable_partition_copy
// Lambda:
template <typename Predicate> struct predicate_key_fun { // predicate fun?
  typedef bool result_of;
...
template <typename Predicate> struct unique_by_key_fun { // compare key?
  typedef bool result_of;
...
template <typename T,
          /*typename T2, */ typename Predicate = class dpstd::identity<T>,
          typename UnaryOperation = class dpstd::identity<T>>
class transform_if_stencil_fun1 {
public:
  typedef typename std::tuple_element<2, T>::type result_of;
  transform_if_stencil_fun1(Predicate _pred = dpstd::identity<T>(),
                            UnaryOperation _op = dpstd::identity<T>())
      : pred(_pred), op(_op) {}
...
public:
  ...
  transform_if_zip_stencil_fun(Predicate _pred = dpstd::identity<T>(),
                               BinaryOperation _op = dpstd::identity<T>())
      : pred(_pred), op(_op) {}
  // template<typename _T> result_of operator() (const T& a, _T&& s) const {
  // return pred(s) ? op(a) : a; }
  template <typename _T> result_of operator()(_T t) {
...
void sequence(Policy &&policy, InputIter first, InputIter last, T init,
  ...
  std::transform(std::forward<Policy>(policy),
                 ...
                 first, dpct::sequence_fun<T>(init, step));
}
...
ForwardIt3 transform_if(Policy &&policy, ForwardIt1 first, ForwardIt1 last,
  ...
  using ZipIterator =
      typename dpstd::zip_iterator<ForwardIt1, ForwardIt2, ForwardIt3>;
  using T = typename std::iterator_traits<ZipIterator>::value_type;
  return std::transform(
      std::forward<Policy>(policy),
      dpstd::make_zip_iterator(first, stencil, result),
      dpstd::make_zip_iterator(last, stencil, result), result,
      dpct::transform_if_stencil_fun1<T, Predicate, UnaryOperation>(pred,
                                                                    unary_op));
}
...
ForwardIt4 transform_if(Policy &&policy, ForwardIt1 first1, ForwardIt1 last1,
  ...
  return std::transform(
      ...
      dpstd::make_zip_iterator(last1, last1 + n, stencil, result), result,
      dpct::transform_if_zip_stencil_fun<T, Predicate, BinaryOperation>(
          pred, binary_op));
}

template <class ForwardIt1, class ForwardIt2, class ForwardIt3,
          class ForwardIt4, class BinaryOperation, class Predicate>
ForwardIt4 transform_if(ForwardIt1 first1, ForwardIt1 last1, ForwardIt2 first2,
                        ForwardIt3 stencil, ForwardIt4 result,
                        BinaryOperation binary_op, Predicate pred) {
  return transform_if(pstl::execution::sycl, first1, last1, first2, stencil,
                      result, binary_op, pred);
}
...
OutputIt transform(InputIt first, InputIt last, OutputIt result,
...
OutputIt transform(InputIt1 first1, InputIt1 last1, InputIt2 first2,
                   OutputIt result, UnaryOperation unary_op) {
  return transform(pstl::execution::sycl, first1, last1, first2, result,
                   unary_op);
}
...
template <typename InputIt, typename UnaryOperation, typename OutputType,
          typename BinaryOperation>
OutputType transform_reduce(InputIt first, InputIt last,
                            UnaryOperation unary_op, OutputType init,
                            BinaryOperation binary_op) {
  return transform_reduce(pstl::execution::sycl, first, last, init, binary_op,
                          unary_op);
}
...
void stable_sort_by_key(Policy &&policy, RandomAccessIter1 keys_first,
  ...
  stable_sort_by_key(std::forward<Policy>(policy), keys_first, keys_last,
                     values_first, __pstl::internal::pstl_less());
}
