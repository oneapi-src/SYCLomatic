...
public:
  static size_t dump(json_stringstream &ss, std::ofstream &ofst, const T value,  queue_t queue) {
    ...
    ofst.write(reinterpret_cast<char*>(&size), sizeof(size_t));
    ofst.write(reinterpret_cast<const char*>(&value), size);
    return get_demangle_type_name<T>().length() + 1  + sizeof(size_t) + size;
  }
...
public:
  static size_t dump(json_stringstream &ss, std::ofstream &ofst, const sycl::float3 value,
                   queue_t queue) {
