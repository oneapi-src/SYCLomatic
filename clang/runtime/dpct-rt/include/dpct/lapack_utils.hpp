  ...
  auto handle_lapack_exception = [&](const oneapi::mkl::lapack::exception &e) {
    ...
    if (info)
      ::dpct::detail::dispatch::memcpy(
          q, info, &info_val, sizeof(int),
          ::dpct::detail::dispatch::memcpy_direction::host_to_device)
          .wait();
    return 1;
  };
  try {
    ...
    std::cerr << "Caught synchronous SYCL exception:" << std::endl
              << "reason: " << e.what() << std::endl;
    if (info)
      ::dpct::detail::dispatch::fill<unsigned char>(q, info, 0, sizeof(int))
          .wait();
    return 1;
  }
  return 0;
...
public:
  working_memory(std::size_t element_number, const sycl::queue &q) : _q(q) {
    _ptr = ::dpct::detail::dispatch::malloc(element_number * sizeof(T), _q);
  }
  ...
  ~working_memory() {
    if (_ptr) {
      ::dpct::detail::dispatch::enqueue_free({_ptr}, {_e}, _q);
    }
  }
...
template <typename T> struct syheevx_impl {
  void operator()(sycl::queue &q, oneapi::mkl::compz jobz,
    ...
    DISPATCH_FLOAT_FOR_CALCULATION(evx, q, jobz, range, uplo, n, a_data, lda,
                                   ...
                                   device_ws_data, device_ws_size);
    ::dpct::detail::dispatch::memcpy(
        q, a, z.get_ptr(), n * lda * sizeof(T),
        ::dpct::detail::dispatch::memcpy_direction::device_to_device);
    ::dpct::detail::dispatch::memcpy(
        q, m, m_device.get_ptr(), sizeof(std::int64_t),
