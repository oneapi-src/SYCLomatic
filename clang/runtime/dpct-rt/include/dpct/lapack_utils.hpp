  ...
  auto handle_lapack_exception = [&](const oneapi::mkl::lapack::exception &e) {
    ...
    if (info)
      ::dpct::detail::switcher::memcpy(
          ...
          ::dpct::detail::switcher::memcpy_direction::host_to_device)
          .wait();
    return 1;
  };
  try {
    ...
    std::cerr << "Caught synchronous SYCL exception:" << std::endl
              << "reason: " << e.what() << std::endl;
    if (info)
      ::dpct::detail::switcher::fill<unsigned char>(q, info, 0, sizeof(int))
          .wait();
    return 1;
  }
  return 0;
...
public:
  working_memory(std::size_t element_number, const sycl::queue &q) : _q(q) {
    _ptr = ::dpct::detail::switcher::malloc(element_number * sizeof(T), _q);
  }
  ...
  ~working_memory() {
    if (_ptr) {
      ::dpct::detail::switcher::enqueue_free({_ptr}, {_e}, _q);
    }
  }
