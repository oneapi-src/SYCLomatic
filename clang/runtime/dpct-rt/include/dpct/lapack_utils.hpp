  ...
  auto handle_lapack_exception = [&](const oneapi::mkl::lapack::exception &e) {
    ...
    if (info)
      ::dpct::detail::proxy::memcpy(
          ...
          ::dpct::detail::proxy::memcpy_direction::host_to_device)
          .wait();
    return 1;
  };
  try {
    ...
    std::cerr << "Caught synchronous SYCL exception:" << std::endl
              << "reason: " << e.what() << std::endl;
    if (info)
      ::dpct::detail::proxy::fill<unsigned char>(q, info, 0, sizeof(int))
          .wait();
    return 1;
  }
  return 0;
...
public:
  working_memory(std::size_t element_number, const sycl::queue &q) : _q(q) {
    _ptr = ::dpct::detail::proxy::malloc(element_number * sizeof(T), _q);
  }
  auto get_memory() {
  ...
  ~working_memory() {
    if (_ptr) {
      ::dpct::detail::proxy::enqueue_free({_ptr}, {_e}, _q);
    }
  }
