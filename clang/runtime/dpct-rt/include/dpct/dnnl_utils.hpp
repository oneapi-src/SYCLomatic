...
enum class order_t { col, row, col32, col4_4r2_8c, col32_2r_4r4 };
...
#define CASE(tag)                                                              \
  case attribute::tag:                                                         \
    ...
    switch (attr) {
      ...
      CASE(epilogue)
      CASE(bias_pointer)
      CASE(bias_batch_stride)
      CASE(epilogue_aux_pointer)
      CASE(epilogue_aux_ld)
      CASE(epilogue_aux_batch_stride)
      CASE(alpha_vector_batch_stride)
      CASE(sm_count_target)
      CASE(a_scale_pointer)
      CASE(b_scale_pointer)
      CASE(c_scale_pointer)
      CASE(d_scale_pointer)
      CASE(amax_d_pointer)
      CASE(epilogue_aux_data_type)
      CASE(epilogue_aux_scale_pointer)
      CASE(epilogue_aux_amax_pointer)
      CASE(fast_accum)
      CASE(bias_data_type)
      CASE(atomic_sync_in_counters_pointer)
      CASE(atomic_sync_out_counters_pointer)
      CASE(atomic_sync_num_chunks_d_rows)
      CASE(atomic_sync_num_chunks_d_cols)
    }
#undef CASE
  }
  ...
  std::int64_t _epilogue_aux_ld;
  std::int64_t _epilogue_aux_batch_stride;
  std::int64_t _alpha_vector_batch_stride;
  std::int32_t _sm_count_target;
  const void *_a_scale_pointer;
  const void *_b_scale_pointer;
  const void *_c_scale_pointer;
  const void *_d_scale_pointer;
  void *_amax_d_pointer;
  library_data_t _epilogue_aux_data_type;
  void *_epilogue_aux_scale_pointer;
  void *_epilogue_aux_amax_pointer;
  std::int8_t _fast_accum;
  library_data_t _bias_data_type;
  std::int32_t *_atomic_sync_in_counters_pointer;
  std::int32_t *_atomic_sync_out_counters_pointer;
...
void matmul(matmul_desc_ptr compute_desc, const void *alpha, const void *a,
  ...
  if (beta != nullptr) {
    ...
    std::memset(beta_zero, 0);
    q_ptr->memcpy(beta_host, beta, beta_size).wait();
    if (std::memcmp(beta_host, beta_zero))
