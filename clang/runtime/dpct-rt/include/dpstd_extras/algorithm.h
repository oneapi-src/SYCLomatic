...
std::pair<Iter1, Iter2> unique_by_key(Policy &&policy, Iter1 keys_first,
...
unique_by_key_copy(Policy &&policy, Iter1 keys_first, Iter1 keys_last,
                   Iter2 values_first, Iter3 keys_result, Iter4 values_result,
                   BinaryPred binary_pred) {
  auto ret_val = std::unique_copy(
      ...
      internal::unique_by_key_fun<BinaryPred>(binary_pred));
...
Iter3 copy_if(Policy &&policy, Iter1 first, Iter1 last, Iter2 mask,
  ...
  auto ret_val = std::copy_if(
      std::forward<Policy>(policy), dpstd::make_zip_iterator(first, mask),
      dpstd::make_zip_iterator(last, mask + std::distance(first, last)),
...
Iter4 transform_if(Policy &&policy, Iter1 first1, Iter1 last1, Iter2 first2,
  ...
  using ZipIterator = typename dpstd::zip_iterator<Iter1, Iter2, Iter3, Iter4>;
  using T = typename std::iterator_traits<ZipIterator>::value_type;
  std::for_each(
      std::forward<Policy>(policy),
      dpstd::make_zip_iterator(first1, first2, mask, result),
      dpstd::make_zip_iterator(last1, first2 + n, mask + n, result + n),
...
merge_by_key(Policy &&policy, Iter1 keys_first1, Iter1 keys_last1,
...
std::pair<Iter5, Iter6> merge_by_key(Policy &&policy, Iter1 keys_first1,
...
void sequence(Policy &&policy, Iter first, Iter last, T init, T step) {
...
template <class Policy, class Iter, class T>
void sequence(Policy &&policy, Iter first, Iter last, T init) {
  sequence(std::forward<Policy>(policy), first, last, init, T(1));
}

template <class Policy, class Iter>
void sequence(Policy &&policy, Iter first, Iter last) {
  using DiffSize = typename std::iterator_traits<Iter>::difference_type;
...
void sort_by_key(Policy &&policy, Iter1 keys_first, Iter1 keys_last,
...
template <class Policy, class Iter1, class Iter2, class Comp>
void stable_sort_by_key(Policy &&policy, Iter1 keys_first, Iter1 keys_last,
                        Iter2 values_first, Comp comp) {
  std::stable_sort(
      std::forward<Policy>(policy),
      dpstd::make_zip_iterator(keys_first, values_first),
...
void tabulate(Policy &&policy, Iter first, Iter last, Operator unary_op) {
  using DiffSize = typename std::iterator_traits<Iter>::difference_type;
  std::transform(std::forward<Policy>(policy),
                 dpstd::counting_iterator<DiffSize>(0),
                 dpstd::counting_iterator<DiffSize>(std::distance(first, last)),
...
set_intersection_by_key(Policy &&policy, Iter1 keys_first1, Iter1 keys_last1,
                        Iter2 keys_first2, Iter2 keys_last2,
                        Iter3 values_first1, Iter4 keys_result,
                        Iter5 values_result) {
...
std::pair<Iter5, Iter6> set_symmetric_difference_by_key(
...
template <class Policy, class Iter1, class Iter2, class Iter3, class Iter4,
          class Iter5, class Iter6>
std::pair<Iter5, Iter6>
set_difference_by_key(Policy &&policy, Iter1 keys_first1, Iter1 keys_last1,
                      Iter2 keys_first2, Iter2 keys_last2, Iter3 values_first1,
                      Iter4 values_first2, Iter5 keys_result,
                      Iter6 values_result) {
...
set_union_by_key(Policy &&policy, Iter1 keys_first1, Iter1 keys_last1,
                 Iter2 keys_first2, Iter2 keys_last2, Iter3 values_first1,
                 Iter4 values_first2, Iter5 keys_result, Iter6 values_result) {
