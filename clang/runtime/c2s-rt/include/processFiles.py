...
content_files_list = ["atomic", "blas_utils", "device",
                      "c2s", "dpl_utils", "image", "kernel", "memory", "util", "rng_utils", "lib_common_utils"]
...
content_files_name_list = ["Atomic", "BlasUtils", "Device",
                           "C2S", "DplUtils", "Image", "Kernel", "Memory", "Util", "RngUtils", "LibCommonUtils"]
...
def is_if_C2S_USM_LEVEL_NONE(line):
    line = line.strip()
    ...
    line = line.strip()
    if (line == bytes("C2S_USM_LEVEL_NONE", 'utf-8')):
        return True
    return False


def is_ifn_C2S_USM_LEVEL_NONE(line):
    line = line.strip()
...
def update_usm_status(line):
    need_skip = True
    if (is_pp_if(line)):
        if (is_if_C2S_USM_LEVEL_NONE(line)):
            stack.append(Usm_status_enum.NON_USM)
        elif (is_ifn_C2S_USM_LEVEL_NONE(line)):
...
def get_file_pathes(cont_file, inc_files_dir, runtime_files_dir, is_dpl_extras):
    if (is_dpl_extras):
        ...
        runtime_files_dir_result = os.path.join(runtime_files_dir, cont_file + ".hpp")
...
def process_a_file(cont_file, inc_files_dir, runtime_files_dir, is_dpl_extras, file_dict):
    ...
    for line in cont_file_handle:
        if (line.startswith(bytes("// C2S_LABEL_BEGIN", 'utf-8'))):
            line = line.replace(bytes('//', 'utf-8'), bytes('', 'utf-8'))
            line = line.strip()
            splited = line.split(bytes('|', 'utf-8'))
            content_begin_line = bytes("C2S_CONTENT_BEGIN(" + helper_file_enum_name + ", \"", 'utf-8') + \
            ...
            if (parent_feature == bytes("", 'utf-8')):
                inc_file_lines.append(bytes("C2S_PARENT_FEATURE(Unknown, \"\")\n", 'utf-8'))
            else:
                inc_file_lines.append(bytes("C2S_PARENT_FEATURE(" + helper_file_enum_name + ", \"", 'utf-8') + \
                                      parent_feature + bytes("\")\n", 'utf-8'))
            inc_file_lines.append(bytes("C2S_CONTENT_END\n", 'utf-8'))
            is_code = False
            have_usm_code = False
            has_code = False
            parent_feature = bytes("", 'utf-8')
        elif (line.startswith(bytes("// C2S_DEPENDENCY_BEGIN", 'utf-8'))):
            dependency_line = bytes("C2S_DEPENDENCY(", 'utf-8')
            is_dependency = True
        elif (line.startswith(bytes("// C2S_DEPENDENCY_END", 'utf-8'))):
            dependency_line = dependency_line + bytes(")\n", 'utf-8')
            inc_file_lines.append(dependency_line)
            is_dependency = False
            dependency_line = bytes("", 'utf-8')
        elif (line.startswith(bytes("// C2S_DEPENDENCY_EMPTY", 'utf-8'))):
            inc_file_lines.append(bytes("C2S_DEPENDENCY()\n", 'utf-8'))
        elif (line.startswith(bytes("// C2S_CODE", 'utf-8'))):
            is_code = True
            have_usm_code = False
            usm_line.clear()
            non_usm_line.clear()
        elif (line.startswith(bytes("// C2S_COMMENT", 'utf-8'))):
            continue
        elif (line.startswith(bytes("// C2S_PARENT_FEATURE", 'utf-8'))):
            line = line.replace(bytes('//', 'utf-8'), bytes('', 'utf-8'))
            line = line.strip()
            splited = line.split(bytes('|', 'utf-8'))
...
def main():
    ...
    if (args.build_dir):
        inc_files_dir = os.path.join(
            args.build_dir, "tools/clang/include/clang/C2S")
        runtime_files_dir = os.path.join(
            args.build_dir, "tools/clang/runtime/c2s-rt/include")
    else:
    ...
    features_enum_str = features_enum_str + bytes("#ifdef C2S_FEATURE_ENUM\n", 'utf-8')
    for element in features_enum_list:
        features_enum_str = features_enum_str + element + bytes(",\n", 'utf-8')
    features_enum_str = features_enum_str + bytes("#endif // C2S_FEATURE_ENUM\n", 'utf-8')
    features_enum_str = features_enum_str + bytes("#ifdef C2S_FEATURE_ENUM_FEATURE_PAIR_MAP\n", 'utf-8')
    for element in features_enum_pair_list:
        features_enum_str = features_enum_str + element + bytes(",\n", 'utf-8')
    features_enum_str = features_enum_str + bytes("#endif // C2S_FEATURE_ENUM_FEATURE_PAIR_MAP\n", 'utf-8')

    features_enum_file_handle.write(features_enum_str)
    features_enum_file_handle.close()

    print("[Note] C2S *.inc files are generated successfully!")
