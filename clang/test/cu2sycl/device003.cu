...
{
int deviceCount = 0;
// CHECK: checkErrors(deviceCount = cu2sycl::get_device_manager().device_count());
checkErrors(cudaGetDeviceCount(&deviceCount));

int dev_id;
// CHECK: checkErrors(dev_id = cu2sycl::get_device_manager().current_device_id());
checkErrors(cudaGetDevice(&dev_id));

cudaDeviceProp deviceProp;
// CHECK: checkErrors(deviceProp = cu2sycl::get_device_manager().get_device( 0).get_device_info());
checkErrors(cudaGetDeviceProperties(&deviceProp, 0));

int atomicSupported;
// CHECK: checkErrors(atomicSupported = cu2sycl::get_device_manager().get_device(  dev_id).is_native_atomic_supported());
...
// CHECK: checkErrors(cu2sycl::get_device_manager().select_device(device2));
