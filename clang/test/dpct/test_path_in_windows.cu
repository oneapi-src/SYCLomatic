// UNSUPPORTED: -linux-
// RUN: dpct --format-range=none --usm-level=none -out-root=%T -in-root=%S %S/test_PATH_in_Windows.cu --cuda-include-path="%cuda-path/include" --sycl-named-lambda --comments -- -x cuda --cuda-host-only
