...
// CHECK: /*
// CHECK: DPCT1124:{{[0-9]+}}: The constant compile-time initialization for device_globals is supported only when compiling with C++20. You may need to adjust the compile commands.
// CHECK: */
...
// CHECK:   arr_a.get()[0] = 1;
// CHECK:   return arr_a.get()[0] + arr_b.get()[0] + arr_c.get()[0] + arr_d.get()[0] + arr_e.get()[0].data + arr_f.get()[0].data;
// CHECK: }
