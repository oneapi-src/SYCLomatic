...
template<class T1, class T2, size_t S> void template_host() {
    ...
    // CHECK-NEXT:      template_kernel1<T2, T1>(a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      template_kernel2<T2>(a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      template_kernel3<T1, S>(a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      template_kernel4<T2>(a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
...
int main() {
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      template_kernel1<sycl::float4, int>(a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      template_kernel2<sycl::float2>(a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      template_kernel3<sycl::float2, 3>(a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      template_kernel4<sycl::float2>(a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
...
template<class T1, class T2, size_t S> void implicit_host() {
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_ptr(d_a, (T1 *)a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    kernel_ptr<<<1,1>>>(d_a);
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_ptr(d_b, (int *)a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    kernel_ptr<<<1,1>>>(d_b);
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_ptr(d_c, (typedef_1 *)a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    kernel_ptr<<<1,1>>>(d_c);
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_ptr(d_d, (using_1 *)a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    kernel_ptr<<<1,1>>>(d_d);
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_ref(r_a, (T1 *)a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    kernel_ref<<<1,1>>>(r_a);
    // CHECK:  sycl::local_accessor<int, 1> a_acc_ct1(sycl::range<1>(10), cgh);
    // CHECK-EMPTY:
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_ref(r_b, (int *)a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    kernel_ref<<<1,1>>>(r_b);
    // CHECK:  sycl::local_accessor<typedef_1, 1> a_acc_ct1(sycl::range<1>(10), cgh);
    // CHECK-EMPTY:
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_ref(r_c, (typedef_1 *)a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    kernel_ref<<<1,1>>>(r_c);
    // CHECK:  sycl::local_accessor<using_1, 1> a_acc_ct1(sycl::range<1>(10), cgh);
    // CHECK-EMPTY:
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_ref(r_d, (using_1 *)a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    kernel_ref<<<1,1>>>(r_d);
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_array(a_a, (T1 *)a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_array(a_b, (int *)a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_array(a_c, (typedef_1 *)a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_array(a_d, (using_1 *)a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_class(c_a, (T1 *)a1_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get(), (T2 *)a2_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_class(c_b, (T1 *)a1_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get(), (T2 *)a2_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_class(c_c, (T1 *)a1_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get(), (int *)a2_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_class(c_d, (int *)a1_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get(), (T2 *)a2_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_class(c_e, (int *)a1_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get(), (double *)a2_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_dependent(h_cpx, (std::complex<int> *)a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
    ...
    // CHECK-NEXT:    sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
    // CHECK-NEXT:    [=](sycl::nd_item<3> item_ct1) {
    // CHECK-NEXT:      kernel_dependent(h_cpx_2, (std::complex<double> *)a_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get());
    // CHECK-NEXT:    });
