...
void foo(){
  ...
  // CHECK: /*
  // CHECK-NEXT: DPCT1074:0: The SYCL image class doesn't support some of the flags used in the original code. Unsupported flags were ignored. Data read from the SYCL image could not be normalized as specified in the original code.
  // CHECK-NEXT: */
  ...
  // CHECK-NEXT: /*
  // CHECK-NEXT: DPCT1074:1: The SYCL image class doesn't support some of the flags used in the original code. Unsupported flags were ignored. Data read from the SYCL image could not be normalized as specified in the original code.
  // CHECK-NEXT: */
  // CHECK-NEXT: tex_tmp.set(sycl::addressing_mode::clamp_to_edge, sycl::filtering_mode::nearest, sycl::coordinate_normalization_mode::normalized);
  tex_tmp.flags = flags;
  tex_tmp.addressMode[0] = CU_TR_ADDRESS_MODE_CLAMP;
