...
// RUN: cat %s > h_hd_constant_db_win.cpp
// RUN: cat %S/constant_header.h > constant_header.h
// RUN: dpct h_hd_constant_db_win.cpp --out-root=./out --cuda-include-path="%cuda-path/include" -- -x c --cuda-host-only
// RUN: echo "[" > compile_commands.json
// RUN: echo "    {" >> compile_commands.json
// RUN: echo "        \"command\": \"ClCompile h_hd_constant_db_win.cpp\"," >> compile_commands.json
...
// RUN: cd ..
// RUN: rm -rf ./h_hd_constant_db_win
