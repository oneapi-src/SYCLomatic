...
int main(int argc, char* argv[]) {
  ...
  // CHECK: dpct::err_tmp0 e = (int)stop->get_info<sycl::info::event::command_execution_status>();
  ...
  // CHECK-NEXT:   }
  // CHECK-NEXT:   dpct::err_tmp0 et1, et2;
...
void foo() {
  ...
  {
    // CHECK: start_ct1 = std::chrono::steady_clock::now();
    // CHECK-NEXT: dpct::err_tmp0 err = (*start = q_ct1.ext_oneapi_submit_barrier(), 0);
