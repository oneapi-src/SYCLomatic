...
int main() {
  ...
  //CHECK-NEXT:*/
  //CHECK-NEXT:int res1 = CHECK_SYCL_ERROR(plan1->commit(&q_ct1, 3, n, inembed, istride, idist, onembed, ostride, odist, dpct::fft::fft_type::complex_double_to_real_double, 12, work_size));
  cufftResult res1 = cufftMakePlanMany(plan1, 3, n, inembed, istride, idist, onembed, ostride, odist, CUFFT_Z2D, 12, work_size);
  //CHECK:int res2 = CHECK_SYCL_ERROR((plan1->compute<sycl::double2, double>(idata, odata, dpct::fft::fft_direction::backward)));
  ...
  //CHECK-NEXT:DPCT1099:{{[0-9]+}}: Verify if the default value of the direction and placement used in the function "commit" is correct.
  //CHECK-NEXT:*/
  //CHECK-NEXT:res1 = CHECK_SYCL_ERROR(plan2->commit(&q_ct1, 3, n, inembed, istride, idist, onembed, ostride, odist, dpct::fft::fft_type::complex_double_to_real_double, 12, work_size));
  res1 = cufftMakePlanMany(plan2, 3, n, inembed, istride, idist, onembed, ostride, odist, CUFFT_Z2D, 12, work_size);
  //CHECK:res2 = CHECK_SYCL_ERROR((plan2->compute<sycl::double2, double>(idata, odata, dpct::fft::fft_direction::backward)));
  ...
  //CHECK-NEXT:DPCT1099:{{[0-9]+}}: Verify if the default value of the direction and placement used in the function "commit" is correct.
  //CHECK-NEXT:*/
  //CHECK-NEXT:HANDLE_CUFFT_ERROR(CHECK_SYCL_ERROR(plan3->commit(&q_ct1, 3, n, inembed, istride, idist, onembed, ostride, odist, dpct::fft::fft_type::complex_double_to_real_double, 12, work_size)));
  HANDLE_CUFFT_ERROR(cufftMakePlanMany(plan3, 3, n, inembed, istride, idist, onembed, ostride, odist, CUFFT_Z2D, 12, work_size));
  //CHECK:HANDLE_CUFFT_ERROR(CHECK_SYCL_ERROR((plan3->compute<sycl::double2, double>(idata, odata, dpct::fft::fft_direction::backward))));
  ...
  //CHECK-NEXT:DPCT1099:{{[0-9]+}}: Verify if the default value of the direction and placement used in the function "commit" is correct.
  //CHECK-NEXT:*/
  //CHECK-NEXT:if(CHECK_SYCL_ERROR(plan4->commit(&q_ct1, 3, n, inembed, istride, idist, onembed, ostride, odist, dpct::fft::fft_type::complex_double_to_real_double, 12, work_size))) {
  //CHECK-NEXT:/*
  //CHECK-NEXT:DPCT1100:{{[0-9]+}}: Currently the DFT external workspace feature in the Intel(R) oneAPI Math Kernel Library (oneMKL) is only supported on GPU devices. Use the internal workspace if your code should run on non-GPU devices.
  //CHECK-NEXT:*/
  //CHECK-NEXT:/*
  //CHECK-NEXT:DPCT1099:{{[0-9]+}}: Verify if the default value of the direction and placement used in the function "commit" is correct.
  //CHECK-NEXT:*/
  //CHECK-NEXT:} else if (CHECK_SYCL_ERROR(plan5->commit(&q_ct1, 3, n, inembed, istride, idist, onembed, ostride, odist, dpct::fft::fft_type::complex_double_to_real_double, 12, work_size))) {
  //CHECK-NEXT:}
  ...
  //CHECK:if (CHECK_SYCL_ERROR((plan4->compute<sycl::double2, double>(idata, odata, dpct::fft::fft_direction::backward)))) {
  //CHECK-NEXT:} else if(CHECK_SYCL_ERROR((plan5->compute<sycl::double2, double>(idata, odata, dpct::fft::fft_direction::backward)))) {
  //CHECK-NEXT:}
  ...
  //CHECK-NEXT:DPCT1099:{{[0-9]+}}: Verify if the default value of the direction and placement used in the function "commit" is correct.
  //CHECK-NEXT:*/
  //CHECK-NEXT:if(int res = CHECK_SYCL_ERROR(plan6->commit(&q_ct1, 3, n, inembed, istride, idist, onembed, ostride, odist, dpct::fft::fft_type::complex_double_to_real_double, 12, work_size))) {
  //CHECK-NEXT:}
  if(cufftResult res = cufftMakePlanMany(plan6, 3, n, inembed, istride, idist, onembed, ostride, odist, CUFFT_Z2D, 12, work_size)) {
  }
  //CHECK:if(int res = CHECK_SYCL_ERROR((plan6->compute<sycl::double2, double>(idata, odata, dpct::fft::fft_direction::backward)))) {
  //CHECK-NEXT:}
  ...
  //CHECK-NEXT:DPCT1099:{{[0-9]+}}: Verify if the default value of the direction and placement used in the function "commit" is correct.
  //CHECK-NEXT:*/
  //CHECK-NEXT:for (;CHECK_SYCL_ERROR(plan8->commit(&q_ct1, 3, n, inembed, istride, idist, onembed, ostride, odist, dpct::fft::fft_type::complex_double_to_real_double, 12, work_size));) {
  //CHECK-NEXT:}
  for (;cufftMakePlanMany(plan8, 3, n, inembed, istride, idist, onembed, ostride, odist, CUFFT_Z2D, 12, work_size);) {
  }

  //CHECK:for (;CHECK_SYCL_ERROR((plan8->compute<sycl::double2, double>(idata, odata, dpct::fft::fft_direction::backward)));) {
  //CHECK-NEXT:}
  ...
  //CHECK-NEXT:DPCT1099:{{[0-9]+}}: Verify if the default value of the direction and placement used in the function "commit" is correct.
  //CHECK-NEXT:*/
  //CHECK-NEXT:while (CHECK_SYCL_ERROR(plan9->commit(&q_ct1, 3, n, inembed, istride, idist, onembed, ostride, odist, dpct::fft::fft_type::complex_double_to_real_double, 12, work_size)) != 0) {
  //CHECK-NEXT:}
  while (cufftMakePlanMany(plan9, 3, n, inembed, istride, idist, onembed, ostride, odist, CUFFT_Z2D, 12, work_size) != 0) {
  }

  //CHECK:while (CHECK_SYCL_ERROR((plan9->compute<sycl::double2, double>(idata, odata, dpct::fft::fft_direction::backward))) != 0) {
  //CHECK-NEXT:}
  ...
  //CHECK-NEXT:DPCT1099:{{[0-9]+}}: Verify if the default value of the direction and placement used in the function "commit" is correct.
  //CHECK-NEXT:*/
  //CHECK-NEXT:} while (CHECK_SYCL_ERROR(plan10->commit(&q_ct1, 3, n, inembed, istride, idist, onembed, ostride, odist, dpct::fft::fft_type::complex_double_to_real_double, 12, work_size)));
  do {
  } while (cufftMakePlanMany(plan10, 3, n, inembed, istride, idist, onembed, ostride, odist, CUFFT_Z2D, 12, work_size));
  //CHECK:do {
  //CHECK-NEXT:} while (CHECK_SYCL_ERROR((plan10->compute<sycl::double2, double>(idata, odata, dpct::fft::fft_direction::backward))));
  do {
  ...
  //CHECK-NEXT:DPCT1099:{{[0-9]+}}: Verify if the default value of the direction and placement used in the function "commit" is correct.
  //CHECK-NEXT:*/
  //CHECK-NEXT:switch (int stat = CHECK_SYCL_ERROR(plan11->commit(&q_ct1, 3, n, inembed, istride, idist, onembed, ostride, odist, dpct::fft::fft_type::complex_double_to_real_double, 12, work_size))){
  //CHECK-NEXT:}
  switch (int stat = cufftMakePlanMany(plan11, 3, n, inembed, istride, idist, onembed, ostride, odist, CUFFT_Z2D, 12, work_size)){
  }

  //CHECK:switch (int stat = CHECK_SYCL_ERROR((plan11->compute<sycl::double2, double>(idata, odata, dpct::fft::fft_direction::backward)))){
  //CHECK-NEXT:}
...
cufftResult foo1(cufftHandle plan) {
  ...
  //CHECK-NEXT:DPCT1099:{{[0-9]+}}: Verify if the default value of the direction and placement used in the function "commit" is correct.
  //CHECK-NEXT:*/
  //CHECK-NEXT:return CHECK_SYCL_ERROR(plan->commit(&dpct::get_default_queue(), 3, n, inembed, istride, idist, onembed, ostride, odist, dpct::fft::fft_type::complex_double_to_real_double, 12, work_size));
...
cufftResult foo2(cufftHandle plan) {
  //CHECK:return CHECK_SYCL_ERROR((plan->compute<sycl::double2, double>(idata, odata, dpct::fft::fft_direction::backward)));
