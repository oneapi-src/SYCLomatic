...
// RUN: cd h_hd_constant_db
// RUN: cat %s > h_hd_constant_db.cpp
// RUN: cat %S/constant_header.h > constant_header.h
// RUN: dpct h_hd_constant_db.cpp --out-root=./out --cuda-include-path="%cuda-path/include" -- -x c --cuda-host-only
// RUN: echo "[" > compile_commands.json
// RUN: echo "    {" >> compile_commands.json
// RUN: echo "        \"command\": \"c++ h_hd_constant_db.cpp\"," >> compile_commands.json
