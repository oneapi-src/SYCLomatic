...
void foo() {
  //CHECK: copy_if_device(oneapi::dpl::execution::par_unseq);
  ...
  //CHECK: std::copy_if(oneapi::dpl::execution::par_unseq, h_data.begin(), h_data.end(), h_result.begin(), is_even<int>());
  //CHECK-NEXT: std::copy_if(oneapi::dpl::execution::par_unseq, h_data.begin(), h_data.end(), h_result.begin(), is_even<int>());
  ...
  //CHECK-NEXT: dpct::copy_if(oneapi::dpl::execution::par_unseq, range, last, (*data[0]).begin(), (*data[0]).end(), oneapi::dpl::identity());
  ...
  //CHECK-NEXT: int pot_cent_num = std::count_if(oneapi::dpl::execution::par_unseq, t, t + 10, [=] (int idx) { return true;});
...

 {
  ...
  //CHECK-NEXT: bool h_result_1 = std::transform_reduce(oneapi::dpl::execution::par_unseq, begin, end, 0, std::plus<bool>(), isfoo_test<int>());
...

{
  ...
  // CHECK: dpct::gather(oneapi::dpl::execution::par_unseq, h_map.begin(), h_map.end(), h_values.begin(), h_output.begin());
  // CHECK-NEXT: dpct::gather(oneapi::dpl::execution::par_unseq, h_map.begin(), h_map.end(), h_values.begin(), h_output.begin());
...

{
  ...
  // CHECK: dpct::scatter(oneapi::dpl::execution::par_unseq, h_values.begin(), h_values.end(), h_map.begin(), h_output.begin());
  // CHECK-NEXT: dpct::scatter(oneapi::dpl::execution::par_unseq, h_values.begin(), h_values.end(), h_map.begin(), h_output.begin());
...

{
  ...
  // CHECK: *new_last_vec.begin() = dpct::unique_copy(oneapi::dpl::execution::par_unseq, h_keys.begin(), h_keys.end(), h_values.begin(), h_output_keys.begin(), h_output_values.begin(), binary_pred);
  ...
  *new_last_vec.begin() = thrust::unique_by_key_copy(h_keys.begin(), h_keys.end(), h_values.begin(), h_output_keys.begin(), h_output_values.begin(), binary_pred);
  ...
  // CHECK: *new_last_vec.begin() = dpct::unique_copy(oneapi::dpl::execution::par_unseq, h_keys.begin(), h_keys.end(), h_values.begin(), h_output_keys.begin(), h_output_values.begin());
...
void mysort(Itr Beg, Itr End){
  ...
  // CHECK:  oneapi::dpl::sort(oneapi::dpl::execution::par_unseq, Beg, End);
  // CHECK:  oneapi::dpl::sort(oneapi::dpl::execution::par_unseq, Beg, End);
  ...
  // CHECK:  oneapi::dpl::sort(oneapi::dpl::execution::par_unseq, h_vec.begin(), h_vec.end());
  // CHECK:  oneapi::dpl::sort(oneapi::dpl::execution::par_unseq, h_vec.begin(), h_vec.end());
...
int main(void){
  ...
  // CHECK: oneapi::dpl::sort(oneapi::dpl::execution::make_device_policy(q_ct1), d_vec.begin(), d_vec.end());
  // CHECK: oneapi::dpl::sort(oneapi::dpl::execution::make_device_policy(q_ct1), d_vec.begin(), d_vec.end());
  // CHECK: oneapi::dpl::sort(oneapi::dpl::execution::par_unseq, d_vec.begin(), d_vec.end());
  thrust::sort(thrust::cuda::par.on(s1), d_vec.begin(), d_vec.end());
  thrust::sort(thrust::device, d_vec.begin(), d_vec.end());
  thrust::sort(d_vec.begin(), d_vec.end());
