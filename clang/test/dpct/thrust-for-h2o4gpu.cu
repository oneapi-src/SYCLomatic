...
void foo() {
  ...
  //CHECK: std::copy_if(dpstd::execution::default_policy, h_data.begin(), h_data.end(), h_result.begin(), is_even<int>());
  //CHECK-NEXT: std::copy_if(dpstd::execution::seq, h_data.begin(), h_data.end(), h_result.begin(), is_even<int>());
  //CHECK-NEXT: dpct::copy_if(dpstd::execution::default_policy, (*data[0]).begin(), (*data[0]).end(), range, d_new_potential_centroids.begin(),[=] (int idx) { return true; });
  ...
  //CHECK-NEXT: int pot_cent_num = std::count_if(dpstd::execution::default_policy, t, t + 10, [=] (int idx) { return true;});
  ...
  //CHECK-NEXT: std::fill(dpstd::execution::default_policy, dev_ptr, dev_ptr + 10, fill_value);
  //CHECK-NEXT: std::fill_n(dpstd::execution::default_policy, dev_ptr, 10, fill_value);
  //CHECK-NEXT: float M_inner = dpct::inner_product(dpstd::execution::default_policy, dev_ptr, dev_ptr + 10, dev_ptr, 0.0f);
...

 {
  ...
  //CHECK-NEXT: std::for_each( dpstd::execution::default_policy, t.begin(), t.end(), absolute_value<double>());
...

 {
  ...
  //CHECK-NEXT: dpct::sort_by_key(dpstd::execution::default_policy, a.begin(), b.end(), c.begin());
...

 {
  ...
  //CHECK-NEXT: auto ret = dpstd::make_zip_iterator(std::make_tuple(int_in.begin(), float_in.begin()));
...

 {
  ...
  //CHECK-NEXT: bool h_result = std::transform_reduce(dpstd::execution::default_policy, begin, end, isnan_test<int>(), 0, std::plus<bool>());
