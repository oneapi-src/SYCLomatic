...
int main() {
  ...
  //CHECK:{
  //CHECK-NEXT:a = (mkl::blas::rotm(handle, N, d_C_S, N, d_C_S, N, const_cast<float*>(x_S)).wait(), 0);
  //CHECK-NEXT:}
  a = cublasSrotm(handle, N, d_C_S, N, d_C_S, N, x_S);
  //CHECK:{
  //CHECK-NEXT:mkl::blas::rotm(handle, N, d_C_D, N, d_C_D, N, const_cast<double*>(x_D)).wait();
  //CHECK-NEXT:}
  ...
  // CHECK: {
  // CHECK-NEXT: mkl::blas::copy(handle, N, x_D, incx, d_C_D, incy).wait();
  // CHECK-NEXT: }
  ...
  // CHECK: {
  // CHECK-NEXT: mkl::blas::copy(handle, N, (std::complex<double>*)(x_Z), incx, (std::complex<double>*)(d_C_Z), incy).wait();
  // CHECK-NEXT: }
  ...
  // CHECK: {
  // CHECK-NEXT: mkl::blas::axpy(handle, N, *(&alpha_D), x_D, incx, result_D, incy).wait();
  // CHECK-NEXT: }
  ...
  // CHECK: {
  // CHECK-NEXT: mkl::blas::axpy(handle, N, std::complex<double>((&alpha_Z)->x(),(&alpha_Z)->y()), (std::complex<double>*)(x_Z), incx, (std::complex<double>*)(result_Z), incy).wait();
  // CHECK-NEXT: }
  ...
  // CHECK: {
  // CHECK-NEXT: mkl::blas::scal(handle, N, *(&alpha_D), result_D, incx).wait();
  // CHECK-NEXT: }
  ...
  // CHECK: {
  // CHECK-NEXT: mkl::blas::scal(handle, N, std::complex<double>((&alpha_Z)->x(),(&alpha_Z)->y()), (std::complex<double>*)(result_Z), incx).wait();
  // CHECK-NEXT: }
  ...
  // CHECK: {
  // CHECK-NEXT: mkl::blas::nrm2(handle, N, x_D, incx, result_D).wait();
  // CHECK-NEXT: }
  ...
  // CHECK: {
  // CHECK-NEXT: mkl::blas::nrm2(handle, N, (std::complex<double>*)(x_Z), incx, (double*)(result_D)).wait();
  // CHECK-NEXT: }
  ...
  // CHECK: {
  // CHECK-NEXT: mkl::blas::gemv(handle, mkl::transpose::nontrans, N, N, *(&alpha_D), x_D, lda, y_D, incx, *(&beta_D), result_D, incy).wait();
  // CHECK-NEXT: }
  ...
  // CHECK: {
  // CHECK-NEXT: mkl::blas::gemv(handle, mkl::transpose::nontrans, N, N, std::complex<double>((&alpha_Z)->x(),(&alpha_Z)->y()), (std::complex<double>*)(x_Z), lda, (std::complex<double>*)(y_Z), incx, std::complex<double>((&beta_Z)->x(),(&beta_Z)->y()), (std::complex<double>*)(result_Z), incy).wait();
  // CHECK-NEXT: }
  ...
  // CHECK: {
  // CHECK-NEXT: mkl::blas::ger(handle, N, N, *(&alpha_D), x_D, incx, y_D, incy, result_D, lda).wait();
  // CHECK-NEXT: }
  ...
  // CHECK: {
  // CHECK-NEXT: mkl::blas::gerc(handle, N, N, std::complex<float>((&alpha_C)->x(),(&alpha_C)->y()), (std::complex<float>*)(x_C), incx, (std::complex<float>*)(y_C), incy, (std::complex<float>*)(result_C), lda).wait();
  // CHECK-NEXT: }
  ...
  // CHECK: {
  // CHECK-NEXT: mkl::blas::gerc(handle, N, N, std::complex<double>((&alpha_Z)->x(),(&alpha_Z)->y()), (std::complex<double>*)(x_Z), incx, (std::complex<double>*)(y_Z), incy, (std::complex<double>*)(result_Z), lda).wait();
  // CHECK-NEXT: }
  ...
  //CHECK:{
  //CHECK-NEXT:mkl::blas::gemm(handle, mkl::transpose::nontrans, mkl::transpose::nontrans, N, N, N, std::complex<double>((&alpha_Z)->x(),(&alpha_Z)->y()), (std::complex<double>*)(d_A_Z), N, (std::complex<double>*)(d_B_Z), N, std::complex<double>((&beta_Z)->x(),(&beta_Z)->y()), (std::complex<double>*)(d_C_Z), N).wait();
  //CHECK-NEXT:}
