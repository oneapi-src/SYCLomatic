...
int main() {
  ...
  //CHECK-NEXT:oneapi::mkl::blas::column_major::iamax(*handle, N, x_S, N, res.get_memory(), oneapi::mkl::index_base::one);
  //CHECK-NEXT:return 0;
  //CHECK-NEXT:}();
  a = cublasIsamax(handle, N, x_S, N, result);
  //CHECK:[&]() {
  //CHECK-NEXT:dpct::blas::result_memory_int_t res(*handle, result);
  //CHECK-NEXT:oneapi::mkl::blas::column_major::iamax(*handle, N, x_D, N, res.get_memory(), oneapi::mkl::index_base::one);
  //CHECK-NEXT:return 0;
  //CHECK-NEXT:}();
  cublasIdamax(handle, N, x_D, N, result);
  //CHECK:a = [&]() {
  //CHECK-NEXT:dpct::blas::result_memory_int_t res(*handle, result);
  //CHECK-NEXT:oneapi::mkl::blas::column_major::iamax(*handle, N, (std::complex<float>*)x_C, N, res.get_memory(), oneapi::mkl::index_base::one);
  //CHECK-NEXT:return 0;
  //CHECK-NEXT:}();
  a = cublasIcamax(handle, N, x_C, N, result);
  //CHECK:[&]() {
  //CHECK-NEXT:dpct::blas::result_memory_int_t res(*handle, result);
  //CHECK-NEXT:oneapi::mkl::blas::column_major::iamax(*handle, N, (std::complex<double>*)x_Z, N, res.get_memory(), oneapi::mkl::index_base::one);
  //CHECK-NEXT:return 0;
  //CHECK-NEXT:}();
  ...
  //CHECK:a = [&]() {
  //CHECK-NEXT:dpct::blas::result_memory_int_t res(*handle, result);
  //CHECK-NEXT:oneapi::mkl::blas::column_major::iamin(*handle, N, x_S, N, res.get_memory(), oneapi::mkl::index_base::one);
  //CHECK-NEXT:return 0;
  //CHECK-NEXT:}();
  a = cublasIsamin(handle, N, x_S, N, result);
  //CHECK:[&]() {
  //CHECK-NEXT:dpct::blas::result_memory_int_t res(*handle, result);
  //CHECK-NEXT:oneapi::mkl::blas::column_major::iamin(*handle, N, x_D, N, res.get_memory(), oneapi::mkl::index_base::one);
  //CHECK-NEXT:return 0;
  //CHECK-NEXT:}();
  cublasIdamin(handle, N, x_D, N, result);
  //CHECK:a = [&]() {
  //CHECK-NEXT:dpct::blas::result_memory_int_t res(*handle, result);
  //CHECK-NEXT:oneapi::mkl::blas::column_major::iamin(*handle, N, (std::complex<float>*)x_C, N, res.get_memory(), oneapi::mkl::index_base::one);
  //CHECK-NEXT:return 0;
  //CHECK-NEXT:}();
  a = cublasIcamin(handle, N, x_C, N, result);
  //CHECK:[&]() {
  //CHECK-NEXT:dpct::blas::result_memory_int_t res(*handle, result);
  //CHECK-NEXT:oneapi::mkl::blas::column_major::iamin(*handle, N, (std::complex<double>*)x_Z, N, res.get_memory(), oneapi::mkl::index_base::one);
  //CHECK-NEXT:return 0;
  //CHECK-NEXT:}();
  ...
  //CHECK:a = DPCT_CHECK_ERROR(oneapi::mkl::blas::column_major::gemm(*handle, dpct::get_transpose(trans0), dpct::get_transpose(trans1), N, N, N, dpct::get_value(&alpha_S, *handle), d_A_S, N, d_B_S, N, dpct::get_value(&beta_S, *handle), d_C_S, N));
  a = cublasSgemm(handle, (cublasOperation_t)trans0, (cublasOperation_t)trans1, N, N, N, &alpha_S, d_A_S, N, d_B_S, N, &beta_S, d_C_S, N);
  //CHECK:oneapi::mkl::blas::column_major::gemm(*handle, dpct::get_transpose(trans0), dpct::get_transpose(trans1), N, N, N, dpct::get_value(&alpha_D, *handle), d_A_D, N, d_B_D, N, dpct::get_value(&beta_D, *handle), d_C_D, N);
  cublasDgemm(handle, (cublasOperation_t)trans0, (cublasOperation_t)trans1, N, N, N, &alpha_D, d_A_D, N, d_B_D, N, &beta_D, d_C_D, N);
  //CHECK:a = DPCT_CHECK_ERROR(oneapi::mkl::blas::column_major::gemm(*handle, oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans, N, N, N, dpct::get_value(&alpha_C, *handle), (std::complex<float>*)d_A_C, N, (std::complex<float>*)d_B_C, N, dpct::get_value(&beta_C, *handle), (std::complex<float>*)d_C_C, N));
  ...
  // CHECK: if(int stat = DPCT_CHECK_ERROR(oneapi::mkl::blas::column_major::gemm(*handle, dpct::get_transpose(trans0), dpct::get_transpose(trans1), N, N, N, dpct::get_value(&alpha_S, *handle), d_A_S, N, d_B_S, N, dpct::get_value(&beta_S, *handle), d_C_S, N))){}
...
// CHECK:int foo2() try {
// CHECK:  return DPCT_CHECK_ERROR(oneapi::mkl::blas::column_major::gemm(*handle, dpct::get_transpose(trans0), dpct::get_transpose(trans1), N, N, N, dpct::get_value(&alpha_S, *handle), d_A_S, N, d_B_S, N, dpct::get_value(&beta_S, *handle), d_C_S, N));
// CHECK-NEXT:}
int foo2(){
