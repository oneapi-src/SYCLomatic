...
int main() {
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: a = (oneapi::mkl::blas::column_major::axpy(*handle, N, dpct::get_value(&alpha_S, *handle), x_S, incx, result_S, incy), 0);
  a = cublasSaxpy(handle, N, &alpha_S, x_S, incx, result_S, incy);
  // CHECK:oneapi::mkl::blas::column_major::axpy(*handle, N, dpct::get_value(&alpha_D, *handle), x_D, incx, result_D, incy);
  cublasDaxpy(handle, N, &alpha_D, x_D, incx, result_D, incy);
  // CHECK: /*
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: a = (oneapi::mkl::blas::column_major::axpy(*handle, N, dpct::get_value(&alpha_C, *handle), (std::complex<float>*)x_C, incx, (std::complex<float>*)result_C, incy), 0);
  a = cublasCaxpy(handle, N, &alpha_C, x_C, incx, result_C, incy);
  // CHECK:oneapi::mkl::blas::column_major::axpy(*handle, N, dpct::get_value(&alpha_Z, *handle), (std::complex<double>*)x_Z, incx, (std::complex<double>*)result_Z, incy);
  cublasZaxpy(handle, N, &alpha_Z, x_Z, incx, result_Z, incy);

  // CHECK: /*
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: a = (oneapi::mkl::blas::column_major::scal(*handle, N, dpct::get_value(&alpha_S, *handle), result_S, incx), 0);
  a = cublasSscal(handle, N, &alpha_S, result_S, incx);
  // CHECK:oneapi::mkl::blas::column_major::scal(*handle, N, dpct::get_value(&alpha_D, *handle), result_D, incx);
  cublasDscal(handle, N, &alpha_D, result_D, incx);
  // CHECK: /*
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: a = (oneapi::mkl::blas::column_major::scal(*handle, N, dpct::get_value(&alpha_C, *handle), (std::complex<float>*)result_C, incx), 0);
  a = cublasCscal(handle, N, &alpha_C, result_C, incx);
  // CHECK:oneapi::mkl::blas::column_major::scal(*handle, N, dpct::get_value(&alpha_Z, *handle), (std::complex<double>*)result_Z, incx);
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: a = (oneapi::mkl::blas::column_major::gemv(*handle, trans2==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans2, N, N, dpct::get_value(&alpha_S, *handle), x_S, lda, y_S, incx, dpct::get_value(&beta_S, *handle), result_S, incy), 0);
  a = cublasSgemv(handle, (cublasOperation_t)trans2, N, N, &alpha_S, x_S, lda, y_S, incx, &beta_S, result_S, incy);
  // CHECK:oneapi::mkl::blas::column_major::gemv(*handle, oneapi::mkl::transpose::nontrans, N, N, dpct::get_value(&alpha_D, *handle), x_D, lda, y_D, incx, dpct::get_value(&beta_D, *handle), result_D, incy);
  cublasDgemv(handle, CUBLAS_OP_N, N, N, &alpha_D, x_D, lda, y_D, incx, &beta_D, result_D, incy);
  // CHECK: /*
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: a = (oneapi::mkl::blas::column_major::gemv(*handle, trans2==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans2, N, N, dpct::get_value(&alpha_C, *handle), (std::complex<float>*)x_C, lda, (std::complex<float>*)y_C, incx, dpct::get_value(&beta_C, *handle), (std::complex<float>*)result_C, incy), 0);
  a = cublasCgemv(handle, (cublasOperation_t)trans2, N, N, &alpha_C, x_C, lda, y_C, incx, &beta_C, result_C, incy);
  // CHECK:oneapi::mkl::blas::column_major::gemv(*handle, oneapi::mkl::transpose::nontrans, N, N, dpct::get_value(&alpha_Z, *handle), (std::complex<double>*)x_Z, lda, (std::complex<double>*)y_Z, incx, dpct::get_value(&beta_Z, *handle), (std::complex<double>*)result_Z, incy);
  cublasZgemv(handle, CUBLAS_OP_N, N, N, &alpha_Z, x_Z, lda, y_Z, incx, &beta_Z, result_Z, incy);

  // CHECK: /*
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: a = (oneapi::mkl::blas::column_major::ger(*handle, N, N, dpct::get_value(&alpha_S, *handle), x_S, incx, y_S, incy, result_S, lda), 0);
  a = cublasSger(handle, N, N, &alpha_S, x_S, incx, y_S, incy, result_S, lda);
  // CHECK:oneapi::mkl::blas::column_major::ger(*handle, N, N, dpct::get_value(&alpha_D, *handle), x_D, incx, y_D, incy, result_D, lda);
  cublasDger(handle, N, N, &alpha_D, x_D, incx, y_D, incy, result_D, lda);
  // CHECK: /*
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: a = (oneapi::mkl::blas::column_major::geru(*handle, N, N, dpct::get_value(&alpha_C, *handle), (std::complex<float>*)x_C, incx, (std::complex<float>*)y_C, incy, (std::complex<float>*)result_C, lda), 0);
  a = cublasCgeru(handle, N, N, &alpha_C, x_C, incx, y_C, incy, result_C, lda);
  // CHECK:oneapi::mkl::blas::column_major::gerc(*handle, N, N, dpct::get_value(&alpha_C, *handle), (std::complex<float>*)x_C, incx, (std::complex<float>*)y_C, incy, (std::complex<float>*)result_C, lda);
  cublasCgerc(handle, N, N, &alpha_C, x_C, incx, y_C, incy, result_C, lda);
  // CHECK: /*
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: a = (oneapi::mkl::blas::column_major::geru(*handle, N, N, dpct::get_value(&alpha_Z, *handle), (std::complex<double>*)x_Z, incx, (std::complex<double>*)y_Z, incy, (std::complex<double>*)result_Z, lda), 0);
  a = cublasZgeru(handle, N, N, &alpha_Z, x_Z, incx, y_Z, incy, result_Z, lda);
  // CHECK:oneapi::mkl::blas::column_major::gerc(*handle, N, N, dpct::get_value(&alpha_Z, *handle), (std::complex<double>*)x_Z, incx, (std::complex<double>*)y_Z, incy, (std::complex<double>*)result_Z, lda);
  ...
  //CHECK-NEXT:DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  //CHECK-NEXT:*/
  //CHECK-NEXT:a = (oneapi::mkl::blas::column_major::gemm(*handle, trans0==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans0, trans1==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans1, N, N, N, dpct::get_value(&alpha_S, *handle), d_A_S, N, d_B_S, N, dpct::get_value(&beta_S, *handle), d_C_S, N), 0);
  a = cublasSgemm(handle, (cublasOperation_t)trans0, (cublasOperation_t)trans1, N, N, N, &alpha_S, d_A_S, N, d_B_S, N, &beta_S, d_C_S, N);
  //CHECK:oneapi::mkl::blas::column_major::gemm(*handle, trans0==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans0, trans1==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans1, N, N, N, dpct::get_value(&alpha_D, *handle), d_A_D, N, d_B_D, N, dpct::get_value(&beta_D, *handle), d_C_D, N);
  cublasDgemm(handle, (cublasOperation_t)trans0, (cublasOperation_t)trans1, N, N, N, &alpha_D, d_A_D, N, d_B_D, N, &beta_D, d_C_D, N);
  //CHECK:/*
  //CHECK-NEXT:DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  //CHECK-NEXT:*/
  //CHECK-NEXT:a = (oneapi::mkl::blas::column_major::gemm(*handle, oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans, N, N, N, dpct::get_value(&alpha_C, *handle), (std::complex<float>*)d_A_C, N, (std::complex<float>*)d_B_C, N, dpct::get_value(&beta_C, *handle), (std::complex<float>*)d_C_C, N), 0);
  a = cublasCgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, N, N, N, &alpha_C, d_A_C, N, d_B_C, N, &beta_C, d_C_C, N);
  //CHECK:oneapi::mkl::blas::column_major::gemm(*handle, oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans, N, N, N, dpct::get_value(&alpha_Z, *handle), (std::complex<double>*)d_A_Z, N, (std::complex<double>*)d_B_Z, N, dpct::get_value(&beta_Z, *handle), (std::complex<double>*)d_C_Z, N);
  cublasZgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N, N, N, N, &alpha_Z, d_A_Z, N, d_B_Z, N, &beta_Z, d_C_Z, N);

  //CHECK:/*
  //CHECK-NEXT:DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  //CHECK-NEXT:*/
  //CHECK-NEXT:a = (oneapi::mkl::blas::column_major::gemm_batch(*handle, trans0==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans0, trans1==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans1, N, N, N, dpct::get_value(&alpha_S, *handle), d_A_S, N, 16, d_B_S, N, 16, dpct::get_value(&beta_S, *handle), d_C_S, N, 16, 10), 0);
  ...
  //CHECK:oneapi::mkl::blas::column_major::gemm_batch(*handle, trans0==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans0, trans1==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans1, N, N, N, dpct::get_value(&alpha_D, *handle), d_A_D, N, 16, d_B_D, N, 16, dpct::get_value(&beta_D, *handle), d_C_D, N, 16, 10);
  ...
  //CHECK-NEXT:DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  //CHECK-NEXT:*/
  //CHECK-NEXT:a = (oneapi::mkl::blas::column_major::gemm_batch(*handle, oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans, N, N, N, dpct::get_value(&alpha_C, *handle), (std::complex<float>*)d_A_C, N, 16, (std::complex<float>*)d_B_C, N, 16, dpct::get_value(&beta_C, *handle), (std::complex<float>*)d_C_C, N, 16, 10), 0);
  a = cublasCgemmStridedBatched(handle, CUBLAS_OP_N, CUBLAS_OP_N, N, N, N, &alpha_C, d_A_C, N, 16, d_B_C, N, 16, &beta_C, d_C_C, N, 16, 10);
  //CHECK:oneapi::mkl::blas::column_major::gemm_batch(*handle, oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans, N, N, N, dpct::get_value(&alpha_Z, *handle), (std::complex<double>*)d_A_Z, N, 16, (std::complex<double>*)d_B_Z, N, 16, dpct::get_value(&beta_Z, *handle), (std::complex<double>*)d_C_Z, N, 16, 10);
  cublasZgemmStridedBatched(handle, CUBLAS_OP_N, CUBLAS_OP_N, N, N, N, &alpha_Z, d_A_Z, N, 16, d_B_Z, N, 16, &beta_Z, d_C_Z, N, 16, 10);
  //CHECK:oneapi::mkl::blas::column_major::gemm_batch(*handle, oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans, N, N, N, dpct::get_value(&alpha_H, *handle), d_A_H, N, 16, d_B_H, N, 16, dpct::get_value(&beta_H, *handle), d_C_H, N, 16, 10);
  ...
  //CHECK-NEXT:DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  //CHECK-NEXT:*/
  //CHECK-NEXT:a = (oneapi::mkl::blas::column_major::gemm(*handle, trans3, trans3, N, N, N, dpct::get_value(&alpha_H, *handle), d_A_H, N, d_B_H, N, dpct::get_value(&beta_H, *handle), d_C_H, N), 0);
  ...
  // CHECK: oneapi::mkl::blas::column_major::gemm(*handle, oneapi::mkl::transpose::conjtrans, oneapi::mkl::transpose::conjtrans, N, N, N, sycl::vec<float, 1>{dpct::get_value(&alpha_S, *handle)}.convert<sycl::half, sycl::rounding_mode::automatic>()[0], (sycl::half*)A, N, (sycl::half*)B, N, sycl::vec<float, 1>{dpct::get_value(&beta_S, *handle)}.convert<sycl::half, sycl::rounding_mode::automatic>()[0], (sycl::half*)C, N);
  //CHECK-NEXT:oneapi::mkl::blas::column_major::gemm(*handle, oneapi::mkl::transpose::conjtrans, oneapi::mkl::transpose::conjtrans, N, N, N, dpct::get_value(&alpha_S, *handle), (sycl::half*)A, N, (sycl::half*)B, N, dpct::get_value(&beta_S, *handle), (float*)C, N);
  //CHECK-NEXT:oneapi::mkl::blas::column_major::gemm(*handle, oneapi::mkl::transpose::conjtrans, oneapi::mkl::transpose::conjtrans, N, N, N, dpct::get_value(&alpha_S, *handle), (float*)A, N, (float*)B, N, dpct::get_value(&beta_S, *handle), (float*)C, N);
  //CHECK-NEXT:oneapi::mkl::blas::column_major::gemm(*handle, oneapi::mkl::transpose::conjtrans, oneapi::mkl::transpose::conjtrans, N, N, N, dpct::get_value(&alpha_C, *handle), (std::complex<float>*)A, N, (std::complex<float>*)B, N, dpct::get_value(&beta_C, *handle), (std::complex<float>*)C, N);
  ...
  //CHECK-NEXT:DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  //CHECK-NEXT:*/
  //CHECK-NEXT:a = (oneapi::mkl::blas::column_major::trmm(*handle, (oneapi::mkl::side)side0, fill0==0 ? oneapi::mkl::uplo::lower : oneapi::mkl::uplo::upper, trans0==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans0, (oneapi::mkl::diag)diag0, N, N, dpct::get_value(&alpha_S, *handle), d_A_S, N, d_C_S, N), 0);
  ...
  //CHECK-NEXT:oneapi::mkl::blas::column_major::trmm(*handle, (oneapi::mkl::side)side0, fill0==0 ? oneapi::mkl::uplo::lower : oneapi::mkl::uplo::upper, trans0==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans0, (oneapi::mkl::diag)diag0, N, N, dpct::get_value(&alpha_D, *handle), d_A_D, N, d_C_D, N);
  ...
  //CHECK-NEXT:DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  //CHECK-NEXT:*/
  //CHECK-NEXT:a = (oneapi::mkl::blas::column_major::trmm(*handle, oneapi::mkl::side::left, oneapi::mkl::uplo::lower, oneapi::mkl::transpose::nontrans, oneapi::mkl::diag::unit, N, N, dpct::get_value(&alpha_C, *handle), (std::complex<float>*)d_A_C, N, (std::complex<float>*)d_C_C, N), 0);
  ...
  //CHECK-NEXT:oneapi::mkl::blas::column_major::trmm(*handle, oneapi::mkl::side::left, oneapi::mkl::uplo::lower, oneapi::mkl::transpose::nontrans, oneapi::mkl::diag::unit, N, N, dpct::get_value(&alpha_Z, *handle), (std::complex<double>*)d_A_Z, N, (std::complex<double>*)d_C_Z, N);
  ...
  //CHECK:a = (oneapi::mkl::blas::column_major::gemmt(*handle, fill0==0 ? oneapi::mkl::uplo::lower : oneapi::mkl::uplo::upper, trans1==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans1, trans1==0 ? oneapi::mkl::transpose::trans : oneapi::mkl::transpose::nontrans, N, N, dpct::get_value(&alpha_S, *handle), d_A_S, N, d_B_S, N, dpct::get_value(&beta_S, *handle), d_C_S, N), 0);
  a = cublasSsyrkx(handle, (cublasFillMode_t)fill0, (cublasOperation_t)trans1, N, N, &alpha_S, d_A_S, N, d_B_S, N, &beta_S, d_C_S, N);
  //CHECK:oneapi::mkl::blas::column_major::gemmt(*handle, fill0==0 ? oneapi::mkl::uplo::lower : oneapi::mkl::uplo::upper, trans1==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans1, trans1==0 ? oneapi::mkl::transpose::trans : oneapi::mkl::transpose::nontrans, N, N, dpct::get_value(&alpha_D, *handle), d_A_D, N, d_B_D, N, dpct::get_value(&beta_D, *handle), d_C_D, N);
  cublasDsyrkx(handle, (cublasFillMode_t)fill0, (cublasOperation_t)trans1, N, N, &alpha_D, d_A_D, N, d_B_D, N, &beta_D, d_C_D, N);



  // CHECK: dpct::matrix_mem_copy(d_C_S, d_B_S, N, N, N, N, dpct::device_to_device, *handle);
  // CHECK-NEXT: oneapi::mkl::blas::column_major::trmm(*handle, (oneapi::mkl::side)side0, fill0==0 ? oneapi::mkl::uplo::lower : oneapi::mkl::uplo::upper, trans0==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans0, (oneapi::mkl::diag)diag0, N, N, dpct::get_value(&alpha_S, *handle), d_A_S, N, d_C_S, N);
  // CHECK-NEXT: /*
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: if(int stat = (oneapi::mkl::blas::column_major::gemm(*handle, trans0==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans0, trans1==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans1, N, N, N, dpct::get_value(&alpha_S, *handle), d_A_S, N, d_B_S, N, dpct::get_value(&beta_S, *handle), d_C_S, N), 0)){}
...
// CHECK-NEXT:  DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
// CHECK-NEXT:  */
// CHECK-NEXT:  return (oneapi::mkl::blas::column_major::gemm(*handle, trans0==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans0, trans1==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans1, N, N, N, dpct::get_value(&alpha_S, *handle), d_A_S, N, d_B_S, N, dpct::get_value(&beta_S, *handle), d_C_S, N), 0);
// CHECK-NEXT:}
int foo2(){
