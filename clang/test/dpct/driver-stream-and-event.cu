...
void foo(){
  ...
  //CHECK: /*
  //CHECK-NEXT: DPCT1132:{{[0-9]+}}: SYCL 2020 don't support accessing the statically allocated shared memory for then kernel. The API is replaced with member variable "shared_size_bytes". Please set the appropriate value for "shared_size_bytes".
  //CHECK-NEXT: */
  ...
  //CHECK: /*
  //CHECK-NEXT: DPCT1132:{{[0-9]+}}: SYCL 2020 don't support accessing the local memory for then kernel. The API is replaced with member variable "local_size_bytes". Please set the appropriate value for "local_size_bytes".
  //CHECK-NEXT: */
  ...
  //CHECK: /*
  //CHECK-NEXT: DPCT1132:{{[0-9]+}}: SYCL 2020 don't support accessing the memory size of user-defined constants for then kernel. The API is replaced with member variable "const_size_bytes". Please set the appropriate value for "const_size_bytes".
  //CHECK-NEXT: */
  ...
  //CHECK: /*
  //CHECK-NEXT: DPCT1132:{{[0-9]+}}: SYCL 2020 don't support accessing the required number of registers for then kernel. The API is replaced with member variable "num_regs". Please set the appropriate value for "num_regs".
  //CHECK-NEXT: */
  //CHECK: rr = dpct::get_kernel_function_info(f).num_regs /* number of register for each thread */;
