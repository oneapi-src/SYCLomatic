...
// RUN: cd %T
// RUN: c2s --no-cl-namespace-inline -out-root %T/formatMigratedGoogle formatMigratedGoogle.cu --cuda-include-path="%cuda-path/include" --format-style=google -- -std=c++14  -x cuda --cuda-host-only
