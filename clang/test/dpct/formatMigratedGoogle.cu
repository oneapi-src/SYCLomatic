...
// RUN: cd %T
// RUN: dpct --no-dpcpp-extensions=free-function-queries  --out-root %T/formatMigratedGoogle formatMigratedGoogle.cu --cuda-include-path="%cuda-path/include" --format-style=google -- -std=c++14  -x cuda --cuda-host-only
