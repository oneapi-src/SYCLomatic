// RUN: dpct --format-range=none -out-root %T/test_for_ctst2018 %s --cuda-include-path="%cuda-path/include" --report-type=stats -- -x cuda --cuda-host-only
// RUN: FileCheck --input-file %T/test_for_ctst2018/test_for_ctst2018.dp.cpp --match-full-lines %s
#include <stdio.h>

// CHECK: void foo(){}
__global__ void foo(){}

// Code piece below is used to catch crash of SIGSEGV reported by CTST-2018
