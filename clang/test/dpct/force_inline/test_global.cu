// RUN: dpct --no-dpcpp-extensions=free-function-queries --format-range=none dpct/force_inline/test_global.cu -in-root %S -out-root %T/force_inline %S/test_device.cu %S/test_global.cu --optimize-migration --cuda-include-path="%cuda-path/include" -- -std=c++14 -x cuda --cuda-host-only
