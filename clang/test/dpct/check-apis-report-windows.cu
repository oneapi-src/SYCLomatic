// UNSUPPORTED: -linux-
// RUN: c2s -report-type=apis -report-file-prefix=check_apis_report -out-root %T/check_apis_report-windows %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
...
// RUN: c2s  -output-file=output_file_all_windows.txt -report-type=apis -report-file-prefix=stdout -out-root %T/check_apis_report-windows %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
...
// RUN: c2s -report-file-prefix=report -report-type=apis  -report-format=formatted -report-only  -out-root %T/check_apis_report-windows %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
...
// RUN: c2s -output-file=output-file.txt -out-root %T/check_apis_report-windows %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
...
// RUN: c2s -output-verbosity=silent  -out-root %T/check_apis_report-windows %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only  2>&1  \
...
// RUN: c2s -output-verbosity=normal  -out-root %T/check_apis_report-windows %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only  2>&1  \
// RUN: | FileCheck -check-prefix=FAKE-FILE-CHECK -allow-empty %s

// RUN: c2s -output-verbosity=detailed  -out-root %T/check_apis_report-windows %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only  2>&1  \
