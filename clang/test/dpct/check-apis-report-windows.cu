...
// RUN: cat %S/check-apis-report_csv_ref_windows.txt > %T/check_apis_report_csv_check_windows.txt
...
// RUN: FileCheck --match-full-lines --input-file %T/check_apis_report_check_windows.txt %T/check_apis_report_check_windows.txt

// RUN: dpct -output-file=output-file.txt -out-root %T %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
// RUN: cat %S/check-apis-report_outputfile_ref_window.txt > %T/check_outputfile_windows.txt
// RUN: cat %T/output-file.txt >>%T/check_outputfile_windows.txt
// RUN: FileCheck --match-full-lines --input-file %T/check_outputfile_windows.txt %T/check_outputfile_windows.txt
