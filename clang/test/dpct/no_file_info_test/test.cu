// RUN: c2s --format-range=none --usm-level=none -in-root %S -out-root %T/no_file_info_test %S/test.cu %s -extra-arg="-I %S" --cuda-include-path="%cuda-path/include" --sycl-named-lambda -- -std=c++11 -x cuda --cuda-host-only
