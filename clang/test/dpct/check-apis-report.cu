...
// RUN: cat %S/check-apis-report_csv_ref.txt > %T/check_apis_report_csv_check.txt
...
// RUN: dpct -output-file=output_file_all.txt -report-type=apis -report-file-prefix=stdout -out-root %T %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
// RUN: cat %S/check-apis-report_outputfile_ref_all.txt > %T/check_output_file_all.txt
// RUN: cat %T/output_file_all.txt >>%T/check_output_file_all.txt
// RUN: FileCheck --match-full-lines --input-file %T/check_output_file_all.txt %T/check_output_file_all.txt
...
// RUN: cat %S/check-apis-report_log_ref.txt > %T/check_apis_report_check.txt
...
// RUN: cat %S/check-apis-report_outputfile_ref.txt > %T/check_output-file.txt
