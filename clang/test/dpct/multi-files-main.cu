// RUN: dpct --format-range=none --usm-level=none -in-root %S -out-root %T %s %S/multi-files-device.cuh --cuda-include-path="%cuda-path/include" --sycl-named-lambda -- -x cuda --cuda-host-only
