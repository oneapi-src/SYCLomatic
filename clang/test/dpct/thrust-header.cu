...
// UNSUPPORTED: v8.0
// RUN: dpct --enable-profiling=0  --format-range=none -out-root %T/thrust-header %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only -std=c++17
...
// CHECK:  dpct::iota(oneapi::dpl::execution::make_device_policy(q_ct1), mapspvalT, mapspvalT + numsH);
