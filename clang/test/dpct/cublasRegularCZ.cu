...
int main(){
  ...
  // CHECK-NEXT: sycl::buffer<std::complex<float>> buffer_ct{{[0-9]+}} = allocation_ct{{[0-9]+}}.buffer.reinterpret<std::complex<float>>(sycl::range<1>(allocation_ct{{[0-9]+}}.size/sizeof(std::complex<float>)));
  ...
  // CHECK-NEXT: sycl::buffer<std::complex<double>> buffer_ct{{[0-9]+}} = allocation_ct{{[0-9]+}}.buffer.reinterpret<std::complex<double>>(sycl::range<1>(allocation_ct{{[0-9]+}}.size/sizeof(std::complex<double>)));
  // CHECK-NEXT: auto allocation_ct{{[0-9]+}} = dpct::mem_mgr::instance().translate_ptr(result);
  // CHECK-NEXT: sycl::buffer<int> buffer_ct{{[0-9]+}} = allocation_ct{{[0-9]+}}.buffer.reinterpret<int>(sycl::range<1>(allocation_ct{{[0-9]+}}.size/sizeof(int)));
  // CHECK-NEXT: sycl::buffer<int64_t> result_temp_buffer(sycl::range<1>(1));
  // CHECK-NEXT: status = (mkl::blas::iamax(handle, n, buffer_ct{{[0-9]+}}, incx, result_temp_buffer), 0);
