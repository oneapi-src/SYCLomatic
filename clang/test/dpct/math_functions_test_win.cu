// UNSUPPORTED: -linux-
// RUN: dpct --format-range=none -out-root %T/math_functions_test_win %s --cuda-include-path="%cuda-path/include" --no-dpcpp-extensions=c_cxx_standard_library -- -x cuda --cuda-host-only
