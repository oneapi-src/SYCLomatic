...
// CHECK-NEXT: {
// CHECK-NEXT:     typedef typename If<bar<sycl::mlonglong2>::temp, int,int>::t3 Word;
// CHECK-NEXT: };
template <typename T>
struct foo_1
{
...
// CHECK-NEXT:     sycl::mlonglong2>::temp, int,int>::t3 Word;
// CHECK-NEXT: };
template <typename T>
struct foo_2
{
...
// CHECK: template<>
// CHECK-NEXT: struct type2<float> { typedef sycl::mfloat2  type; };
