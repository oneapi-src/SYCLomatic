...
// CHECK-NEXT:                kernel(stream_ct1, *a_ptr_ct1, b_acc_ct1, al_ptr_ct1, cl_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get(), reinterpret_cast<int (*)[12]>(bl_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get()), dpct::image_accessor_ext<sycl::uint2, 1>(tex21_smpl, tex21_acc));
// CHECK-NEXT:              });
// CHECK-NEXT:          });
// CHECK-NEXT:    }
