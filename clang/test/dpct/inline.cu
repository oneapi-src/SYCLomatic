// RUN: c2s --format-range=none -out-root %T/inline %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
...
// CHECK: __c2s_inline__ void kernel1(sycl::nd_item<3> [[ITEM:item_ct1]], float *out) {
...
// CHECK: #define NEW_INLINE __c2s_inline__
...
// CHECK: #define INLINE_KERNEL3 __c2s_inline__  void kernel3() {int a = 2;}
...
// CHECK: extern __c2s_inline__ void error(void);
