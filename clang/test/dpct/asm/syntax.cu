...
// RUN: dpct --format-range=none -out-root %T/syntax %s --cuda-include-path="%cuda-path/include" -- -std=c++14 -x cuda --cuda-host-only
// RUN: FileCheck %s --match-full-lines --input-file %T/syntax/syntax.dp.cpp
// RUN: %if build_lit %{icpx -c -fsycl %T/syntax/syntax.dp.cpp -o %T/syntax/syntax.dp.o %}
...
__device__ void unsupport(void* dst, const void* src) {
