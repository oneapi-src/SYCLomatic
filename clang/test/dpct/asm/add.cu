...
// RUN: dpct --format-range=none -out-root %T/add %s --cuda-include-path="%cuda-path/include" -- -std=c++14 -x cuda --cuda-host-only
// RUN: FileCheck %s --match-full-lines --input-file %T/add/add.dp.cpp
// RUN: %if build_lit %{icpx -c -fsycl %T/add/add.dp.cpp -o %T/add/add.dp.o %}
