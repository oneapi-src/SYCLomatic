...
// RUN: dpct --no-dpcpp-extensions=free-function-queries --format-range=none -out-root %T/cvta %s --cuda-include-path="%cuda-path/include" -- -std=c++14 -x cuda --cuda-host-only --no-dpcpp-extensions=free-function-queries
