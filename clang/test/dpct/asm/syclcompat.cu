...
__global__ void vset(unsigned *d) {
  int e, f;
  // CHECK: DPCT1131:{{.*}}: The migration of "vset.s32.s32.eq %0, %1, %2, %3;" is not supported with SYCLcompat currently, please adjust the code manually.
  asm("vset.s32.s32.eq %0, %1, %2, %3;" : "=r"(*d) : "r"(e), "r"(f), "r"(0));
}

__device__ int vset2() {
  int i;
  // CHECK: DPCT1131:{{.*}}: The migration of "vset.s32.s32.eq %0, %1, %2;" is not supported with SYCLcompat currently, please adjust the code manually.
...
__global__ void vset4(unsigned *d) {
  int e, f;
  // CHECK: DPCT1131:{{.*}}: The migration of "vset4.s32.s32.eq %0, %1, %2, %3;" is not supported with SYCLcompat currently, please adjust the code manually.
