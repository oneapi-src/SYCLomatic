...
int main(){
  ...
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (oneapi::mkl::blas::column_major::trmm(*handle, (oneapi::mkl::side)side0, fill0==0 ? oneapi::mkl::uplo::lower : oneapi::mkl::uplo::upper, trans0==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans0, (oneapi::mkl::diag)diag0, m, n, dpct::get_value(&alpha_S, *handle), A_S_buf_ct{{[0-9]+}}, lda, C_S_buf_ct{{[0-9]+}}, ldc), 0);
  // CHECK-NEXT: }
  // CHECK-NEXT: {
  // CHECK-NEXT: dpct::matrix_mem_copy(C_S, B_S, ldc, ldb, m, n, dpct::device_to_device, *handle);
  // CHECK-NEXT: auto A_S_buf_ct{{[0-9]+}} = dpct::get_buffer<float>(A_S);
  // CHECK-NEXT: auto C_S_buf_ct{{[0-9]+}} = dpct::get_buffer<float>(C_S);
  // CHECK-NEXT: oneapi::mkl::blas::column_major::trmm(*handle, oneapi::mkl::side::left, oneapi::mkl::uplo::upper, oneapi::mkl::transpose::nontrans, oneapi::mkl::diag::nonunit, m, n, dpct::get_value(&alpha_S, *handle), A_S_buf_ct{{[0-9]+}}, lda, C_S_buf_ct{{[0-9]+}}, ldc);
  // CHECK-NEXT: }
  ...
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (oneapi::mkl::blas::column_major::trmm(*handle, (oneapi::mkl::side)side1, fill1==0 ? oneapi::mkl::uplo::lower : oneapi::mkl::uplo::upper, trans1==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans1, (oneapi::mkl::diag)diag1, m, n, dpct::get_value(&alpha_D, *handle), A_D_buf_ct{{[0-9]+}}, lda, C_D_buf_ct{{[0-9]+}}, ldc), 0);
  // CHECK-NEXT: }
  // CHECK-NEXT: {
  // CHECK-NEXT: dpct::matrix_mem_copy(C_D, B_D, ldc, ldb, m, n, dpct::device_to_device, *handle);
  // CHECK-NEXT: auto A_D_buf_ct{{[0-9]+}} = dpct::get_buffer<double>(A_D);
  // CHECK-NEXT: auto C_D_buf_ct{{[0-9]+}} = dpct::get_buffer<double>(C_D);
  // CHECK-NEXT: oneapi::mkl::blas::column_major::trmm(*handle, oneapi::mkl::side::left, oneapi::mkl::uplo::upper, oneapi::mkl::transpose::nontrans, oneapi::mkl::diag::nonunit, m, n, dpct::get_value(&alpha_D, *handle), A_D_buf_ct{{[0-9]+}}, lda, C_D_buf_ct{{[0-9]+}}, ldc);
  // CHECK-NEXT: }
  ...
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (oneapi::mkl::blas::column_major::trmm(*handle, oneapi::mkl::side::left, oneapi::mkl::uplo::lower, trans2==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)trans2, oneapi::mkl::diag::nonunit, m, n, dpct::get_value(&alpha_C, *handle), A_C_buf_ct{{[0-9]+}}, lda, C_C_buf_ct{{[0-9]+}}, ldc), 0);
  // CHECK-NEXT: }
  // CHECK-NEXT: {
  // CHECK-NEXT: dpct::matrix_mem_copy(C_C, B_C, ldc, ldb, m, n, dpct::device_to_device, *handle);
  // CHECK-NEXT: auto A_C_buf_ct{{[0-9]+}} = dpct::get_buffer<std::complex<float>>(A_C);
  // CHECK-NEXT: auto C_C_buf_ct{{[0-9]+}} = dpct::get_buffer<std::complex<float>>(C_C);
  // CHECK-NEXT: oneapi::mkl::blas::column_major::trmm(*handle, oneapi::mkl::side::left, oneapi::mkl::uplo::upper, oneapi::mkl::transpose::nontrans, oneapi::mkl::diag::nonunit, m, n, dpct::get_value(&alpha_C, *handle), A_C_buf_ct{{[0-9]+}}, lda, C_C_buf_ct{{[0-9]+}}, ldc);
  // CHECK-NEXT: }
  ...
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (oneapi::mkl::blas::column_major::trmm(*handle, oneapi::mkl::side::right, oneapi::mkl::uplo::upper, oneapi::mkl::transpose::conjtrans, oneapi::mkl::diag::unit, m, n, dpct::get_value(&alpha_Z, *handle), A_Z_buf_ct{{[0-9]+}}, lda, C_Z_buf_ct{{[0-9]+}}, ldc), 0);
  // CHECK-NEXT: }
  // CHECK-NEXT: {
  // CHECK-NEXT: dpct::matrix_mem_copy(C_Z, B_Z, ldc, ldb, m, n, dpct::device_to_device, *handle);
  // CHECK-NEXT: auto A_Z_buf_ct{{[0-9]+}} = dpct::get_buffer<std::complex<double>>(A_Z);
  // CHECK-NEXT: auto C_Z_buf_ct{{[0-9]+}} = dpct::get_buffer<std::complex<double>>(C_Z);
  // CHECK-NEXT: oneapi::mkl::blas::column_major::trmm(*handle, oneapi::mkl::side::left, oneapi::mkl::uplo::upper, oneapi::mkl::transpose::nontrans, oneapi::mkl::diag::nonunit, m, n, dpct::get_value(&alpha_Z, *handle), A_Z_buf_ct{{[0-9]+}}, lda, C_Z_buf_ct{{[0-9]+}}, ldc);
  // CHECK-NEXT: }
  ...
  // CHECK-NEXT: oneapi::mkl::blas::column_major::trmm(*handle, oneapi::mkl::side::left, oneapi::mkl::uplo::upper, (int)transpose_ct{{[0-9]+}}==2 ? oneapi::mkl::transpose::conjtrans : (oneapi::mkl::transpose)transpose_ct{{[0-9]+}}, oneapi::mkl::diag::nonunit, m, n, dpct::get_value(&alpha_Z, *handle), A_Z_buf_ct{{[0-9]+}}, lda,  foo2_ptr_ct{{[0-9]+}}_buf_ct{{[0-9]+}}, ldc);
  // CHECK-NEXT: }
