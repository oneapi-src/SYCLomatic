...
// CHECK-NEXT: /*
// CHECK-NEXT: DPCT1112:{{[0-9]+}}: The filtering mode may be "linear". You may need to adjust the code.
// CHECK-NEXT: */
...
int main() {
  ...
  // CHECK-NEXT:       sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
  // CHECK-NEXT:       [=](sycl::nd_item<3> item_ct1) {
  // CHECK-NEXT:         kernel(dpct::image_accessor_ext<sycl::uint2, 1>(tex21_smpl, tex21_acc), dpct::image_accessor_ext<sycl::float4, 2>(tex42_smpl, tex42_acc));
  // CHECK-NEXT:       });
  // CHECK-NEXT:   });
