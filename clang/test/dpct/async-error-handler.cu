...
int main() {
  ...
  // CHECK: s0 = new sycl::queue(dpct::get_default_context(), dpct::get_current_device(), dpct::exception_handler);
  cudaStreamCreate(&s0);

  // CHECK: /*
  // CHECK-NEXT: DPCT1025:{{[0-9]+}}: The SYCL queue is created ignoring the flag/priority options.
  // CHECK-NEXT: */
  // CHECK-NEXT: s1 = new sycl::queue(dpct::get_default_context(), dpct::get_current_device(), dpct::exception_handler);
  cudaStreamCreateWithFlags(&s1, cudaStreamDefault);

  // CHECK: /*
  // CHECK-NEXT: DPCT1025:{{[0-9]+}}: The SYCL queue is created ignoring the flag/priority options.
  // CHECK-NEXT: */
  // CHECK-NEXT: s2 = new sycl::queue(dpct::get_default_context(), dpct::get_current_device(), dpct::exception_handler);
