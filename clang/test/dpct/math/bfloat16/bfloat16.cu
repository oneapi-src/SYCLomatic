...
__global__ void test_conversions_device(__nv_bfloat16 *deviceArrayBFloat16) {
  ...
  // CHECK: i = sycl::vec<float, 1>(bf16).template convert<int, sycl::rounding_mode::rtn>()[0];
  i = __bfloat162int_rd(bf16);
  // CHECK: i = sycl::vec<float, 1>(bf16).template convert<int, sycl::rounding_mode::rte>()[0];
  i = __bfloat162int_rn(bf16);
  // CHECK: i = sycl::vec<float, 1>(bf16).template convert<int, sycl::rounding_mode::rtp>()[0];
  i = __bfloat162int_ru(bf16);
  // CHECK: i = sycl::vec<float, 1>(bf16).template convert<int, sycl::rounding_mode::rtz>()[0];
  i = __bfloat162int_rz(bf16);
  // CHECK: ll = sycl::vec<float, 1>(bf16).template convert<long long, sycl::rounding_mode::rtn>()[0];
  ll = __bfloat162ll_rd(bf16);
  // CHECK: ll = sycl::vec<float, 1>(bf16).template convert<long long, sycl::rounding_mode::rte>()[0];
  ll = __bfloat162ll_rn(bf16);
  // CHECK: ll = sycl::vec<float, 1>(bf16).template convert<long long, sycl::rounding_mode::rtp>()[0];
  ll = __bfloat162ll_ru(bf16);
  // CHECK: ll = sycl::vec<float, 1>(bf16).template convert<long long, sycl::rounding_mode::rtz>()[0];
  ll = __bfloat162ll_rz(bf16);
  // CHECK: s = sycl::vec<float, 1>(bf16).template convert<short, sycl::rounding_mode::rtn>()[0];
  s = __bfloat162short_rd(bf16);
  // CHECK: s = sycl::vec<float, 1>(bf16).template convert<short, sycl::rounding_mode::rte>()[0];
  s = __bfloat162short_rn(bf16);
  // CHECK: s = sycl::vec<float, 1>(bf16).template convert<short, sycl::rounding_mode::rtp>()[0];
  s = __bfloat162short_ru(bf16);
  // CHECK: s = sycl::vec<float, 1>(bf16).template convert<short, sycl::rounding_mode::rtz>()[0];
  s = __bfloat162short_rz(bf16);
  // CHECK: u = sycl::vec<float, 1>(bf16).template convert<unsigned, sycl::rounding_mode::rtn>()[0];
  u = __bfloat162uint_rd(bf16);
  // CHECK: u = sycl::vec<float, 1>(bf16).template convert<unsigned, sycl::rounding_mode::rte>()[0];
  u = __bfloat162uint_rn(bf16);
  // CHECK: u = sycl::vec<float, 1>(bf16).template convert<unsigned, sycl::rounding_mode::rtp>()[0];
  u = __bfloat162uint_ru(bf16);
  // CHECK: u = sycl::vec<float, 1>(bf16).template convert<unsigned, sycl::rounding_mode::rtz>()[0];
  u = __bfloat162uint_rz(bf16);
  // CHECK: ull = sycl::vec<float, 1>(bf16).template convert<unsigned long long, sycl::rounding_mode::rtn>()[0];
  ull = __bfloat162ull_rd(bf16);
  // CHECK: ull = sycl::vec<float, 1>(bf16).template convert<unsigned long long, sycl::rounding_mode::rte>()[0];
  ull = __bfloat162ull_rn(bf16);
  // CHECK: ull = sycl::vec<float, 1>(bf16).template convert<unsigned long long, sycl::rounding_mode::rtp>()[0];
  ull = __bfloat162ull_ru(bf16);
  // CHECK: ull = sycl::vec<float, 1>(bf16).template convert<unsigned long long, sycl::rounding_mode::rtz>()[0];
  ull = __bfloat162ull_rz(bf16);
  // CHECK: us = sycl::vec<float, 1>(bf16).template convert<unsigned short, sycl::rounding_mode::rtn>()[0];
  us = __bfloat162ushort_rd(bf16);
  // CHECK: us = sycl::vec<float, 1>(bf16).template convert<unsigned short, sycl::rounding_mode::rte>()[0];
  us = __bfloat162ushort_rn(bf16);
  // CHECK: us = sycl::vec<float, 1>(bf16).template convert<unsigned short, sycl::rounding_mode::rtp>()[0];
  us = __bfloat162ushort_ru(bf16);
  // CHECK: us = sycl::vec<float, 1>(bf16).template convert<unsigned short, sycl::rounding_mode::rtz>()[0];
