...
// RUN: dpct --no-dpcpp-extensions=free-function-queries --format-range=none --use-experimental-features=matrix -out-root %T/wmma_using_nvcuda %s --cuda-include-path="%cuda-path/include" -- -std=c++14 -x cuda --cuda-host-only
