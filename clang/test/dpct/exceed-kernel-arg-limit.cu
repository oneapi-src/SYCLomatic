...
// RUN: FileCheck --input-file %T/exceed-kernel-arg-limit.dp.cpp --match-full-lines %s
...
int main() {
    ...
    //CHECK: int *global;
    //CHECK-NEXT: /*
    //CHECK-NEXT: DPCT1042:{{[0-9]+}}: The size of the arguments passed to the SYCL kernel exceeds the minimum size limit (1024) for non-custom SYCL device. You can get the hardware argument size limit by querying info::device::max_parameter_size and may need rewrite this code, if the size of the arguments exceeds hardware limit.
    //CHECK-NEXT: */
    int *global;
