...
// RUN: mkdir %T/vector_add_format_force_migration
// RUN: cat %s > %T/vector_add_format_force_migration/vector_add_format_force_migration.cu
// RUN: cd %T/vector_add_format_force_migration
// RUN: dpct -out-root %T/vector_add_format_force_migration --force-migration vector_add_format_force_migration.cu --cuda-include-path="%cuda-path/include" -- -std=c++14  -x cuda --cuda-host-only
// RUN: FileCheck -strict-whitespace %s --match-full-lines --input-file %T/vector_add_format_force_migration/vector_add_format_force_migration.dp.cpp
// RUN: cd ..
// RUN: rm -rf ./vector_add_format_force_migration
