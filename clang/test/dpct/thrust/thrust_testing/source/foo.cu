...
// UNSUPPORTED: v8.0
// RUN: dpct -out-root %T/thrust/thrust_testing %s --cuda-include-path="%cuda-path/include" -extra-arg="-I%S" -- -x cuda --cuda-host-only -std=c++17
...
int main() {
  int a = bar(); // to check SYCLomatic parses user defined complex.h
  return 0;
}