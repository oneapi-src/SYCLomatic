...
// RUN: cd dh_d_constant_db
// RUN: cat %s > dh_d_constant_db.cpp
// RUN: cat %S/constant_header.h > constant_header.h
// RUN: echo "[" > compile_commands.json
// RUN: echo "    {" >> compile_commands.json
// RUN: echo "        \"command\": \"nvcc dh_d_constant_db.cpp\"," >> compile_commands.json
// RUN: echo "        \"directory\": \"%/T/dh_d_constant_db\"," >> compile_commands.json
// RUN: echo "        \"file\": \"%/T/dh_d_constant_db/dh_d_constant_db.cpp\"" >> compile_commands.json
// RUN: echo "    }," >> compile_commands.json
// RUN: echo "    {" >> compile_commands.json
// RUN: echo "        \"command\": \"c++ dh_d_constant_db.cpp\"," >> compile_commands.json
// RUN: echo "        \"directory\": \"%/T/dh_d_constant_db\"," >> compile_commands.json
// RUN: echo "        \"file\": \"%/T/dh_d_constant_db/dh_d_constant_db.cpp\"" >> compile_commands.json
// RUN: echo "    }" >> compile_commands.json
// RUN: echo "]" >> compile_commands.json
...
// RUN: c2s dh_d_constant_db.cpp --out-root=./out --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
// RUN: FileCheck %s --match-full-lines --input-file %T/dh_d_constant_db/out/constant_header.h
// RUN: cd ..
// RUN: rm -rf ./dh_d_constant_db
