...
int main() {
    ...
    // CHECK-NEXT: mkl::blas::gemm(handle, (int)transpose_ct1==2 ? mkl::transpose::conjtrans : (mkl::transpose)transpose_ct1, (int)transpose_ct2==2 ? mkl::transpose::conjtrans : (mkl::transpose)transpose_ct2, N, N, N, alpha_S, d_A_S_buf_ct1, N, d_B_S_buf_ct1, N, beta_S, d_C_S_buf_ct1, N);
    // CHECK-NEXT: return 0;
    // CHECK-NEXT: }());
    ...
    //CHECK-NEXT: auto d_B_S_buf_ct1 = dpct::get_buffer<float>(d_B_S);
    //CHECK-NEXT: auto d_C_S_buf_ct1 = dpct::get_buffer<float>(d_C_S);
    //CHECK-NEXT: mkl::blas::gemmt(handle, (int)fill0==0 ? mkl::uplo::lower : mkl::uplo::upper, (int)transpose_ct2==2 ? mkl::transpose::conjtrans : (mkl::transpose)transpose_ct2, (int)transpose_ct2==0 ? mkl::transpose::trans : mkl::transpose::nontrans, N, N, alpha_S, d_A_S_buf_ct1, N, d_B_S_buf_ct1, N, beta_S, d_C_S_buf_ct1, N);
    // CHECK-NEXT: return 0;
    //CHECK-NEXT: }());
    ...
    // CHECK-NEXT: mkl::blas::trmm(handle, (mkl::side)side0, (int)fill0==0 ? mkl::uplo::lower : mkl::uplo::upper, (int)transpose_ct3==2 ? mkl::transpose::conjtrans : (mkl::transpose)transpose_ct3, (mkl::diag)diag0, m_ct5, n_ct6, alpha_S, ptr_ct8_buf_ct1, N, ptr_ct12_buf_ct1, ld_ct13);
    // CHECK-NEXT: return 0;
    // CHECK-NEXT: }());
    ...
    // CHECK-NEXT:mkl::blas::gemm(handle, (int)transpose_ct{{[0-9]+}}==2 ? mkl::transpose::conjtrans : (mkl::transpose)transpose_ct{{[0-9]+}}, (int)transpose_ct{{[0-9]+}}==2 ? mkl::transpose::conjtrans : (mkl::transpose)transpose_ct{{[0-9]+}}, N, N, N, std::complex<float>(alpha_C.x(),alpha_C.y()), d_A_C_buf_ct1, N, d_B_C_buf_ct1, N, std::complex<float>(beta_C.x(),beta_C.y()), d_C_C_buf_ct1, N);
    // CHECK-NEXT:return 0;
    // CHECK-NEXT:}());
    ...
    // CHECK-NEXT:auto ptr_ct{{[0-9]+}}_buf_ct1 = dpct::get_buffer<std::complex<float>>(ptr_ct{{[0-9]+}});
    ...
    // CHECK-NEXT:mkl::blas::trmm(handle, (mkl::side)side0, (int)fill0==0 ? mkl::uplo::lower : mkl::uplo::upper, (int)transpose_ct{{[0-9]+}}==2 ? mkl::transpose::conjtrans : (mkl::transpose)transpose_ct{{[0-9]+}}, (mkl::diag)diag0, m_ct{{[0-9]+}}, n_ct{{[0-9]+}}, std::complex<float>(alpha_C.x(),alpha_C.y()), ptr_ct{{[0-9]+}}_buf_ct1, N,  ptr_ct{{[0-9]+}}_buf_ct1, ld_ct{{[0-9]+}});
    // CHECK-NEXT:return 0;
    // CHECK-NEXT:}());
    ...
    //CHECK-NEXT:mkl::rng::generate(distr_ct1, rng, (100 + 1) * (200) * 4, h_data_buf_ct1);
    //CHECK-NEXT:return 0;
    //CHECK-NEXT:}());
    //CHECK-NEXT:/*
