...
int main() {
    ...
    // CHECK-NEXT: oneapi::mkl::blas::column_major::iamax(handle->get_queue(), N, dpct::rvalue_ref_to_lvalue_ref(dpct::get_buffer<float>(x_S)), N, dpct::rvalue_ref_to_lvalue_ref(dpct::get_buffer<std::int64_t>(res.get_memory())), oneapi::mkl::index_base::one);
    // CHECK-NEXT: return 0;
    // CHECK-NEXT: }());
    ...
    // CHECK: cublasErrCheck([&]() {
    // CHECK-NEXT: dpct::blas::out_mem_int_t res(handle->get_queue(), result);
    // CHECK-NEXT: oneapi::mkl::blas::column_major::iamax(handle->get_queue(), N, dpct::rvalue_ref_to_lvalue_ref(dpct::get_buffer<std::complex<float>>(x_C)), N, dpct::rvalue_ref_to_lvalue_ref(dpct::get_buffer<std::int64_t>(res.get_memory())), oneapi::mkl::index_base::one);
    // CHECK-NEXT: return 0;
    // CHECK-NEXT: }());
