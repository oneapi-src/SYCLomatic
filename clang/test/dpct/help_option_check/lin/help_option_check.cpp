...
// RUN: dpct --no-dpcpp-extensions=free-function-queries --help > output.txt
// RUN: diff --strip-trailing-cr %S/help_all.txt %T/help_option_check/output.txt >> %T/diff.txt
// RUN: dpct --no-dpcpp-extensions=free-function-queries --help=basic > output.txt
// RUN: diff --strip-trailing-cr %S/help_basic.txt %T/help_option_check/output.txt >> %T/diff.txt
// RUN: dpct --no-dpcpp-extensions=free-function-queries --help=advanced > output.txt
