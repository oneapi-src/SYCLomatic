...
int main() {
  ...
  // CHECK: /*
  // CHECK-NEXT: DPCT1123:{{[0-9]+}}: The kernel function pointer cannot be deduced correctly. You need cast "kernel_func" to a function pointer. According to the kernel function definition, adjusting the kernel function arguments and the dimension of the sycl::nd_item may be also required.
  // CHECK-NEXT: */
