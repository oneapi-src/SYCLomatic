...
void foo_test_2() {
    ...
    // CHECK:    CHECK(CHECK_SYCL_ERROR(*stop = q_ct1.ext_oneapi_submit_barrier()));
...
// CHECK:  CHECK(CHECK_SYCL_ERROR(*start = q_ct1.ext_oneapi_submit_barrier()));
  ...
  // CHECK-NEXT:  CHECK(CHECK_SYCL_ERROR(stop->wait_and_throw()));
  // CHECK-NEXT:  float execution_time;
  // CHECK-NEXT:  CHECK(CHECK_SYCL_ERROR(execution_time = (stop->get_profiling_info<sycl::info::event_profiling::command_end>() - start->get_profiling_info<sycl::info::event_profiling::command_start>()) / 1000000.0f));  
...
// CHECK: SAFE_CALL(CHECK_SYCL_ERROR(*start = q_ct1.ext_oneapi_submit_barrier()));
  ...
  // CHECK:  SAFE_CALL(CHECK_SYCL_ERROR(*stop = q_ct1.ext_oneapi_submit_barrier()));
  // CHECK:  SAFE_CALL(CHECK_SYCL_ERROR(stop->wait_and_throw()));
...
// CHECK:    CHECK(CHECK_SYCL_ERROR(*stop = q_ct1.ext_oneapi_submit_barrier()));
// CHECK:    CHECK(CHECK_SYCL_ERROR(stop->wait_and_throw()));
    CHECK(cudaEventRecord(stop, 0));
    CHECK(cudaEventSynchronize(stop));
