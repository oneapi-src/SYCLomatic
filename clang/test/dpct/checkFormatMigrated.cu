...
// RUN: cd %T
// RUN: c2s --no-cl-namespace-inline -out-root %T/checkFormatMigrated checkFormatMigrated.cu --cuda-include-path="%cuda-path/include" -- -std=c++14  -x cuda --cuda-host-only
