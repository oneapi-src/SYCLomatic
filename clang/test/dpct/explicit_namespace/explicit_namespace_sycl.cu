// RUN: c2s --format-range=none -in-root %S -out-root %T/explicit_namespace_sycl %S/explicit_namespace_sycl.cu --cuda-include-path="%cuda-path/include" --use-explicit-namespace=sycl --sycl-named-lambda -- -x cuda --cuda-host-only
