...
// RUN: dpct --enable-profiling=0 --format-range=none -out-root %T/dup_using_namespace %s --cuda-include-path="%cuda-path/include" --use-explicit-namespace=sycl -- -x cuda --cuda-host-only --std=c++14
