// RUN: c2s --format-range=none -out-root %T/kernel-usm %s --usm-level=restricted --cuda-include-path="%cuda-path/include" --sycl-named-lambda -- -x cuda --cuda-host-only -std=c++14
...
int main() {
  ...
  int karg3 = 80;
  // CHECK:   q_ct1.parallel_for<c2s_kernel_name<class testKernelPtr_{{[a-f0-9]+}}>>(
...
int run_foo11() {
  // CHECK:q_ct1.parallel_for<c2s_kernel_name<class my_kernel3_{{[0-9a-z]+}}>>(
  // CHECK-NEXT:      sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
  // CHECK-NEXT:      [=](sycl::nd_item<3> item_ct1) {
  // CHECK-NEXT:        my_kernel3();
  // CHECK-NEXT:      });
  // CHECK-NEXT:q_ct1.parallel_for<c2s_kernel_name<class my_kernel3_{{[0-9a-z]+}}>>(
  // CHECK-NEXT:      sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
  // CHECK-NEXT:      [=](sycl::nd_item<3> item_ct1) {
  // CHECK-NEXT:        my_kernel3();
  // CHECK-NEXT:      });
