...
int foo(int aaaaa){
  ...
  //CHECK-NEXT: descrA = mkl::index_base::zero;
  //CHECK-NEXT: /*
  //CHECK-NEXT: DPCT1045:{{[0-9]+}}: The Intel(R) DPC++ Compatibility Tool only supports to migrate general sparse matrix type for this API. If the sparse matrix is other type, you need adjust the code.
  //CHECK-NEXT: */
