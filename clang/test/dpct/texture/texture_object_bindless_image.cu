...
int main() {
  ...
  // CHECK: dpct::mem_cpy_param_wrapper p3d;
  ...
  cudaMemcpyKind k;
  // CHECK: p3d.set_from_image_mem(pArr_src);
  p3d.srcArray = pArr_src;
  // CHECK: pArr_src = p3d.get_from_image_mem();
  pArr_src = p3d.srcArray;
  // CHECK: p3d.set_from_pos(pos);
  p3d.srcPos = pos;
  // CHECK: pos = p3d.get_from_pos();
  pos = p3d.srcPos;
  // CHECK: p3d.set_from_data(pp);
  p3d.srcPtr = pp;
  // CHECK: pp = p3d.get_from_data();
  pp = p3d.srcPtr;
  // CHECK: p3d.set_to_image_mem(pArr);
  p3d.dstArray = pArr;
  // CHECK: pArr = p3d.get_to_image_mem();
  pArr = p3d.dstArray;
  // CHECK: p3d.set_to_pos(pos);
  p3d.dstPos = pos;
  // CHECK: pos = p3d.get_to_pos();
  pos = p3d.dstPos;
  // CHECK: p3d.set_to_data(pp);
  p3d.dstPtr = pp;
  // CHECK: pp = p3d.get_to_data();
  pp = p3d.dstPtr;
  // CHECK: p3d.set_size(e);
  p3d.extent = e;
  // CHECK: e = p3d.get_size();
  e = p3d.extent;
  // CHECK: p3d.set_direction(k);
  p3d.kind = k;
  // CHECK: k = p3d.get_direction();
  k = p3d.kind;
