...
int main(){
  ...
  //CHECK:if(s1 = CHECK_SYCL_ERROR(rng->generate_uniform_bits(d_data_ull, 100*100))){}
...
#undef N
  ...
  //CHECK:s1 = CHECK_SYCL_ERROR(rng2->skip_ahead(200));
...
void bar3(){
//CHECK:dpct::rng::host_rng_ptr rng;
//CHECK-NEXT:curandErrCheck(CHECK_SYCL_ERROR(rng = dpct::rng::create_host_rng(dpct::rng::random_engine_type::philox4x32x10)));
...
//CHECK-NEXT:  rng2 = dpct::rng::create_host_rng(dpct::rng::random_engine_type::sobol);
//CHECK-NEXT:  rng2->set_dimensions(1111);
//CHECK-NEXT:  return CHECK_SYCL_ERROR(rng2->generate_uniform(d_data, 100*100));
//CHECK-NEXT:}
int bar5(){
  float *d_data;
  curandGenerator_t rng2;
