...
__device__ void foo_3() {
  ...
  int64_t m = 0;
  // CHECK: m = dpct::dpct_min(m, int64_t((*pd3)[2]));
  // CHECK-NEXT: m = dpct::dpct_min(m, int64_t((*pd3)[1]));
  // CHECK-NEXT: m = dpct::dpct_min(m, int64_t((*pd3)[0]));
  // CHECK-NEXT: m = dpct::dpct_min(m, int64_t(d3[2]));
  // CHECK-NEXT: m = dpct::dpct_min(m, int64_t(d3[1]));
  // CHECK-NEXT: m = dpct::dpct_min(m, int64_t(d3[0]));
