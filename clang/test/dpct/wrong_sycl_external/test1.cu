// RUN: c2s --format-range=none --usm-level=none -in-root %S -out-root %T/wrong_sycl_external %S/test1.cu %S/test2.cu -extra-arg="-I %S" --cuda-include-path="%cuda-path/include" --sycl-named-lambda -- -std=c++14 -x cuda --cuda-host-only
