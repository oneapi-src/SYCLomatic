...
// UNSUPPORTED: v8.0
// RUN: dpct --enable-profiling=0  --format-range=none -out-root %T/thrust-reduce %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only -fno-delayed-template-parsing -std=c++17 -fsized-deallocation -ferror-limit=50
