...
void thrust_test() {
  ...
  // CHECK:  result = std::reduce(oneapi::dpl::execution::par_unseq, data, data + 6);
  // CHECK-NEXT:  result = std::reduce(oneapi::dpl::execution::par_unseq, data, data + 6);
  // CHECK-NEXT:  result = std::reduce(oneapi::dpl::execution::par_unseq, data, data + 6, 1);
  // CHECK-NEXT:  result = std::reduce(oneapi::dpl::execution::par_unseq, data, data + 6, 1);
  // CHECK-NEXT:  result = std::reduce(oneapi::dpl::execution::par_unseq, data, data + 6, -1, oneapi::dpl::maximum<int>());
  // CHECK-NEXT:  result = std::reduce(oneapi::dpl::execution::par_unseq, data, data + 6, -1, oneapi::dpl::maximum<int>());
  ...
  // CHECK-NEXT:  result = std::reduce(oneapi::dpl::execution::make_device_policy(q_ct1), d_data.begin(), d_data.begin() + 6, -1, oneapi::dpl::maximum<int>());
  // CHECK-NEXT:  result = std::reduce(oneapi::dpl::execution::par_unseq, h_data.begin(), h_data.begin() + 6);
  // CHECK-NEXT:  result = std::reduce(oneapi::dpl::execution::par_unseq, h_data.begin(), h_data.begin() + 6);
  // CHECK-NEXT:  result = std::reduce(oneapi::dpl::execution::par_unseq, h_data.begin(), h_data.begin() + 6, 1);
  // CHECK-NEXT:  result = std::reduce(oneapi::dpl::execution::par_unseq, h_data.begin(), h_data.begin() + 6, 1);
  // CHECK-NEXT:  result = std::reduce(oneapi::dpl::execution::par_unseq, h_data.begin(), h_data.begin() + 6, -1, oneapi::dpl::maximum<int>());
  // CHECK-NEXT:  result = std::reduce(oneapi::dpl::execution::par_unseq, h_data.begin(), h_data.begin() + 6, -1, oneapi::dpl::maximum<int>());
