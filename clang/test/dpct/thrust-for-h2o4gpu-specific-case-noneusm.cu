...
// RUN: dpct --enable-profiling=0  --usm-level=none --format-range=none -out-root %T/thrust-for-h2o4gpu-specific-case-noneusm %s --cuda-include-path="%cuda-path/include" -- -std=c++14 -x cuda --cuda-host-only
