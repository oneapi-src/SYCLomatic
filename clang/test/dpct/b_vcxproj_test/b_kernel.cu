...
// RUN: cat %s > %T/b_kernel.cu
// RUN: cat %S/header.cuh > %T/header.cuh
// RUN: cd %T
// RUN: dpct -output-file=b_kernel_outputfile_win.txt --format-range=none  --vcxprojfile=%T/SVMbenchmark.vcxproj  -in-root=%T -out-root=%T/out  b_kernel.cu -extra-arg="-I %S" --cuda-include-path="%cuda-path/include"
