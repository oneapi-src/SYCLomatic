...
int main(int argc, char **argv) {
  ...
  // CHECK: dpct::get_default_queue().memset((void*)(d_array), 0, sizeof(float) * size).wait();
  ...
  // CHECK-NEXT:*/
  // CHECK-NEXT:   (dpct::get_default_queue().memcpy(const_angle.get_ptr(), (void*)(&h_array[0]), sizeof(float) * 360).wait(), 0);
  cudaMemcpyToSymbol(&const_angle[0], &h_array[0], sizeof(float) * 360);

  // CHECK:/*
  // CHECK-NEXT:DPCT1003:{{[0-9]+}}: Migrated api does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT:*/
  // CHECK-NEXT:   (dpct::get_default_queue().memcpy(const_angle.get_ptr() + sizeof(float) * (3), (void*)(&h_array[0]), sizeof(float) * 357).wait(), 0);
  cudaMemcpyToSymbol(&const_angle[3], &h_array[0], sizeof(float) * 357);

  // CHECK:  /*
  // CHECK-NEXT:  DPCT1003:{{[0-9]+}}: Migrated api does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT:  */
  // CHECK-NEXT:  (dpct::get_default_queue().memcpy((void*)(&h_array[0]), const_angle.get_ptr() + sizeof(float) * (3), sizeof(float) * 357).wait(), 0);
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated api does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: (dpct::get_default_queue().memcpy(const_angle.get_ptr() + sizeof(float) * (3+NUM), (void*)(&h_array[0]), sizeof(float) * 354).wait(), 0);
  cudaMemcpyToSymbol(&const_angle[3+NUM], &h_array[0], sizeof(float) * 354);

  // CHECK:  /*
  // CHECK-NEXT:  DPCT1003:{{[0-9]+}}: Migrated api does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT:  */
  // CHECK-NEXT:  (dpct::get_default_queue().memcpy((void*)(&h_array[0]), const_angle.get_ptr() + sizeof(float) * (3+NUM), sizeof(float) * 354).wait(), 0);
  ...
  float hangle_h[360];
  // CHECK:  dpct::get_default_queue().memcpy((void*)(hangle_h), (void*)(d_array), 360 * sizeof(float)).wait();
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated api does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT:  (dpct::get_default_queue().memcpy(const_one.get_ptr(), (void*)(&h_array[0]), sizeof(float) * 1).wait(), 0);
