...
// CHECK-NEXT:        kernel(atm_sync_ct1);
// CHECK-NEXT:      }).wait();
// CHECK-NEXT:  }
  kernel<<<2, 2>>>();
...
__device__ void foo5() {
  // CHECK:  auto thread = dpct::experimental::logical_group(sycl::ext::oneapi::this_work_item::get_nd_item<3>(), sycl::ext::oneapi::this_work_item::get_work_group<3>(), 1);
