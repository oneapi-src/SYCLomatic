...
__global__ void picount(int *totals) {
  ...
  //CHECK: rng = dpct::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<1>>(clock64(), {0, static_cast<std::uint64_t>(tid * 8)});
  curandState_t rng;
...
//CHECK: void cuda_kernel_initRND(unsigned long seed, dpct::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<1>> *States,
...
__global__ void cuda_kernel_initRND(unsigned long seed, curandState *States)
{
  ...
  //CHECK: States[id] = dpct::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<1>>(seed, {0, static_cast<std::uint64_t>(pixel * 8)});
  curand_init(seed, pixel, 0, &States[id]);
}

//CHECK: void cuda_kernel_RNDnormalDitribution(double *Image, dpct::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<1>> *States,
//CHECK:                                  const sycl::nd_item<3> &item_ct1)
...
int main(int argc, char **argv) {
  ...
  int size = 10;
  double *Image;
  //CHECK: dpct::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<1>> *RandomStates;
  curandState *RandomStates;
  void *dev;
  //CHECK: dev = (void *)sycl::malloc_device(size * sizeof(dpct::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<1>>), q_ct1);
  cudaMalloc((void**)&dev, size * sizeof(curandState));
  //CHECK: RandomStates = (dpct::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<1>>*)dev;
  RandomStates = (curandState*)dev;
  //CHECK: RandomStates = (dpct::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<1>> *)sycl::malloc_device(size * sizeof(dpct::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<1>>) * 10, q_ct1);
  cudaMalloc((void**)&RandomStates, size * sizeof(curandState) * 10);
  //CHECK: RandomStates = sycl::malloc_device<dpct::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<1>>>(size, q_ct1);
  ...
  //CHECK: CHECK((RandomStates = (dpct::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<1>> *)sycl::malloc_device(sizeof(dpct::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<1>>) * 10 * 10, q_ct1), 0));
