...
__global__ void picount(int *totals) {
  ...
  //CHECK: oneapi::mkl::rng::device::philox4x32x10<1> rng;
  //CHECK: rng = oneapi::mkl::rng::device::philox4x32x10<1>(clock64(), {0, static_cast<std::uint64_t>(tid * 8)});
  curandState_t rng;
  ...
  for (int i = 0; i < ITERATIONS; i++) {
    ...
    //CHECK-NEXT: float y = oneapi::mkl::rng::device::generate(distr_ct{{[0-9]+}}, rng);
...
//CHECK: void cuda_kernel_initRND(unsigned long seed, oneapi::mkl::rng::device::philox4x32x10<1> *States,
...
__global__ void cuda_kernel_initRND(unsigned long seed, curandState *States)
{
  ...
  //CHECK: States[id] = oneapi::mkl::rng::device::philox4x32x10<1>(seed, {0, static_cast<std::uint64_t>(pixel * 8)});
  curand_init(seed, pixel, 0, &States[id]);
}

//CHECK: void cuda_kernel_RNDnormalDitribution(double *Image, oneapi::mkl::rng::device::philox4x32x10<1> *States,
//CHECK-NEXT:                                  sycl::nd_item<3> item_ct1)
...
int main(int argc, char **argv) {
  ...
  int size = 10;
  double *Image;
  //CHECK: oneapi::mkl::rng::device::philox4x32x10<1> *RandomStates;
  curandState *RandomStates;
  void *dev;
  //CHECK: dev = (void *)sycl::malloc_device(size * sizeof(oneapi::mkl::rng::device::philox4x32x10<1>), q_ct1);
