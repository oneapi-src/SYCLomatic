// RUN: c2s --format-range=none -extra-arg-before=-std=c++14 -out-root %T/curand-device-usm %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
...
//CHECK-NEXT: #include <c2s/rng_utils.hpp>
...
__global__ void picount(int *totals) {
  ...
  //CHECK: rng = c2s::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<4>>(clock64(), {0, static_cast<std::uint64_t>(tid * 8)});
  curandState_t rng;
...
//CHECK: void cuda_kernel_initRND(unsigned long seed, c2s::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<4>> *States,
...
__global__ void cuda_kernel_initRND(unsigned long seed, curandState *States)
{
  ...
  //CHECK: States[id] = c2s::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<4>>(seed, {0, static_cast<std::uint64_t>(pixel * 8)});
  curand_init(seed, pixel, 0, &States[id]);
}

//CHECK: void cuda_kernel_RNDnormalDitribution(double *Image, c2s::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<4>> *States,
//CHECK-NEXT:                                  sycl::nd_item<3> item_ct1)
...
int main(int argc, char **argv) {
  ...
  void *dev;
  //CHECK: dev = (void *)sycl::malloc_device(size * sizeof(c2s::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<4>>), q_ct1);
  cudaMalloc((void**)&dev, size * sizeof(curandState));
  //CHECK: RandomStates = (c2s::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<4>>*)dev;
  ...
  //CHECK: CHECK((RandomStates = (c2s::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<4>> *)sycl::malloc_device(sizeof(c2s::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<4>>) * 10 * 10, q_ct1), 0));
...
__global__ void test() {
  ...
  //CHECK:state = c2s::rng::device::rng_generator<oneapi::mkl::rng::device::philox4x32x10<4>>(std::get<0>(seeds), {static_cast<std::uint64_t>(std::get<1>(seeds)), static_cast<std::uint64_t>(idx * 4)});
  curand_init(std::get<0>(seeds), idx, std::get<1>(seeds), &state);
}

// Test description:
// This test covers the case when the type of the arg has alias.
// CHECK: using state_type = c2s::rng::device::rng_generator<oneapi::mkl::rng::device::mrg32k3a<4>>;
...
__device__ void foo() {
  ...
  // CHECK: state.state = c2s::rng::device::rng_generator<oneapi::mkl::rng::device::mrg32k3a<4>>(seed, {static_cast<std::uint64_t>(offset), static_cast<std::uint64_t>(sequence * 8)});
