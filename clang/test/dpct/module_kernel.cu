...
//CHECK: extern "C" void foo(float* k, float* y, uint8_t *dpct_local,
//CHECK-NEXT:                unsigned int const *const_data);
...
//CHECK: void foo(float* k, float* y, uint8_t *dpct_local,
//CHECK-NEXT:     unsigned int const *const_data){
...
// CHECK-NEXT:             foo(k, y, dpct_local_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get(), const_data_ptr_ct1);
// CHECK-NEXT:           });
// CHECK-NEXT:       });
// CHECK-NEXT:   }
// CHECK-NEXT: }
...
// CHECK: extern "C" void foo2(float* k, float* y, sycl::int2 x=sycl::int2(1, 2)) {
// CHECK-NEXT: (void)sycl::ext::oneapi::this_work_item::get_nd_item<3>().get_local_id(2);
// CHECK-NEXT: }
...
//CHECK-NEXT:              [=](sycl::nd_item<3> item_ct1) {
//CHECK-NEXT:                foo(a, b, dpct_local_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get(), const_data_ptr_ct1);
//CHECK-NEXT:              });
//CHECK-NEXT:          });
//CHECK-NEXT:     }
//CHECK-NEXT: }
