...
int main(int argc, char *argv[])
{
    ...
    //CHECK-NEXT: devIpiv_buf_ct{{[0-9]+}}.get_host_access(sycl::write)[0] = (int)result_temp_buffer6.get_host_access(sycl::read)[0];
    //CHECK-NEXT: }
    //CHECK-NEXT: {
    //CHECK-NEXT: auto A_f_buf_ct{{[0-9]+}} = dpct::get_buffer<float>(&A_f);
    //CHECK-NEXT: auto workspace_f_buf_ct{{[0-9]+}} = dpct::get_buffer<float>(&workspace_f);
    ...
    //CHECK-NEXT: devIpiv_buf_ct{{[0-9]+}}.get_host_access(sycl::write)[0] = (int)result_temp_buffer5.get_host_access(sycl::read)[0];
    //CHECK-NEXT: }
    //CHECK-NEXT: {
    //CHECK-NEXT: auto A_f_buf_ct{{[0-9]+}} = dpct::get_buffer<float>(&A_f);
    //CHECK-NEXT: auto devIpiv_buf_ct{{[0-9]+}} = dpct::get_buffer<int>(&devIpiv);
