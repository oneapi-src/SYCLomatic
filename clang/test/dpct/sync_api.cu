...
// UNSUPPORTED: v8.0
// RUN: dpct --no-dpcpp-extensions=free-function-queries --format-range=none -out-root %T/sync_api %s --cuda-include-path="%cuda-path/include" --use-experimental-features=root-group,logical-group -- -x cuda --cuda-host-only -std=c++14
