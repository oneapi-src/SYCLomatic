...
__global__ void k() {
  ...
  // CHECK: int c = (sycl::group_barrier(item_ct1.get_group()), sycl::reduce_over_group(item_ct1.get_group(), p == 0 ? 0 : 1, sycl::ext::oneapi::plus<>()));
  ...
  // CHECK: foo((sycl::group_barrier(item_ct1.get_group()), sycl::reduce_over_group(item_ct1.get_group(), p == 0 ? 0 : 1, sycl::ext::oneapi::plus<>())));
