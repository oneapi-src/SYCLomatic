...
// RUN: dpct --format-range=none -in-root %S -out-root %T/devicelevel/device_scan_flagged %S/device_scan_flagged.cu --cuda-include-path="%cuda-path/include" -- -std=c++14 -x cuda --cuda-host-only
// RUN: FileCheck --input-file %T/devicelevel/device_scan_flagged/device_scan_flagged.dp.cpp --match-full-lines %s
