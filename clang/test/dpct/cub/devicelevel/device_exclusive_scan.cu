...
// RUN: dpct --format-range=none -in-root %S -out-root %T/devicelevel/device_exclusive_scan %S/device_exclusive_scan.cu --cuda-include-path="%cuda-path/include" -- -std=c++14 -x cuda --cuda-host-only
// RUN: FileCheck --input-file %T/devicelevel/device_exclusive_scan/device_exclusive_scan.dp.cpp --match-full-lines %s
// RUN: %if build_lit %{icpx -c -fsycl %T/devicelevel/device_exclusive_scan/device_exclusive_scan.dp.cpp -o %T/devicelevel/device_exclusive_scan/device_exclusive_scan.dp.o %}
...
void test_1() {
  ...
  cub::DeviceScan::ExclusiveScan(device_tmp, n_device_tmp, device_in, device_out, op, 0, N);
  cudaMalloc((void **)&device_tmp, n_device_tmp);
  cub::DeviceScan::ExclusiveScan((void *)device_tmp, n_device_tmp, device_in, device_out, op, 0, N);
...
void test_2() {
 int *device_in = nullptr;
  ...
  cudaMalloc((void **)&device_out, N * sizeof(int));
  cudaMemcpy(device_in, (void *)host_in, sizeof(host_in), cudaMemcpyHostToDevice);
  cub::DeviceScan::ExclusiveScan(device_tmp, n_device_tmp, device_in, device_out, op, 0, N), 0;
  cudaMalloc((void **)&device_tmp, n_device_tmp);
  cub::DeviceScan::ExclusiveScan((void *)device_tmp, n_device_tmp, device_in, device_out, op, 0, N);
  cudaMemcpy((void *)host_out, (void *)device_out, sizeof(host_out), cudaMemcpyDeviceToHost);
  cudaFree(device_in);
