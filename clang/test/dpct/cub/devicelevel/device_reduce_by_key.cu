...
// CHECK-NEXT:  DPCT1026{{.*}}
// CHECK-NEXT:  stream->fill(d_num, std::distance(d_unq, oneapi::dpl::reduce_by_key(oneapi::dpl::execution::device_policy(*stream), d_key, d_key + n, d_val, d_unq, d_agg, std::equal_to<typename std::iterator_traits<decltype(d_key)>::value_type>(), op).first), 1).wait();
// CHECK-NEXT:  q_ct1.memcpy(&num, d_num, sizeof(int)){{.*}};
// CHECK-NEXT:  q_ct1.memcpy(unq, d_unq, sizeof(int) * num){{.*}};
// CHECK-NEXT:  q_ct1.memcpy(agg, d_agg, sizeof(int) * num){{.*}};
// CHECK-NEXT:}
