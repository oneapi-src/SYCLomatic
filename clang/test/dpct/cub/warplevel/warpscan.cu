...
// RUN: dpct --no-dpcpp-extensions=free-function-queries -in-root %S -out-root %T/warplevel/warpscan %S/warpscan.cu --cuda-include-path="%cuda-path/include" -- -std=c++14 -x cuda --cuda-host-only
