...
int main() {
  ...
  // CHECK-NEXT:         BlockedKernel(d_data, 5, stream_ct1, &temp_storage_acc[0]);
  // CHECK-NEXT:       });
  // CHECK-NEXT:   });
  ...
  // CHECK-NEXT:       sycl::nd_range<3>(sycl::range<3>(1, 1, 128), sycl::range<3>(1, 1, 128)),
  // CHECK-NEXT:       [=](sycl::nd_item<3> item_ct1) {
  // CHECK-NEXT:         StripedKernel(d_data, 128, stream_ct1, &temp_storage_acc[0]);
  // CHECK-NEXT:       });
  // CHECK-NEXT:   });
...
// CHECK:          TransposeKernel(d_data, 128, stream_ct1, &temp_storage_acc[0]);
// CHECK:        });
// CHECK:    });
...
// CHECK:        sycl::nd_range<3>(sycl::range<3>(1, 1, 128), sycl::range<3>(1, 1, 128)),
// CHECK:        [=](sycl::nd_item<3> item_ct1) {
// CHECK:          SubGroupTransposeKernel(d_data, 128, stream_ct1, &temp_storage_acc[0]);
// CHECK:        });
// CHECK:    });
