...
__global__ void foo2() {
  ...
  __shared__ typename BlockReduce::TempStorage m;
  // CHECK: v = sycl::reduce_over_group(sycl::ext::oneapi::this_work_item::get_work_group<3>(), (sycl::ext::oneapi::this_work_item::get_work_group<3>().get_local_linear_id() < sycl::ext::oneapi::this_work_item::get_nd_item<3>().get_local_range(2)) ? v : sycl::known_identity_v<sycl::plus<>, sycl::half>, sycl::plus<>());
  v = BlockReduce(m).Reduce(v, cub::Sum{}, blockDim.x);
}
