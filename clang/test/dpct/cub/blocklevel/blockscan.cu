...
//CHECK-NEXT:  output = sycl::exclusive_scan_over_group(item_ct1.get_group(), input, 0, sycl::ONEAPI::plus<>());
...
//CHECK-NEXT:  int input = data[threadid];
//CHECK-NEXT:  int output = 0;
//CHECK-NEXT:  output = sycl::inclusive_scan_over_group(item_ct1.get_group(), input, sycl::ONEAPI::plus<>());
//CHECK-NEXT:  data[threadid] = output;
//CHECK-NEXT:}
