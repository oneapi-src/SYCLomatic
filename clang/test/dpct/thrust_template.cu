...
void foo_host(){
  ...
  //CHECK-NEXT: oneapi::dpl::adjacent_difference(oneapi::dpl::execution::par_unseq, B.begin(), B.end(), R2.begin(), std::minus<T>());
  ...
  //CHECK-NEXT: oneapi::dpl::inclusive_scan(oneapi::dpl::execution::par_unseq, B.begin(), B.end(), R2.begin(), std::minus<T>());
...
template <typename ELT_TYPE> void testfunc() {
  ...
  // CHECK: dpct::remove_copy_if(oneapi::dpl::execution::par_unseq, V2.begin(), V2.end(), V1.begin(), V6.begin(), pred_A);
  thrust::remove_copy_if(V2.begin(), V2.end(), V1.begin(), V6.begin(), pred_A);

  // CHECK: oneapi::dpl::unique_copy(oneapi::dpl::execution::par_unseq, V2.begin(), V2.end(), V2.begin());
  thrust::unique_copy(V2.begin(), V2.end(), V2.begin());

  // CHECK:oneapi::dpl::stable_sort(oneapi::dpl::execution::par_unseq, V1.begin(), V1.end(), std::not2(std::greater_equal<int>()));
