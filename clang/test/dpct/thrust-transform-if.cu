...
// UNSUPPORTED: v8.0
// RUN: dpct --format-range=none -out-root %T/thrust-transform-if %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only -std=c++17 -fsized-deallocation -ferror-limit=50
...
int main() {
  ...
  // Policy
// CHECK:  dpct::transform_if(oneapi::dpl::execution::seq, inDataH, inDataH + dataLen, outDataH, neg, is_odd());
  thrust::transform_if(thrust::host, inDataH, inDataH + dataLen, outDataH, neg, is_odd());

  // Policy and stencil
// CHECK:  dpct::transform_if(oneapi::dpl::execution::seq, inDataH, inDataH + dataLen, stencilH, outDataH, neg, identity());
...
// CHECK:  dpct::transform_if(oneapi::dpl::execution::seq, inDataH, inDataH + dataLen, inDataH, stencilH, outDataH, plus, identity());
  thrust::transform_if(thrust::host, inDataH, inDataH + dataLen, inDataH, stencilH, outDataH, plus, identity());

  // No policy
// CHECK:  dpct::transform_if(oneapi::dpl::execution::make_device_policy(q_ct1), inDataD.begin(), inDataD.end(), outDataD.begin(), neg, is_odd());
  thrust::transform_if(inDataD.begin(), inDataD.end(), outDataD.begin(), neg, is_odd());

  // No policy and stencil
// CHECK:  dpct::transform_if(oneapi::dpl::execution::make_device_policy(q_ct1), inDataD.begin(), inDataD.end(), stencilD.begin(), outDataD.begin(), neg, identity());
...
// CHECK:  dpct::transform_if(oneapi::dpl::execution::make_device_policy(q_ct1), inDataD.begin(), inDataD.end(), inDataD.begin(), stencilD.begin(), outDataD.begin(), plus, identity());
