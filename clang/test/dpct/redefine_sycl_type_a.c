// RUN: c2s --format-range=none --usm-level=none -in-root %S -out-root %T/redefine_sycl_type %s %S/redefine_sycl_type_b.cu -extra-arg="-I %S" --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
