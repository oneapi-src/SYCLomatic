...
__global__ void test(T *data) {
  ...
  // CHECK: dpct::atomic_fetch_add<T, sycl::access::address_space::generic_space>(&data[0], tid);
...
__global__ void test(unsigned long long int* data) {
  ...
  // CHECK: dpct::atomic_fetch_add<unsigned long long, sycl::access::address_space::generic_space>(&data[0], tid);
  atomicAdd(&data[0], tid);
...
__global__ void test(float* data) {
  ...
  // CHECK: dpct::atomic_fetch_add<float, sycl::access::address_space::generic_space>(&data[0], tid);
  atomicAdd(&data[0], tid);
...
__global__ void test(double* data) {
  ...
  // CHECK: dpct::atomic_fetch_add<double, sycl::access::address_space::generic_space>(&data[0], tid);
  atomicAdd(&data[0], tid);
}
...
__device__ void fun(){
  double *a;
  float b;
  // CHECK: dpct::atomic_fetch_add<double, sycl::access::address_space::generic_space>(a, 1);
  atomicAdd(a, 1);

  // CHECK: dpct::atomic_fetch_add<double, sycl::access::address_space::generic_space>(a, b);
  atomicAdd(a, b);

  // CHECK: dpct::atomic_fetch_add<uint32_t, sycl::access::address_space::generic_space>(d_error, 1);
  atomicAdd(d_error, 1);
}
...
// CHECK-NEXT:    dpct::atomic_fetch_add<uint32_t, sycl::access::address_space::generic_space>(&share_array[b], 1);
// CHECK-NEXT:    dpct::atomic_fetch_add<uint32_t, sycl::access::address_space::generic_space>(share_array, 1);
// CHECK-NEXT:  }
// CHECK-EMPTY:
// CHECK-NEXT:  dpct::atomic_fetch_add<uint32_t, sycl::access::address_space::generic_space>(&share_v, 1);
// CHECK-NEXT:}
__global__ void foo() {
...
// CHECK-NEXT:    dpct::atomic_fetch_add<uint32_t, sycl::access::address_space::generic_space>(p_1, 1);
// CHECK-NEXT:    dpct::atomic_fetch_add<uint32_t, sycl::access::address_space::generic_space>(p_2, 1);
// CHECK-NEXT:    dpct::atomic_fetch_add<uint32_t, sycl::access::address_space::generic_space>(p_3, 1);
// CHECK-NEXT:  }
// CHECK-EMPTY:
...
// CHECK-NEXT:  dpct::atomic_fetch_add<uint32_t, sycl::access::address_space::generic_space>(p_4, 1);
// CHECK-NEXT:}
...
// CHECK-NEXT:  dpct::atomic_fetch_add<uint32_t, sycl::access::address_space::generic_space>(p_5, 1);
// CHECK-NEXT:}
...
// CHECK:  dpct::atomic_fetch_add<int, sycl::access::address_space::generic_space>(&i, i);
...
// CHECK:  dpct::atomic_fetch_add<unsigned int, sycl::access::address_space::generic_space>(&ui, ui);
...
// CHECK:  dpct::atomic_fetch_add<unsigned long long, sycl::access::address_space::generic_space>(&ull, ull);
...
// CHECK-NEXT:  dpct::atomic_fetch_max<unsigned long long, sycl::access::address_space::generic_space>(&ull, ull);
  ...
  // CHECK: dpct::atomic_fetch_add<unsigned int, sycl::access::address_space::generic_space>(&ui, i);
  atomicAdd(&ui, i);

  // CHECK: dpct::atomic_fetch_add<unsigned int, sycl::access::address_space::generic_space>(&ui, i + i);
  atomicAdd(&ui, i + i);

  // CHECK: dpct::atomic_fetch_add<uint32_t, sycl::access::address_space::generic_space>(&u32, u32);
  atomicAdd(&u32, u32);

  // CHECK: dpct::atomic_fetch_add<float, sycl::access::address_space::generic_space>(&f, f);
  atomicAdd(&f, f);
}
...
// CHECK-NEXT:    dpct::atomic_fetch_add<unsigned int, sycl::access::address_space::generic_space>(&temp[buffer[i]], 1);
// CHECK-NEXT:    i += offset;
// CHECK-NEXT:  }
// CHECK-NEXT:  item_ct1.barrier(sycl::access::fence_space::local_space);
...
// CHECK-NEXT:  dpct::atomic_fetch_add<unsigned int, sycl::access::address_space::generic_space>(s_WarpHist + data, 1);
// CHECK-NEXT:}
...
//CHECK-NEXT:  dpct::atomic_fetch_add<int, sycl::access::address_space::generic_space>((int *)&g_mutex, 1);
