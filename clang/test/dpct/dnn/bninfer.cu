...
int main() {
    ...
    // CHECK: auto status = (handle.async_batch_normalization_forward_inference(dpct::dnnl::batch_normalization_mode::per_activation, eps, alpha, dataTensor, data, beta, outTensor, out, scalebiasTensor, scale, bias, smean, svar), 0);
