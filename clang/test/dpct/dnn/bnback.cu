...
// CHECK: auto status = (handle.batch_normalization_forward_training(m, eps, factor, alpha, dataTensor, data, beta, outTensor, out, scalebiasTensor, scale, bias, rmean, rvar, smean, svar), 0);
    auto status = cudnnBatchNormalizationForwardTraining(
        ...
        smean,
        svar);
// CHECK: status = (handle.batch_normalization_backward(dpct::dnnl::batch_normalization_mode::per_activation, eps, alpha, dataTensor, data, outTensor, diffout, beta, dataTensor, diffdata, alpha, scalebiasTensor, scale, beta, diffscale, diffbias, smean, svar), 0);
