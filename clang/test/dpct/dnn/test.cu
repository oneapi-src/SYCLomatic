...
// RUN: dpct -in-root %S -out-root %T/test %S/test.cu --cuda-include-path="%cuda-path/include" -- -std=c++14 -x cuda --cuda-host-only
// RUN: FileCheck --input-file %T/test/test.dp.cpp --match-full-lines %s
