// UNSUPPORTED: -linux-
// RUN: c2s --format-range=none -out-root %T/module_kernel_win %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only -ptx
...
//CHECK: void foo(float* k, float* y, sycl::nd_item<3> item_ct1, uint8_t *c2s_local);
...
//CHECK: void foo(float* k, float* y, sycl::nd_item<3> item_ct1, uint8_t *c2s_local){
//CHECK-NEXT:     auto s = (int *)c2s_local;
...
//CHECK-NEXT:     sycl::accessor<uint8_t, 1, sycl::access_mode::read_write, sycl::access::target::local> c2s_local_acc_ct1(sycl::range<1>(localMemSize), cgh);
//CHECK:     cgh.parallel_for(
//CHECK-NEXT: nr,
//CHECK-NEXT:       [=](sycl::nd_item<3> item_ct1) {
//CHECK-NEXT:         foo(k, y, item_ct1, c2s_local_acc_ct1.get_pointer());
//CHECK-NEXT:       });
//CHECK-NEXT:   });
//CHECK-NEXT: }
//CHECK-NEXT: }
...
//CHECK-NEXT:       [&](sycl::handler &cgh) {
//CHECK-NEXT:         sycl::accessor<uint8_t, 1, sycl::access_mode::read_write, sycl::access::target::local> c2s_local_acc_ct1(sycl::range<1>(0), cgh);

//CHECK:         cgh.parallel_for(
//CHECK-NEXT:           sycl::nd_range<3>(sycl::range<3>(1, 1, 2), sycl::range<3>(1, 1, 2)), 
//CHECK-NEXT:           [=](sycl::nd_item<3> item_ct1) {
//CHECK-NEXT:             foo(a, b, item_ct1, c2s_local_acc_ct1.get_pointer());
//CHECK-NEXT:           });
//CHECK-NEXT:       });
//CHECK-NEXT: }
void goo(){
