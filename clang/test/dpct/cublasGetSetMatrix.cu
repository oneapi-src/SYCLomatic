...
#define LDA_MARCO 100
  ...
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (dpct::matrix_mem_copy((void*)d_A, (void*)A, 100, LDA_MARCO, 100, colsA, sizeof(A[0])), 0);
...
#define LDB_MARCO 99
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (dpct::matrix_mem_copy((void*)d_A, (void*)A, 100, 100, 99, colsA, sizeof(A[0])), 0);
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (dpct::matrix_mem_copy((void*)d_A, (void*)A, 100, 100, 100, colsA, sizeof(A[0]), dpct::automatic, *stream, true), 0);
