...
// RUN: %if build_lit %{icpx -c -fsycl %T/cublasGetSetVector/cublasGetSetVector.dp.cpp -o %T/cublasGetSetVector/cublasGetSetVector.dp.o %}
#include <cstdio>
#include <cublas_v2.h>
...
int main() {
  ...
  // CHECK: status = DPCT_CHECK_ERROR(dpct::blas::matrix_mem_copy(h_C, d_C, 1, 1, 1, N, sizeof(h_C[0])));
  status = cublasGetVector(N, sizeof(h_C[0]), d_C, 1, h_C, 1);

#define INCX_MARCO 1
  const int ConstIncy = 1;
  // CHECK: status = DPCT_CHECK_ERROR(dpct::blas::matrix_mem_copy(d_A, h_A, ConstIncy, INCX_MARCO, 1, N, sizeof(h_A[0])));
  status = cublasSetVector(N, sizeof(h_A[0]), h_A, INCX_MARCO, d_A, ConstIncy);

  // CHECK: dpct::blas::matrix_mem_copy(h_C, d_C, 1, 1, 1, N, sizeof(h_C[0]));
  cublasGetVector(N, sizeof(h_C[0]), d_C, 1, h_C, 1);

  // CHECK: dpct::blas::matrix_mem_copy(d_A, h_A, 1, 1, 1, N, sizeof(h_A[0]));
  ...
  // CHECK-NEXT: */
  // CHECK-NEXT: dpct::blas::matrix_mem_copy(h_C, d_C, 1, 2, 1, N, sizeof(h_C[0]));
...
#define INCY_MARCO 2
  ...
  // CHECK-NEXT: */
  // CHECK: status = DPCT_CHECK_ERROR(dpct::blas::matrix_mem_copy(d_A, h_A, INCY_MARCO, 1, 1, N, sizeof(h_A[0])));
  ...
  // CHECK-NEXT: */
  // CHECK: status = DPCT_CHECK_ERROR(dpct::blas::matrix_mem_copy(d_A, h_A, INCY_MARCO, ConstIncx, 1, N, sizeof(h_A[0])));
  ...
  // CHECK-NEXT: */
  // CHECK: status = DPCT_CHECK_ERROR(dpct::blas::matrix_mem_copy(d_A, h_A, incy, incx, 1, N, sizeof(h_A[0])));
  ...
  // CHECK-NEXT: */
  // CHECK: status = DPCT_CHECK_ERROR(dpct::blas::matrix_mem_copy(d_A, h_A, ConstIncyNE, ConstIncxNE, 1, N, sizeof(h_A[0])));
  ...
  // CHECK-NEXT: */
  // CHECK-NEXT: dpct::blas::matrix_mem_copy(d_A, h_A, foo(incy), foo(incx), 1, N, sizeof(h_A[0]));
  cublasSetVector(N, sizeof(h_A[0]), h_A, foo(incx), d_A, foo(incy));

  // CHECK: dpct::blas::matrix_mem_copy(d_A, h_A, foo(ConstIncyT), foo(ConstIncxT), 1, N, sizeof(h_A[0]));
