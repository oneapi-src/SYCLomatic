// RUN: c2s --format-range=none --usm-level=none -extra-arg-before=-std=c++14 -out-root %T/cublasGetSetVector %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
...
int main() {
  ...
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (c2s::matrix_mem_copy((void*)h_C, (void*)d_C, 1, 1, 1, N, sizeof(h_C[0])), 0);
...
#define INCX_MARCO 1
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (c2s::matrix_mem_copy((void*)d_A, (void*)h_A, ConstIncy, INCX_MARCO, 1, N, sizeof(h_A[0])), 0);
  status = cublasSetVector(N, sizeof(h_A[0]), h_A, INCX_MARCO, d_A, ConstIncy);

  // CHECK: c2s::matrix_mem_copy((void*)h_C, (void*)d_C, 1, 1, 1, N, sizeof(h_C[0]));
  cublasGetVector(N, sizeof(h_C[0]), d_C, 1, h_C, 1);

  // CHECK: c2s::matrix_mem_copy((void*)d_A, (void*)h_A, 1, 1, 1, N, sizeof(h_A[0]));
  ...
  // CHECK-NEXT: */
  // CHECK-NEXT: c2s::matrix_mem_copy((void*)h_C, (void*)d_C, 1, 2, 1, N, sizeof(h_C[0]));
...
#define INCY_MARCO 2
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (c2s::matrix_mem_copy((void*)d_A, (void*)h_A, INCY_MARCO, 1, 1, N, sizeof(h_A[0])), 0);
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (c2s::matrix_mem_copy((void*)d_A, (void*)h_A, INCY_MARCO, ConstIncx, 1, N, sizeof(h_A[0])), 0);
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (c2s::matrix_mem_copy((void*)d_A, (void*)h_A, incy, incx, 1, N, sizeof(h_A[0])), 0);
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (c2s::matrix_mem_copy((void*)d_A, (void*)h_A, ConstIncyNE, ConstIncxNE, 1, N, sizeof(h_A[0])), 0);
  ...
  // CHECK-NEXT: */
  // CHECK-NEXT: c2s::matrix_mem_copy((void*)d_A, (void*)h_A, foo(incy), foo(incx), 1, N, sizeof(h_A[0]));
  cublasSetVector(N, sizeof(h_A[0]), h_A, foo(incx), d_A, foo(incy));

  // CHECK: c2s::matrix_mem_copy((void*)d_A, (void*)h_A, foo(ConstIncyT), foo(ConstIncxT), 1, N, sizeof(h_A[0]));
  ...
  // CHECK-NEXT: */
  // CHECK-NEXT: c2s::matrix_mem_copy((void*)d_A, (void*)h_A, ConstExprIncy, foo(ConstExprIncx), 1, N, sizeof(h_A[0]));
  cublasGetVector(N, sizeof(h_A[0]), h_A, foo(ConstExprIncx), d_A, ConstExprIncy);

  // CHECK: /*
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (c2s::matrix_mem_copy((void*)h_C, (void*)d_C, 1, 1, 1, N, sizeof(h_C[0]), c2s::automatic, *stream, true), 0);
  // CHECK-NEXT: c2s::matrix_mem_copy((void*)h_C, (void*)d_C, 1, 1, 1, N, sizeof(h_C[0]), c2s::automatic, *stream, true);
