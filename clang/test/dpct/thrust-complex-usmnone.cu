...
// RUN: dpct --enable-profiling=0  --format-range=none --optimize-migration -out-root %T/thrust-complex-usmnone %s --cuda-include-path="%cuda-path/include" -usm-level=none -- -x cuda --cuda-host-only --std=c++14
// RUN: FileCheck --input-file %T/thrust-complex-usmnone/thrust-complex-usmnone.dp.cpp --match-full-lines %s
// RUN: %if build_lit %{icpx -c -fsycl %T/thrust-complex-usmnone/thrust-complex-usmnone.dp.cpp -o %T/thrust-complex-usmnone/thrust-complex-usmnone.dp.o %}
...
//CHECK-NEXT:   double sum = std::reduce(oneapi::dpl::execution::make_device_policy(dpct::get_out_of_order_queue()), dp, dp + 10);
