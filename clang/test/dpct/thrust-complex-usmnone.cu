...
// RUN: dpct --format-range=none --optimize-migration=remove-not-referenced-kernel-arg -out-root %T/thrust-complex-usmnone %s --cuda-include-path="%cuda-path/include" -usm-level=none -- -x cuda --cuda-host-only --std=c++14
