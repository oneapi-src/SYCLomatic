...
// CHECK: void process(dpct::queue_ptr st, char *data, dpct::err_tmp0 status) {}
void process(cudaStream_t st, char *data, cudaError_t status) {}

template<typename T>
// CHECK: void callback(dpct::queue_ptr st, dpct::err_tmp0 status, void *vp) {
...
static void func()
{
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: dpct::err_tmp0 status = (std::async([&]() { s0->wait(); callback<char *>(s0, 0, str); }), 0);
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: MY_ERROR_CHECKER((s0->ext_oneapi_submit_barrier({*e}), 0));
...
// CHECK: void gFunc0(dpct::err_tmp0 e, dpct::queue_ptr s) {}
void gFunc0(cudaError_t e, cudaStream_t s) {}
// CHECK: void gFunc1(dpct::queue_ptr s, dpct::err_tmp0 e) {}
void gFunc1(cudaStream_t s ,cudaError_t e) {}

// CHECK: void gFunc2(dpct::err_tmp0 e, dpct::queue_ptr const s) {}
void gFunc2(cudaError_t e, cudaStream_t const s) {}
// CHECK: void gFunc3(dpct::queue_ptr const s, dpct::err_tmp0 e) {}
void gFunc3(cudaStream_t const s, cudaError_t e) {}

// CHECK: void gFunc4(dpct::err_tmp0 e, const dpct::queue_ptr s) {}
void gFunc4(cudaError_t e, const cudaStream_t s) {}
// CHECK: void gFunc5(const dpct::queue_ptr s, dpct::err_tmp0 e) {}
void gFunc5(const cudaStream_t s ,cudaError_t e) {}

void bar() {
   // CHECK: std::function<void(dpct::err_tmp0, dpct::queue_ptr)> f0 = gFunc0;
   std::function<void(cudaError_t, cudaStream_t)> f0 = gFunc0;
   // CHECK: std::function<void(dpct::queue_ptr, dpct::err_tmp0)> f1 = gFunc1;
   std::function<void(cudaStream_t, cudaError_t)> f1 = gFunc1;

   // CHECK: std::function<void(dpct::err_tmp0, dpct::queue_ptr const)> f2 = gFunc2;
   std::function<void(cudaError_t, cudaStream_t const)> f2 = gFunc2;
   // CHECK: std::function<void(dpct::queue_ptr const, dpct::err_tmp0)> f3 = gFunc3;
   std::function<void(cudaStream_t const, cudaError_t)> f3 = gFunc3;

   // CHECK: std::function<void(dpct::err_tmp0, const dpct::queue_ptr)> f4 = gFunc4;
   std::function<void(cudaError_t, const cudaStream_t)> f4 = gFunc4;
   // CHECK: std::function<void(const dpct::queue_ptr, dpct::err_tmp0)> f5 = gFunc5;
