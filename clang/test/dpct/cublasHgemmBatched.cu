...
// RUN: c2s --format-range=none -out-root %T/cublasHgemmBatched %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
...
int main() {
  ...
  // CHECK-NEXT: sycl::half alpha_ct{{[0-9]+}} = alpha_H, beta_ct{{[0-9]+}} = beta_H;
  // CHECK-NEXT: oneapi::mkl::blas::column_major::gemm_batch(*handle, &trans3, &trans3, &m_ct{{[0-9]+}}, &n_ct{{[0-9]+}}, &k_ct{{[0-9]+}}, &alpha_ct{{[0-9]+}}, d_A_H, &lda_ct{{[0-9]+}}, d_B_H, &ldb_ct{{[0-9]+}}, &beta_ct{{[0-9]+}}, d_C_H, &ldc_ct{{[0-9]+}}, 1, &group_size_ct{{[0-9]+}}, {});
