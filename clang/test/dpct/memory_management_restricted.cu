...
#define SIZEOF(x) sizeof(x)
  ...
  // CHECK:  /*
  // CHECK-NEXT:DPCT1106:{{[0-9]+}}: 'cudaMemGetInfo' was migrated with the Intel extensions for device information which may be not supported by all compilers or runtimes. You may need to adjust the code.
  // CHECK-NEXT:*/
  ...
  // CHECK:  /*
  // CHECK-NEXT:  DPCT1106:{{[0-9]+}}: 'cuMemGetInfo' was migrated with the Intel extensions for device information which may be not supported by all compilers or runtimes. You may need to adjust the code.
  // CHECK-NEXT:  */
  // CHECK-NEXT:  dpct::get_current_device().get_memory_info(free_mem, total_mem);
