...
void foo() {
  ...
  // CHECK: errorCode = CHECK_SYCL_ERROR(d_A = (float *)sycl::malloc_device(size, q_ct1));
  errorCode = cudaMalloc((void **)&d_A, size);
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(d_A = (float *)sycl::malloc_device(size, q_ct1)));
  ...
  // CHECK: errorCode = CHECK_SYCL_ERROR(h_A = (float *)sycl::malloc_host(size, q_ct1));
  errorCode = cudaHostAlloc((void **)&h_A, size, cudaHostAllocDefault);
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(h_A = (float *)sycl::malloc_host(size, q_ct1)));
...
#undef SIZE_1
  ...
  // CHECK: errorCode = CHECK_SYCL_ERROR(d_A = (float *)sycl::malloc_shared(size, q_ct1));
  errorCode = cudaMallocManaged((void **)&d_A, size);
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(d_A = (float *)sycl::malloc_shared(size, q_ct1)));
  ...
  // CHECK: errorCode  = CHECK_SYCL_ERROR(q_ct1.memcpy(d_A, h_A, size));
  errorCode  = cudaMemcpy(d_A, h_A, size, cudaMemcpyHostToDevice);
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(q_ct1.memcpy(d_A, h_A, size)));
...
#define MY_SAFE_CALL2(x) MY_SAFE_CALL(x)
  //CHECK: MY_SAFE_CALL2(CHECK_SYCL_ERROR(q_ct1.memcpy(d_A, h_A, size)));
  MY_SAFE_CALL2(cudaMemcpy(d_A, h_A, size, cudaMemcpyHostToDevice));
  //CHECK: MY_SAFE_CALL2(CHECK_SYCL_ERROR(q_ct1.memcpy(d_A, h_A, MACRO_B)));
  MY_SAFE_CALL2(cudaMemcpy(d_A, h_A, MACRO_B, cudaMemcpyDeviceToHost));
  //CHECK: MY_SAFE_CALL2(CHECK_SYCL_ERROR(q_ct1.memcpy(d_A, h_A, MACRO_A2(1))));
  MY_SAFE_CALL2(cudaMemcpy(d_A, h_A, MACRO_A2(1), MACOR_C(1)));
  //CHECK: MY_SAFE_CALL2(CHECK_SYCL_ERROR(q_ct1.memcpy(d_A, h_A, foo_b(1)).wait()));
...
#define SIZE 100
  ...
  // CHECK: errorCode = CHECK_SYCL_ERROR(stream->memcpy(d_A, h_A, size));
  errorCode = cudaMemcpyAsync(d_A, h_A, size, cudaMemcpyHostToDevice, stream);
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(stream->memcpy(d_A, h_A, size)));
  ...
  // CHECK: q_ct1.memcpy(h_A, (char *)(constData.get_ptr()) + 1, size).wait();
  cudaMemcpyFromSymbol(h_A, "constData", size, 1);
  // CHECK: errorCode = CHECK_SYCL_ERROR(q_ct1.memcpy(h_A, (char *)(constData.get_ptr()) + 1, size));
  errorCode = cudaMemcpyFromSymbol(h_A, constData, size, 1);
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(q_ct1.memcpy(h_A, (char *)(constData.get_ptr()) + 1, size).wait()));
  ...
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(q_ct1.memcpy(h_A, (char *)(constData.get_ptr()) + 1, size)));
  ...
  // CHECK: errorCode = CHECK_SYCL_ERROR(q_ct1.memcpy(h_A, (char *)(constData.get_ptr()) + 2, size));
  errorCode = cudaMemcpyFromSymbolAsync(h_A, constData, size, 2, cudaMemcpyDeviceToHost, 0);
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(q_ct1.memcpy(h_A, (char *)(constData.get_ptr()) + 2, size)));
  ...
  // CHECK: stream->memcpy(h_A, (char *)(constData.get_ptr(*stream)) + 3, size);
  cudaMemcpyFromSymbolAsync(h_A, "constData", size, 3, cudaMemcpyDeviceToHost, stream);
  // CHECK: errorCode = CHECK_SYCL_ERROR(stream->memcpy(h_A, (char *)(constData.get_ptr(*stream)) + 3, size));
  errorCode = cudaMemcpyFromSymbolAsync(h_A, constData, size, 3, cudaMemcpyDeviceToHost, stream);
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(stream->memcpy(h_A, (char *)(constData.get_ptr(*stream)) + 3, size)));
  ...
  // CHECK: q_ct1.memcpy((char *)(constData.get_ptr()) + 1, h_A, size).wait();
  cudaMemcpyToSymbol("constData", h_A, size, 1);
  // CHECK: errorCode = CHECK_SYCL_ERROR(q_ct1.memcpy((char *)(constData.get_ptr()) + 1, h_A, size));
  errorCode = cudaMemcpyToSymbol(constData, h_A, size, 1);
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(q_ct1.memcpy((char *)(constData.get_ptr()) + 1, h_A, size).wait()));
  ...
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(q_ct1.memcpy((char *)(constData.get_ptr()) + 1, h_A, size)));
  ...
  // CHECK: q_ct1.memcpy((char *)(constData.get_ptr()) + 2, h_A, size);
  cudaMemcpyToSymbolAsync("constData", h_A, size, 2, cudaMemcpyHostToDevice, 0);
  // CHECK: errorCode = CHECK_SYCL_ERROR(q_ct1.memcpy((char *)(constData.get_ptr()) + 2, h_A, size));
  errorCode = cudaMemcpyToSymbolAsync(constData, h_A, size, 2, cudaMemcpyHostToDevice, 0);
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(q_ct1.memcpy((char *)(constData.get_ptr()) + 2, h_A, size)));
  ...
  // CHECK: stream->memcpy((char *)(constData.get_ptr(*stream)) + 3, h_A, size);
  cudaMemcpyToSymbolAsync("constData", h_A, size, 3, cudaMemcpyHostToDevice, stream);
  // CHECK: errorCode = CHECK_SYCL_ERROR(stream->memcpy((char *)(constData.get_ptr(*stream)) + 3, h_A, size));
  errorCode = cudaMemcpyToSymbolAsync(constData, h_A, size, 3, cudaMemcpyHostToDevice, stream);
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(stream->memcpy((char *)(constData.get_ptr(*stream)) + 3, h_A, size)));
  ...
  // CHECK: errorCode = CHECK_SYCL_ERROR(q_ct1.memset(d_A, 23, size).wait());
  errorCode = cudaMemset(d_A, 23, size);
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(q_ct1.memset(d_A, 23, size).wait()));
  ...
  // CHECK: errorCode = CHECK_SYCL_ERROR(q_ct1.memset(d_A, 23, size));
  errorCode = cudaMemsetAsync(d_A, 23, size);
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(q_ct1.memset(d_A, 23, size)));
  ...
  // CHECK: errorCode = CHECK_SYCL_ERROR(stream->memset(d_A, 23, size));
  errorCode = cudaMemsetAsync(d_A, 23, size, stream);
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(stream->memset(d_A, 23, size)));
...
int foo2() {
  ...
  // CHECK: q_ct1.memcpy(h_A, constData.get_ptr(), size).wait();
  cudaMemcpyFromSymbol(h_A, "constData", size);
  // CHECK: errorCode = CHECK_SYCL_ERROR(q_ct1.memcpy(h_A, constData.get_ptr(), size));
  errorCode = cudaMemcpyFromSymbol(h_A, constData, size);
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(q_ct1.memcpy(h_A, constData.get_ptr(), size).wait()));
  ...
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(q_ct1.memcpy(h_A, constData.get_ptr(), size)));
  ...
  // CHECK: q_ct1.memcpy(constData.get_ptr(), h_A, size).wait();
  cudaMemcpyToSymbol("constData", h_A, size);
  // CHECK: errorCode = CHECK_SYCL_ERROR(q_ct1.memcpy(constData.get_ptr(), h_A, size));
  errorCode = cudaMemcpyToSymbol(constData, h_A, size);
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(q_ct1.memcpy(constData.get_ptr(), h_A, size).wait()));
  ...
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(q_ct1.memcpy(constData.get_ptr(), h_A, size)));
...
void foo3() {
  ...
  // CHECK: q_ct1.memcpy(h_A, (char *)(constData.get_ptr()) + 3, size);
  // CHECK: q_ct1.memcpy(h_A, (char *)(constData.get_ptr()) + 3, size);
  // CHECK: errorCode = CHECK_SYCL_ERROR(q_ct1.memcpy(h_A, (char *)(constData.get_ptr()) + 3, size));
  // CHECK: errorCode = CHECK_SYCL_ERROR(q_ct1.memcpy(h_A, (char *)(constData.get_ptr()) + 3, size));
  // CHECK: errorCode = CHECK_SYCL_ERROR(q_ct1.memcpy(h_A, (char *)(constData.get_ptr()) + 3, size));
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(q_ct1.memcpy(h_A, (char *)(constData.get_ptr()) + 3, size)));
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(q_ct1.memcpy(h_A, (char *)(constData.get_ptr()) + 3, size)));
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(q_ct1.memcpy(h_A, (char *)(constData.get_ptr()) + 3, size)));
  ...
  // CHECK: dpct::dev_mgr::instance().get_device(deviceID).default_queue().prefetch(d_A,100);
  // CHECK: dpct::dev_mgr::instance().get_device(deviceID).default_queue().prefetch(d_A,100);
  // CHECK: errorCode = CHECK_SYCL_ERROR(dpct::dev_mgr::instance().get_device(deviceID).default_queue().prefetch(d_A,100));
  // CHECK: errorCode = CHECK_SYCL_ERROR(dpct::dev_mgr::instance().get_device(deviceID).default_queue().prefetch(d_A,100));
  // CHECK: errorCode = CHECK_SYCL_ERROR(dpct::dev_mgr::instance().get_device(deviceID).default_queue().prefetch(d_A,100));
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(dpct::dev_mgr::instance().get_device(deviceID).default_queue().prefetch(d_A,100)));
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(dpct::dev_mgr::instance().get_device(deviceID).default_queue().prefetch(d_A,100)));
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(dpct::dev_mgr::instance().get_device(deviceID).default_queue().prefetch(d_A,100)));
  ...
  // CHECK: dpct::dev_mgr::instance().get_device(cudevice).default_queue().prefetch(devPtr, 100);
  // CHECK: dpct::dev_mgr::instance().get_device(cudevice).default_queue().prefetch(devPtr, 100);
  // CHECK: errorCode = CHECK_SYCL_ERROR(dpct::dev_mgr::instance().get_device(cudevice).default_queue().prefetch(devPtr, 100));
  // CHECK: errorCode = CHECK_SYCL_ERROR(dpct::dev_mgr::instance().get_device(cudevice).default_queue().prefetch(devPtr, 100));
  // CHECK: errorCode = CHECK_SYCL_ERROR(dpct::dev_mgr::instance().get_device(cudevice).default_queue().prefetch(devPtr, 100));
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(dpct::dev_mgr::instance().get_device(cudevice).default_queue().prefetch(devPtr, 100)));
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(dpct::dev_mgr::instance().get_device(cudevice).default_queue().prefetch(devPtr, 100)));
  // CHECK: MY_SAFE_CALL(CHECK_SYCL_ERROR(dpct::dev_mgr::instance().get_device(cudevice).default_queue().prefetch(devPtr, 100)));
...
void foo5(float* a) {
// CHECK: MY_SAFE_CALL3(CHECK_SYCL_ERROR(q_ct1.memcpy(a, a, 16)));
// CHECK: MY_SAFE_CALL3(CHECK_SYCL_ERROR(q_ct1.memcpy(a, a, 16)));
// CHECK: MY_SAFE_CALL3(CHECK_SYCL_ERROR(q_ct1.memcpy(a, a, 16).wait()));
...
void foo6(float* a) {
  // CHECK: printf("%d\n", CHECK_SYCL_ERROR(q_ct1.memcpy(a, a, 16).wait()));
  // CHECK: printf("%d\n", CHECK_SYCL_ERROR(q_ct1.memcpy(a, a, 16).wait()));
...
void foo13(float* a, bool flag) {
  ...
  while(flag) {
    ...
    if(flag) {
      // CHECK: MY_SAFE_CALL3(CHECK_SYCL_ERROR(q_ct1.memcpy(a, a, 16)));
      // CHECK: MY_SAFE_CALL3(CHECK_SYCL_ERROR(q_ct1.memcpy(constData.get_ptr(), a, 16).wait()));
      MY_SAFE_CALL3(cudaMemcpy(a, a, 16, cudaMemcpyDeviceToHost));
      MY_SAFE_CALL3(cudaMemcpyToSymbol(constData, a, 16));
    } else {
      // CHECK: MY_SAFE_CALL3(CHECK_SYCL_ERROR(q_ct1.memcpy(constData.get_ptr(), a, 16)));
      // CHECK: MY_SAFE_CALL3(CHECK_SYCL_ERROR(q_ct1.memcpy(a, a, 16).wait()));
      MY_SAFE_CALL3(cudaMemcpyToSymbol(constData, a, 16));
      MY_SAFE_CALL3(cudaMemcpy(a, a, 16, cudaMemcpyDeviceToHost));
    }
  ...
  do {
    // CHECK: MY_SAFE_CALL3(CHECK_SYCL_ERROR(q_ct1.memcpy(a, a, 16)));
    // CHECK: MY_SAFE_CALL3(CHECK_SYCL_ERROR(q_ct1.memcpy(a, constData.get_ptr(), 16).wait()));
    MY_SAFE_CALL3(cudaMemcpy(a, a, 16, cudaMemcpyDeviceToHost));
    MY_SAFE_CALL3(cudaMemcpyFromSymbol(a, constData, 16));
  } while(flag);

  for(;;) {
    // CHECK: MY_SAFE_CALL3(CHECK_SYCL_ERROR(q_ct1.memcpy(a, constData.get_ptr(), 16)));
    // CHECK: MY_SAFE_CALL3(CHECK_SYCL_ERROR(q_ct1.memcpy(a, a, 16).wait()));
    MY_SAFE_CALL3(cudaMemcpyFromSymbol(a, constData, 16));
    MY_SAFE_CALL3(cudaMemcpy(a, a, 16, cudaMemcpyDeviceToHost));
  }
