...
// UNSUPPORTED: v8.0
// RUN: dpct --no-dpcpp-extensions=free-function-queries --format-range=none --assume-nd-range-dim=1  -out-root %T/kernel_1d_range_sync_api %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
