...
void foo3() {
  ...
  // CHECK: dpct::async_dpct_memcpy(d_A, h_A, size, dpct::host_to_device);
  // CHECK: dpct::async_dpct_memcpy(d_A, h_A, size, dpct::host_to_device);
  // CHECK: errorCode = DPCT_CHECK_ERROR(dpct::async_dpct_memcpy(d_A, h_A, size, dpct::host_to_device));
  // CHECK: errorCode = DPCT_CHECK_ERROR(dpct::async_dpct_memcpy(d_A, h_A, size, dpct::host_to_device));
  // CHECK: errorCode = DPCT_CHECK_ERROR(dpct::async_dpct_memcpy(d_A, h_A, size, dpct::host_to_device));
  // CHECK: MY_SAFE_CALL(DPCT_CHECK_ERROR(dpct::async_dpct_memcpy(d_A, h_A, size, dpct::host_to_device)));
  // CHECK: MY_SAFE_CALL(DPCT_CHECK_ERROR(dpct::async_dpct_memcpy(d_A, h_A, size, dpct::host_to_device)));
  // CHECK: MY_SAFE_CALL(DPCT_CHECK_ERROR(dpct::async_dpct_memcpy(d_A, h_A, size, dpct::host_to_device)));
  ...
  // CHECK: dpct::async_dpct_memcpy((char *)(constData.get_ptr()) + 1, h_A, size, dpct::host_to_device);
  // CHECK: dpct::async_dpct_memcpy((char *)(constData.get_ptr()) + 1, h_A, size, dpct::host_to_device);
  // CHECK: errorCode = DPCT_CHECK_ERROR(dpct::async_dpct_memcpy((char *)(constData.get_ptr()) + 1, h_A, size, dpct::host_to_device));
  // CHECK: errorCode = DPCT_CHECK_ERROR(dpct::async_dpct_memcpy((char *)(constData.get_ptr()) + 1, h_A, size, dpct::host_to_device));
  // CHECK: errorCode = DPCT_CHECK_ERROR(dpct::async_dpct_memcpy((char *)(constData.get_ptr()) + 1, h_A, size, dpct::host_to_device));
  // CHECK: MY_SAFE_CALL(DPCT_CHECK_ERROR(dpct::async_dpct_memcpy((char *)(constData.get_ptr()) + 1, h_A, size, dpct::host_to_device)));
  // CHECK: MY_SAFE_CALL(DPCT_CHECK_ERROR(dpct::async_dpct_memcpy((char *)(constData.get_ptr()) + 1, h_A, size, dpct::host_to_device)));
  // CHECK: MY_SAFE_CALL(DPCT_CHECK_ERROR(dpct::async_dpct_memcpy((char *)(constData.get_ptr()) + 1, h_A, size, dpct::host_to_device)));
  ...
  // CHECK: dpct::async_dpct_memcpy(h_A, (char *)(constData.get_ptr()) + 3, size, dpct::device_to_host);
  // CHECK: dpct::async_dpct_memcpy(h_A, (char *)(constData.get_ptr()) + 3, size, dpct::device_to_host);
  // CHECK: errorCode = DPCT_CHECK_ERROR(dpct::async_dpct_memcpy(h_A, (char *)(constData.get_ptr()) + 3, size, dpct::device_to_host));
  // CHECK: errorCode = DPCT_CHECK_ERROR(dpct::async_dpct_memcpy(h_A, (char *)(constData.get_ptr()) + 3, size, dpct::device_to_host));
  // CHECK: errorCode = DPCT_CHECK_ERROR(dpct::async_dpct_memcpy(h_A, (char *)(constData.get_ptr()) + 3, size, dpct::device_to_host));
  // CHECK: MY_SAFE_CALL(DPCT_CHECK_ERROR(dpct::async_dpct_memcpy(h_A, (char *)(constData.get_ptr()) + 3, size, dpct::device_to_host)));
  // CHECK: MY_SAFE_CALL(DPCT_CHECK_ERROR(dpct::async_dpct_memcpy(h_A, (char *)(constData.get_ptr()) + 3, size, dpct::device_to_host)));
  // CHECK: MY_SAFE_CALL(DPCT_CHECK_ERROR(dpct::async_dpct_memcpy(h_A, (char *)(constData.get_ptr()) + 3, size, dpct::device_to_host)));
  ...
  // CHECK: dpct::async_dpct_memset(d_A, 23, size);
  // CHECK: dpct::async_dpct_memset(d_A, 23, size);
  // CHECK: errorCode = DPCT_CHECK_ERROR(dpct::async_dpct_memset(d_A, 23, size));
  // CHECK: errorCode = DPCT_CHECK_ERROR(dpct::async_dpct_memset(d_A, 23, size));
  // CHECK: errorCode = DPCT_CHECK_ERROR(dpct::async_dpct_memset(d_A, 23, size));
  // CHECK: MY_SAFE_CALL(DPCT_CHECK_ERROR(dpct::async_dpct_memset(d_A, 23, size)));
  // CHECK: MY_SAFE_CALL(DPCT_CHECK_ERROR(dpct::async_dpct_memset(d_A, 23, size)));
  // CHECK: MY_SAFE_CALL(DPCT_CHECK_ERROR(dpct::async_dpct_memset(d_A, 23, size)));
...
void foo4(){
  ...
  // CHECK: errorCode = DPCT_CHECK_ERROR(h_A = (float *)malloc(size));
  errorCode = cuMemHostAlloc((void **)&h_A, size, CU_MEMHOSTALLOC_PORTABLE);
  // CHECK: MY_SAFE_CALL(DPCT_CHECK_ERROR(h_A = (float *)malloc(size)));
