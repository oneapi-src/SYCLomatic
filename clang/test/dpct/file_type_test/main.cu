...
// RUN: dpct --format-range=none  -in-root=%S -out-root=%T -p=%T %s %S/bar/util.gpu  --sycl-named-lambda -extra-arg="-I%S/bar" 
...
// RUN: dpct --format-range=none  -in-root=%S -out-root=%T  %S/main.gpu   --sycl-named-lambda 
