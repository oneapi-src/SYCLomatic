...
// RUN: cd %T
// RUN: c2s --no-cl-namespace-inline -out-root %T/formatMigratedLLVM formatMigratedLLVM.cu --cuda-include-path="%cuda-path/include" --format-style=llvm  -- -std=c++14  -x cuda --cuda-host-only
