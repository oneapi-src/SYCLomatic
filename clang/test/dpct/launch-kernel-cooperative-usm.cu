...
// CHECK:         auto tex_acc = static_cast<dpct::image_wrapper<int, 1> *>(tex)->get_access(cgh, queue);
// CHECK:         auto tex_smpl = (tex)->get_sampler();
// CHECK:         cgh.parallel_for(
// CHECK:           nr,
// CHECK:           [=](sycl::nd_item<3> item_ct1) {
// CHECK:             kernel(d, dpct::image_accessor_ext<int, 1>(tex_smpl, tex_acc));
// CHECK:           });
// CHECK:       });
// CHECK: }
...
int main() {
  ...
  // CHECK-NEXT:        kernel(d_ct0, dpct::image_accessor_ext<int, 1>(tex_smpl, tex_acc));
  // CHECK-NEXT:      });
  // CHECK-NEXT:  });
  ...
  // CHECK-NEXT:      sycl::nd_range<3>(sycl::range<3>(1, 1, 16) * sycl::range<3>(1, 1, 16), sycl::range<3>(1, 1, 16)),
  // CHECK-NEXT:      [=](sycl::nd_item<3> item_ct1) {
  // CHECK-NEXT:        template_kernel<int>(d_ct0, dpct_local_acc_ct1.template get_multi_ptr<sycl::access::decorated::no>().get(), s_acc_ct1.template get_multi_ptr<sycl::access::decorated::no>().get());
  // CHECK-NEXT:      });
  // CHECK-NEXT:  });
