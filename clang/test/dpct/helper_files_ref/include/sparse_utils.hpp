...
public:
  ...
  template <typename T> auto get() const { return _mat_type; }
  template <> auto get<oneapi::mkl::diag>() const { return _unit_diag; }
  template <> auto get<oneapi::mkl::uplo>() const { return _upper_lower; }
  template <> auto get<oneapi::mkl::index_base>() const { return _index; }
...
void spmv(
    sycl::queue &queue, oneapi::mkl::transpose trans, const void *alpha,
    sparse_matrix_handle_t mat_handle, dense_vector_handle_t x,
    const void *beta, dense_vector_handle_t y, library_data_t compute_type,
    sparse_matrix_info::matrix_type mt = sparse_matrix_info::matrix_type::ge) {
...
void csrmv(sycl::queue &queue, oneapi::mkl::transpose trans, int num_rows,
           int num_cols, const T *alpha, const sparse_matrix_info info,
           const T *val, const int *row_ptr, const int *col_ind, const T *x,
           const T *beta, T *y) {
  ...
  switch (info.get<sparse_matrix_info::matrix_type>()) {
  case sparse_matrix_info::matrix_type::ge: {
  ...
  default:
    throw std::runtime_error(
        "the spmv does not support sparse_matrix_info::matrix_type::he");
  }
