// RUN: dpct --no-dpcpp-extensions=free-function-queries --format-range=none -out-root %T/template-instantiation %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only -std=c++14
