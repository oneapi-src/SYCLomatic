...
// CHECK: __dpct_inline__ void kernel(int *, const sycl::nd_item<3> &item_ct1, T *a);
template<class T>
__global__ void kernel(int *);

// CHECK: template __dpct_inline__ void kernel<sycl::float2>(int *, const sycl::nd_item<3> &item_ct1,
// CHECK-NEXT: sycl::float2 *a);
template __global__ void kernel<float2>(int *);

// CHECK: template __dpct_inline__ void kernel<int>(int *, const sycl::nd_item<3> &item_ct1, int *a);
template __global__ void kernel<int>(int *);

template<class T>
// CHECK: __dpct_inline__ void kernel1(T *, const sycl::nd_item<3> &item_ct1, T *a) {
__global__ void kernel1(T *) {
  __shared__ T a[10];
  int b = blockDim.x;
}

// CHECK: template __dpct_inline__ void kernel1(sycl::char4 *, const sycl::nd_item<3> &item_ct1, sycl::char4 *a);
template __global__ void kernel1(char4 *);

// CHECK: template __dpct_inline__ void kernel1<int>(int *, const sycl::nd_item<3> &item_ct1, int *a);
template __global__ void kernel1<int>(int *);

// CHECK: __dpct_inline__ void kernel2(T1 *, T2 *, const sycl::nd_item<3> &item_ct1, T1 *a1, T2 *a2) {
...
// CHECK: template __dpct_inline__ void kernel2(sycl::char4 *, int *, const sycl::nd_item<3> &item_ct1,
// CHECK-NEXT: sycl::char4 *a1, int *a2);
template __global__ void kernel2(char4 *, int *);

// CHECK: template __dpct_inline__ void kernel2<int>(int *, sycl::float2 *, const sycl::nd_item<3> &item_ct1,
// CHECK-NEXT:  int *a1, sycl::float2 *a2);
template __global__ void kernel2<int>(int *, float2 *);

// CHECK: template __dpct_inline__ void kernel2<sycl::float2, sycl::char4>(sycl::float2 *, sycl::char4 *,
...
// CHECK: __dpct_inline__ void kernel3(T *, const sycl::nd_item<3> &item_ct1, T *a) {
__global__ void kernel3(T *) {
  __shared__ T a[S];
  int b = blockDim.x;
}
// CHECK: template __dpct_inline__ void kernel3<20>(int *, const sycl::nd_item<3> &item_ct1, int *a);
