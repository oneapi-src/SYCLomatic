...
// CHECK: dpct::image_wrapper<sycl::mfloat4, 3, true> tex42;
static texture<float4, cudaTextureType2DLayered> tex42;
// CHECK: /*
// CHECK: DPCT1059:{{[0-9]+}}: SYCL only supports 4-channel image format. Adjust the code.
// CHECK: */
// CHECK: dpct::image_wrapper<sycl::muint2, 2, true> tex21;
static texture<uint2, cudaTextureType1DLayered> tex21;

// CHECK: void device01(sycl::mfloat4 *out, dpct::image_accessor_ext<sycl::muint2, 1, true> tex21) {
// CHECK-NEXT: sycl::muint2 u21 = tex21.read(12, 1.0f);
// CHECK-NEXT: out[0][0] =  u21[0];
__device__ void device01(float4 *out) {
  uint2 u21 = tex1DLayered(tex21, 1.0f, 12);
  out[0].x = u21.x;
}

// CHECK: void kernel(sycl::mfloat4 *out, dpct::image_accessor_ext<sycl::mfloat4, 2, true> tex42,
// CHECK-NEXT:        dpct::image_accessor_ext<sycl::muint2, 1, true> tex21) {
...
int main() {
  ...
  // CHECK-NEXT:               kernel((sycl::mfloat4 *)(&d_acc_ct0[0]), dpct::image_accessor_ext<sycl::mfloat4, 2, true>(tex42_smpl, tex42_acc), dpct::image_accessor_ext<sycl::muint2, 1, true>(tex21_smpl, tex21_acc));
  // CHECK-NEXT:             });
  // CHECK-NEXT:       });
  kernel<<<1, 1>>>(d);
