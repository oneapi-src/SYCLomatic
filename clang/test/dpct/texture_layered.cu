// RUN: c2s --format-range=none --usm-level=none -out-root %T/texture_layered %s --cuda-include-path="%cuda-path/include" --sycl-named-lambda -- -x cuda --cuda-host-only -std=c++14
...
// CHECK: */
// CHECK: c2s::image_wrapper<int, 2, true> tex_no_ref;
static texture<int, cudaTextureType1DLayered> tex_no_ref;
// CHECK: c2s::image_wrapper<sycl::float4, 3, true> tex42;
static texture<float4, cudaTextureType2DLayered> tex42;
// CHECK: /*
// CHECK: DPCT1059:{{[0-9]+}}: SYCL only supports 4-channel image format. Adjust the code.
// CHECK: */
// CHECK: c2s::image_wrapper<sycl::uint2, 2, true> tex21;
static texture<uint2, cudaTextureType1DLayered> tex21;

// CHECK: void device01(sycl::float4 *out, c2s::image_accessor_ext<sycl::uint2, 1, true> tex21) {
...
// CHECK: void kernel(sycl::float4 *out, c2s::image_accessor_ext<sycl::float4, 2, true> tex42,
// CHECK-NEXT:        c2s::image_accessor_ext<sycl::uint2, 1, true> tex21) {
...
int main() {
  ...
  // CHECK-NEXT:         auto d_acc_ct0 = c2s::get_access(d, cgh);
  // CHECK-EMPTY:
  ...
  // CHECK-NEXT:               kernel((sycl::float4 *)(&d_acc_ct0[0]), c2s::image_accessor_ext<sycl::float4, 2, true>(tex42_smpl, tex42_acc), c2s::image_accessor_ext<sycl::uint2, 1, true>(tex21_smpl, tex21_acc));
  // CHECK-NEXT:             });
  // CHECK-NEXT:       });
  kernel<<<1, 1>>>(d);
