...
// RUN: dpct --no-dpcpp-extensions=free-function-queries --use-experimental-features=non-stdandard-sycl-builtins --format-range=none -out-root %T/sycl_exp_printf %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
