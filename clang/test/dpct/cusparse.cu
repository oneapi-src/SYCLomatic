...
int main(){
  ...
  //CHECK-NEXT: handle->set_queue(s);
  //CHECK-NEXT: s = &(handle->get_queue());
  ...
  cusparseScsrmv(handle, (cusparseOperation_t)zero, m, n, nnz, &alpha, descrA, csrValA, csrRowPtrA, csrColIndA, x, &beta_value, y);
...
void foo4() {
  ...
  //CHECK:dpct::sparse::csrsv(handle->get_queue(), oneapi::mkl::transpose::nontrans, 3, &alpha_s, descrA, a_s_val, a_row_ptr, a_col_ind, info, f_s, x_s);
  //CHECK-NEXT:dpct::sparse::csrsv(handle->get_queue(), oneapi::mkl::transpose::nontrans, 3, &alpha_d, descrA, a_d_val, a_row_ptr, a_col_ind, info, f_d, x_d);
  //CHECK-NEXT:dpct::sparse::csrsv(handle->get_queue(), oneapi::mkl::transpose::nontrans, 3, &alpha_c, descrA, a_c_val, a_row_ptr, a_col_ind, info, f_c, x_c);
  //CHECK-NEXT:dpct::sparse::csrsv(handle->get_queue(), oneapi::mkl::transpose::nontrans, 3, &alpha_z, descrA, a_z_val, a_row_ptr, a_col_ind, info, f_z, x_z);
  //CHECK-NEXT:info.reset();
  ...
  //CHECK-NEXT:dpct::sparse::csrsv(handle->get_queue(), oneapi::mkl::transpose::nontrans, 3, &alpha_z, descrA, a_z_val, a_row_ptr, a_col_ind, info2, f_z, x_z);
...
void foo5() {
  ...
  //CHECK:dpct::sparse::csr2csc(handle->get_queue(), 3, 4, 7, a_s_val, a_row_ptr, a_col_ind, b_s_val, b_col_ptr, b_row_ind, dpct::sparse::conversion_scope::index_and_value, oneapi::mkl::index_base::zero);
  //CHECK-NEXT:dpct::sparse::csr2csc(handle->get_queue(), 3, 4, 7, a_d_val, a_row_ptr, a_col_ind, b_d_val, b_col_ptr, b_row_ind, dpct::sparse::conversion_scope::index_and_value, oneapi::mkl::index_base::zero);
  //CHECK-NEXT:dpct::sparse::csr2csc(handle->get_queue(), 3, 4, 7, a_c_val, a_row_ptr, a_col_ind, b_c_val, b_col_ptr, b_row_ind, dpct::sparse::conversion_scope::index_and_value, oneapi::mkl::index_base::zero);
  //CHECK-NEXT:dpct::sparse::csr2csc(handle->get_queue(), 3, 4, 7, a_z_val, a_row_ptr, a_col_ind, b_z_val, b_col_ptr, b_row_ind, dpct::sparse::conversion_scope::index_and_value, oneapi::mkl::index_base::zero);
...
void foo6(){
  ...
  // CHECK: dpct::sparse::csrgemm_nnz(handle, oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans, 3, 4, 3, descrA, 4, val_a_s, row_ptr_a, col_ind_a, descrB, 5, val_b_s, row_ptr_b, col_ind_b, nullptr, nullptr, nullptr, nullptr, descrC, row_ptr_c, &c_nnz);
  // CHECK-NEXT: dpct::sparse::csrgemm(handle, oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans, 3, 4, 3, descrA, val_a_s, row_ptr_a, col_ind_a, descrB, val_b_s, row_ptr_b, col_ind_b, nullptr, nullptr, nullptr, nullptr, descrC, val_c_s, row_ptr_c, col_ind_c);
  // CHECK-NEXT: dpct::sparse::csrgemm(handle, oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans, 3, 4, 3, descrA, val_a_d, row_ptr_a, col_ind_a, descrB, val_b_d, row_ptr_b, col_ind_b, nullptr, nullptr, nullptr, nullptr, descrC, val_c_d, row_ptr_c, col_ind_c);
  // CHECK-NEXT: dpct::sparse::csrgemm(handle, oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans, 3, 4, 3, descrA, val_a_c, row_ptr_a, col_ind_a, descrB, val_b_c, row_ptr_b, col_ind_b, nullptr, nullptr, nullptr, nullptr, descrC, val_c_c, row_ptr_c, col_ind_c);
  // CHECK-NEXT: dpct::sparse::csrgemm(handle, oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans, 3, 4, 3, descrA, val_a_z, row_ptr_a, col_ind_a, descrB, val_b_z, row_ptr_b, col_ind_b, nullptr, nullptr, nullptr, nullptr, descrC, val_c_z, row_ptr_c, col_ind_c);
...
void foo7_2(int c_nnz,
            ...
            int* row_ptr_c,
            int* col_ind_c) {
  // CHECK: dpct::sparse::csrgemm(handle, oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans, 3, 4, 3, descrA, val_a_s, row_ptr_a, col_ind_a, descrB, val_b_s, row_ptr_b, col_ind_b, nullptr, nullptr, nullptr, nullptr, descrC, val_c_s, row_ptr_c, col_ind_c);
  cusparseScsrgemm(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, CUSPARSE_OPERATION_NON_TRANSPOSE, 3, 4, 3, descrA, 4, val_a_s, row_ptr_a, col_ind_a, descrB, 5, val_b_s, row_ptr_b, col_ind_b, descrC, val_c_s, row_ptr_c, col_ind_c);
}
...
void foo7_1(int c_nnz,
  ...
  // CHECK-NEXT: */
  // CHECK-NEXT: dpct::sparse::csrgemm_nnz(handle, oneapi::mkl::transpose::nontrans, oneapi::mkl::transpose::nontrans, 3, 4, 3, descrA, 4, dpct_placeholder, row_ptr_a, col_ind_a, descrB, 5, dpct_placeholder, row_ptr_b, col_ind_b, nullptr, nullptr, nullptr, nullptr, descrC, row_ptr_c, &c_nnz);
  cusparseXcsrgemmNnz(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, CUSPARSE_OPERATION_NON_TRANSPOSE, 3, 4, 3, descrA, 4, row_ptr_a,col_ind_a, descrB, 5, row_ptr_b, col_ind_b, descrC, row_ptr_c, &c_nnz);
