...
// RUN: cat %s > %T/c_kernel.cu
// RUN: cd %T

// RUN: dpct --format-range=none  --vcxprojfile=%T/proj_c.vcxproj  -in-root=%T -out-root=%T/out  c_kernel.cu %T/CuTmp_1.cu --cuda-include-path="%cuda-path/include"

// RUN: cat %S/CuTmp_1.cu > %T/out/CuTmp_1.dp.cpp
...
// RUN: FileCheck %S/CuTmp_1.cu --match-full-lines --input-file %T/out/CuTmp_1.dp.cpp
