...
// RUN: dpct --format-range=none -out-root %T/cuda-math-intrinsics-cuda12-after %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only --std=c++14
// RUN: FileCheck --input-file %T/cuda-math-intrinsics-cuda12-after/cuda-math-intrinsics-cuda12-after.dp.cpp --match-full-lines %s
