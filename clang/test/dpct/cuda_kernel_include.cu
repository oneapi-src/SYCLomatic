...
// CHECK-NEXT:  int tmp = dpct::min((item_ct1.get_group(2)+1)*item_ct1.get_local_range(2)+item_ct1.get_local_id(2), dpct::max(index, 45));
// CHECK-NEXT:  int num = dpct::max((item_ct1.get_group(2)+1)*item_ct1.get_local_range(2)+item_ct1.get_local_id(2), dpct::min(tmp, 45));
