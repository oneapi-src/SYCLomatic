// RUN: dpct --format-range=none --no-dpcpp-extensions=free_mem_size --usm-level=none  --use-custom-helper=api -out-root %T/Device/api_test13_out %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
