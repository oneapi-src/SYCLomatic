// RUN: dpct --format-range=none --use-custom-helper=api -out-root %T/DnnlUtils/api_test15_out %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
// RUN: grep "IsCalled" %T/DnnlUtils/api_test15_out/MainSourceFiles.yaml | wc -l > %T/DnnlUtils/api_test15_out/count.txt
// RUN: FileCheck --input-file %T/DnnlUtils/api_test15_out/count.txt --match-full-lines %s
// RUN: rm -rf %T/DnnlUtils/api_test15_out

// CHECK: 18
// TEST_FEATURE: DnnlUtils_batch_normalization_forward_training_ex
