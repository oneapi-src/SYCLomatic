...
// RUN: c2s --format-range=none  --use-custom-helper=api -out-root %T/DplExtrasDpcppExtensions/api_test3_out %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only -std=c++14
