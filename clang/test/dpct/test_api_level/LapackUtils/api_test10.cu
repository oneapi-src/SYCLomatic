...
// RUN: dpct --format-range=none --use-custom-helper=api -out-root %T/LapackUtils/api_test10_out %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
// RUN: grep "IsCalled" %T/LapackUtils/api_test10_out/MainSourceFiles.yaml | wc -l > %T/LapackUtils/api_test10_out/count.txt
// RUN: FileCheck --input-file %T/LapackUtils/api_test10_out/count.txt --match-full-lines %s
// RUN: rm -rf %T/LapackUtils/api_test10_out
