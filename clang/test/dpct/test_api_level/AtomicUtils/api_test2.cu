...
// RUN: dpct --format-range=none  --usm-level=none  --use-custom-helper=api -out-root %T/AtomicUtils/api_test2_out %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
// RUN: grep "IsCalled" %T/AtomicUtils/api_test2_out/MainSourceFiles.yaml | wc -l > %T/AtomicUtils/api_test2_out/count.txt
// RUN: FileCheck --input-file %T/AtomicUtils/api_test2_out/count.txt --match-full-lines %s
// RUN: rm -rf %T/AtomicUtils/api_test2_out
