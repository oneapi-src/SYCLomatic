...
// UNSUPPORTED: v8.0
// RUN: c2s --format-range=none  --usm-level=none  --use-custom-helper=api -out-root %T/DplExtrasMemory/api_test3_out %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only -std=c++17
