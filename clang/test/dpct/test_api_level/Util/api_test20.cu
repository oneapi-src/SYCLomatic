// RUN: dpct --format-range=none  --usm-level=none  --use-custom-helper=api -out-root %T/Util/api_test20_out %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
