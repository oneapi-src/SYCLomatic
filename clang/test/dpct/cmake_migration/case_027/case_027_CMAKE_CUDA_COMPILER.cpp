...
// RUN: dpct -in-root ./ -out-root out  ./input.cmake --migrate-build-script-only --migrate-build-script=CMake
// RUN: echo "begin" > %T/diff.txt
// RUN: diff --strip-trailing-cr %S/expected.txt %T/out/input.cmake >> %T/diff.txt
// RUN: echo "end" >> %T/diff.txt

// RUN: dpct  ./input.cmake --migrate-build-script-only --migrate-build-script=CMake
