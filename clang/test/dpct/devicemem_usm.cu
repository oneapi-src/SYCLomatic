// RUN: dpct --no-dpcpp-extensions=free-function-queries --format-range=none -out-root %T/devicemem_usm %s --cuda-include-path="%cuda-path/include" --sycl-named-lambda -- -x cuda --cuda-host-only
// RUN: FileCheck %s --match-full-lines --input-file %T/devicemem_usm/devicemem_usm.dp.cpp
// RUN: %if build_lit %{icpx -c -fsycl %T/devicemem_usm/devicemem_usm.dp.cpp -o %T/devicemem_usm/devicemem_usm.dp.o %}
...
// CHECK: inline dpct::global_memory<TestStruct, 0> t1;
...
// CHECK: inline dpct::global_memory<float, 1> in(NUM_ELEMENTS);
...
// CHECK: inline dpct::global_memory<int, 0> al;
__device__ int al;
// CHECK: inline dpct::global_memory<int, 0> ainit(NUM_ELEMENTS);
__device__ int ainit = NUM_ELEMENTS;

const int num_elements = 16;
// CHECK: inline dpct::global_memory<float, 1> fx(2);
// CHECK: inline dpct::global_memory<float, 2> fy(num_elements, 4 * num_elements);
