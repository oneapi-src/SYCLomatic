...
//CHECK:void d1() {
//CHECK-NEXT:  auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<1>();
...
//CHECK:void d8() {
//CHECK-NEXT:  int a = sycl::ext::oneapi::this_work_item::get_nd_item<1>().get_local_id(0);
//CHECK-NEXT:  d10();
//CHECK-NEXT:}
__device__ void d8() {
  int a = threadIdx.x;
...
int main() {
  ...
  //CHECK:q_ct1.parallel_for(
  //CHECK-NEXT:      sycl::nd_range<1>(sycl::range<1>(7) * sycl::range<1>(8), sycl::range<1>(8)), 
  //CHECK-NEXT:      [=](sycl::nd_item<1> item_ct1) {
  //CHECK-NEXT:        k3();
  //CHECK-NEXT:      });
  k3<<<7, 8>>>();
...
//CHECK-NEXT:   auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<1>();
//CHECK-NEXT:   unsigned int tid = MUL(sycl::ext::oneapi::this_work_item::get_nd_item<1>().get_local_range(0), sycl::ext::oneapi::this_work_item::get_nd_item<1>().get_group(0)) + item_ct1.get_local_range(0);
...
//CHECK: void foo2() {
//CHECK-NEXT:   auto item_ct1 = sycl::ext::oneapi::this_work_item::get_nd_item<3>();
//CHECK-NEXT:   unsigned int tid = MUL(sycl::ext::oneapi::this_work_item::get_nd_item<3>().get_local_range(1), sycl::ext::oneapi::this_work_item::get_nd_item<3>().get_group(0)) + item_ct1.get_local_range(2);
...
//CHECK:void device3() {
//CHECK-NEXT:  int a = sycl::ext::oneapi::this_work_item::get_nd_item<3>().get_local_id(1);
//CHECK-NEXT:}
...
int foo4() {
  //CHECK:dpct::get_in_order_queue().parallel_for(
  //CHECK-NEXT:      sycl::nd_range<1>(sycl::range<1>(3) * sycl::range<1>(4), sycl::range<1>(4)), 
  //CHECK-NEXT:      [=](sycl::nd_item<1> item_ct1) {
  //CHECK-NEXT:        global2();
  //CHECK-NEXT:      });
  global2<<<3 ,4>>>();
...
//CHECK:#define TIDx sycl::ext::oneapi::this_work_item::get_nd_item<1>().get_local_id(0)
