// RUN: dpct --no-dpcpp-extensions=free-function-queries --format-range=none -out-root %T/nested_device_call %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
