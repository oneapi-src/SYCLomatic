// RUN: c2s --format-range=none -out-root %T/driver_device %s --cuda-include-path="%cuda-path/include" -- -std=c++14 -x cuda --cuda-host-only
...
int main(){
  ...
  // CHECK: result1 = c2s::dev_mgr::instance().get_device(device).get_major_version();
  cuDeviceGetAttribute(&result1, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MAJOR, device);

  // CHECK: result1 = c2s::dev_mgr::instance().get_device(device).get_minor_version();
  cuDeviceGetAttribute(&result1, CU_DEVICE_ATTRIBUTE_COMPUTE_CAPABILITY_MINOR, device);

  // CHECK: result1 = c2s::dev_mgr::instance().get_device(device).get_integrated();
  cuDeviceGetAttribute(&result1, CU_DEVICE_ATTRIBUTE_INTEGRATED, device);

  // CHECK: result1 = c2s::dev_mgr::instance().get_device(device).get_max_clock_frequency();
  cuDeviceGetAttribute(&result1, CU_DEVICE_ATTRIBUTE_CLOCK_RATE, device);

  // CHECK: result1 = c2s::dev_mgr::instance().get_device(device).get_max_compute_units();
  cuDeviceGetAttribute(&result1, CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT, device);

  // CHECK: result1 = c2s::dev_mgr::instance().get_device(device).is_native_atomic_supported();
  ...
  // CHECK: MY_SAFE_CALL((result1 = c2s::dev_mgr::instance().get_device(device).get_max_compute_units(), 0));
  MY_SAFE_CALL(cuDeviceGetAttribute(&result1, CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT, device));

  // CHECK: result1 = c2s::dev_mgr::instance().get_device(device).get_major_version();
  // CHECK: result2 = c2s::dev_mgr::instance().get_device(device).get_minor_version();
  cuDeviceComputeCapability(&result1, &result2, device);

  // CHECK: MY_SAFE_CALL([&](){
  // CHECK-NEXT:   result1 = c2s::dev_mgr::instance().get_device(device).get_major_version();
  // CHECK-NEXT:   result2 = c2s::dev_mgr::instance().get_device(device).get_minor_version();
  // CHECK-NEXT:   return 0;
  // CHECK-NEXT: }());
  ...
  // CHECK-NEXT: */
  // CHECK-NEXT: result1 = c2s::get_current_device().get_info<sycl::info::device::version>();
  cuDriverGetVersion(&result1);

  // CHECK: MY_SAFE_CALL((result1 = c2s::dev_mgr::instance().device_count(), 0));
  MY_SAFE_CALL(cuDeviceGetCount(&result1));

  // CHECK: result1 = c2s::dev_mgr::instance().device_count();
  ...
  // CHECK: memcpy(name, c2s::dev_mgr::instance().get_device(device).get_info<sycl::info::device::name>().c_str(), 90);
  cuDeviceGetName(name, 90, device);

  // CHECK: MY_SAFE_CALL((memcpy(name, c2s::dev_mgr::instance().get_device(device).get_info<sycl::info::device::name>().c_str(), 90), 0));
