...
// CHECK-NEXT:        [=](sycl::nd_item<3> item_ct1) {
// CHECK-NEXT:          convert_kernel(b, item_ct1, aaa_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get(), reinterpret_cast<double (*)[0]>(bbb_acc_ct1.get_multi_ptr<sycl::access::decorated::no>().get()));
// CHECK-NEXT:        });
// CHECK-NEXT:    });
// CHECK-NEXT:  }
// CHECK-NEXT:}
