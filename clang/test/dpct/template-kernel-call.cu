...
void runTest() {
  ...
  // CHECK-NEXT:*/
  // CHECK-NEXT:/*
  // CHECK-NEXT:DPCT1128:{{[0-9]+}}: The type "TestTemplate<T>" is used in kernel but it is not device copyable. The tool has added the sycl::is_device_copyable specialization for this type.
  // CHECK-NEXT:*/
  // CHECK-NEXT:   q_ct1.submit(
  // CHECK-NEXT:     [&](sycl::handler &cgh) {
  // CHECK-NEXT:       auto ktarg_ct2 = ktarg;
