// RUN: c2s --format-range=none -usm-level=none -out-root %T/memory_data_types %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
// RUN: FileCheck --match-full-lines --input-file %T/memory_data_types/memory_data_types.dp.cpp %s

void foo(int *data, int x, int y) {
  // CHECK: c2s::pitched_data p1 = c2s::pitched_data(data, x, x, y);
  ...
  p2.extent = extent;
  // CHECK: p2_direction_ct1 = c2s::device_to_host;
  p2.kind = cudaMemcpyDeviceToHost;
  // CHECK: c2s::c2s_memcpy(p2_to_data_ct1, p2_to_pos_ct1, p2_from_data_ct1, p2_from_pos_ct1, p2_size_ct1, p2_direction_ct1);
  cudaMemcpy3D(&p2);

  // CHECK: c2s::pitched_data p3;
  cudaPitchedPtr p3;
  
  // CHECK: (p3 = c2s::c2s_malloc(sycl::range<3>(x, y, 1)), 0);
  cudaMalloc3D(&p3, make_cudaExtent(x, y, 1));

  // CHECK: p2_from_data_ct1 = c2s::pitched_data(data, x, x, y);
  ...
  p2.extent.depth = 1;
  // CHECK: p2_direction_ct1 = c2s::host_to_device;
