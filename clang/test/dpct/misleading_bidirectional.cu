// RUN: dpct --misleading-bidirectional-detection --format-range=none -out-root %T/misleading_bidirectional %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
