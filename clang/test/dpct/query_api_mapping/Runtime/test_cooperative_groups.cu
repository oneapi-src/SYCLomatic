...
// CG_TILED_PARTITION-NEXT:    cooperative_groups::thread_block cta = cooperative_groups::this_thread_block();
...
// CG_TILED_PARTITION-NEXT:    auto cta = sycl::ext::oneapi::experimental::this_group<3>();
...
// CG_THREAD_RANK-NEXT:   cooperative_groups::thread_block cta = cooperative_groups::this_thread_block();
...
// CG_THREAD_RANK-NEXT:   auto cta = sycl::ext::oneapi::experimental::this_group<3>();
...
// CG_THIS_THREAD_BLOCK-NEXT:   cooperative_groups::thread_block cta = cooperative_groups::this_thread_block();
// CG_THIS_THREAD_BLOCK-NEXT: Is migrated to (with the option --use-experimental-features=free-function-queries):
// CG_THIS_THREAD_BLOCK-NEXT:   auto cta = sycl::ext::oneapi::experimental::this_group<3>();
...
// CG_REDUCE-NEXT:    cooperative_groups::thread_block cta = cooperative_groups::this_thread_block();
// CG_REDUCE-NEXT:    const unsigned int tid = cta.thread_rank();
// CG_REDUCE-NEXT:    cooperative_groups::thread_block_tile<32> tile32 = cooperative_groups::tiled_partition<32>(cta);
// CG_REDUCE-NEXT:    cooperative_groups::reduce(tile32 /* type group */, sdata[tid] /* type argument */, cooperative_groups::plus<double>() /* type operator */ );
// CG_REDUCE-NEXT: Is migrated to (with the option --use-experimental-features=free-function-queries):
// CG_REDUCE-NEXT:    double *sdata;
// CG_REDUCE-NEXT:    auto cta = sycl::ext::oneapi::experimental::this_group<3>();
// CG_REDUCE-NEXT:    const unsigned int tid = sycl::ext::oneapi::experimental::this_nd_item<3>().get_local_linear_id();
// CG_REDUCE-NEXT:    sycl::sub_group tile32 = sycl::ext::oneapi::experimental::this_sub_group();
