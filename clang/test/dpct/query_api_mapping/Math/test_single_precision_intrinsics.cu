...
// RUN: dpct --query-api-mapping=__cosf | FileCheck %s -check-prefix=__COSF
// __COSF: CUDA API:
// __COSF-NEXT:   __cosf(f /*float*/);
// __COSF-NEXT: Is migrated to:
// __COSF-NEXT:   sycl::cos(f);

// RUN: dpct --query-api-mapping=__exp10f | FileCheck %s -check-prefix=__EXP10F
// __EXP10F: CUDA API:
// __EXP10F-NEXT:   __exp10f(f /*float*/);
// __EXP10F-NEXT: Is migrated to:
// __EXP10F-NEXT:   sycl::exp10(f);

// RUN: dpct --query-api-mapping=__expf | FileCheck %s -check-prefix=_EXPF
// _EXPF: CUDA API:
// _EXPF-NEXT:   __expf(f /*float*/);
// _EXPF-NEXT: Is migrated to:
// _EXPF-NEXT:   sycl::native::exp(f);

// RUN: dpct --query-api-mapping=__fadd_rd | FileCheck %s -check-prefix=__FADD_RD
// __FADD_RD: CUDA API:
...
// RUN: dpct --query-api-mapping=__fadd_rn | FileCheck %s -check-prefix=__FADD_RN
// __FADD_RN: CUDA API:
...
// RUN: dpct --query-api-mapping=__fadd_ru | FileCheck %s -check-prefix=__FADD_RU
// __FADD_RU: CUDA API:
...
// RUN: dpct --query-api-mapping=__fadd_rz | FileCheck %s -check-prefix=__FADD_RZ
// __FADD_RZ: CUDA API:
...
// RUN: dpct --query-api-mapping=__fmul_rn | FileCheck %s -check-prefix=__FMUL_RN
// __FMUL_RN: CUDA API:
...
// RUN: dpct --query-api-mapping=__fmul_ru | FileCheck %s -check-prefix=__FMUL_RU
// __FMUL_RU: CUDA API:
...
// RUN: dpct --query-api-mapping=__fmul_rz | FileCheck %s -check-prefix=__FMUL_RZ
// __FMUL_RZ: CUDA API:
...
// RUN: dpct --query-api-mapping=__fsub_rn | FileCheck %s -check-prefix=__FSUB_RN
// __FSUB_RN: CUDA API:
...
// RUN: dpct --query-api-mapping=__fsub_ru | FileCheck %s -check-prefix=__FSUB_RU
// __FSUB_RU: CUDA API:
...
// RUN: dpct --query-api-mapping=__fsub_rz | FileCheck %s -check-prefix=__FSUB_RZ
// __FSUB_RZ: CUDA API:
...
// RUN: dpct --query-api-mapping=__log10f | FileCheck %s -check-prefix=__LOG10F
// __LOG10F: CUDA API:
// __LOG10F-NEXT:   __log10f(f /*float*/);
// __LOG10F-NEXT: Is migrated to:
// __LOG10F-NEXT:   sycl::log10(f);

// RUN: dpct --query-api-mapping=__log2f | FileCheck %s -check-prefix=__LOG2F
// __LOG2F: CUDA API:
// __LOG2F-NEXT:   __log2f(f /*float*/);
// __LOG2F-NEXT: Is migrated to:
// __LOG2F-NEXT:   sycl::log2(f);

// RUN: dpct --query-api-mapping=__logf | FileCheck %s -check-prefix=__LOGF
// __LOGF: CUDA API:
// __LOGF-NEXT:   __logf(f /*float*/);
// __LOGF-NEXT: Is migrated to:
// __LOGF-NEXT:   sycl::log(f);

// RUN: dpct --query-api-mapping=__powf | FileCheck %s -check-prefix=__POWF
// __POWF: CUDA API:
// __POWF-NEXT:   __powf(f1 /*float*/, f2 /*float*/);
// __POWF-NEXT: Is migrated to:
// __POWF-NEXT:   dpct::pow(f1, f2);

// RUN: dpct --query-api-mapping=__saturatef | FileCheck %s -check-prefix=__SATURATEF
...
// RUN: dpct --query-api-mapping=__sincosf | FileCheck %s -check-prefix=__SINCOSF
// __SINCOSF: CUDA API:
...
// RUN: dpct --query-api-mapping=__sinf | FileCheck %s -check-prefix=__SINF
// __SINF: CUDA API:
// __SINF-NEXT:   __sinf(f /*float*/);
// __SINF-NEXT: Is migrated to:
// __SINF-NEXT:   sycl::sin(f);

// RUN: dpct --query-api-mapping=__tanf | FileCheck %s -check-prefix=__TANF
// __TANF: CUDA API:
