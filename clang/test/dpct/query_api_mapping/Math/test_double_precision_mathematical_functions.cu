...
// RUN: dpct --query-api-mapping=cyl_bessel_i1 | FileCheck %s -check-prefix=CYL_BESSEL_I1
...
// RUN: dpct --query-api-mapping=erf | FileCheck %s -check-prefix=ERF
// ERF: CUDA API:
// ERF-NEXT:   erf(d /*double*/);
// ERF-NEXT: Is migrated to:
// ERF-NEXT:   sycl::erf(d);

// RUN: dpct --query-api-mapping=erfc | FileCheck %s -check-prefix=ERFC
// ERFC: CUDA API:
// ERFC-NEXT:   erfc(d /*double*/);
// ERFC-NEXT: Is migrated to:
// ERFC-NEXT:   sycl::erfc(d);

// RUN: dpct --query-api-mapping=erfcinv | FileCheck %s -check-prefix=ERFCINV
// ERFCINV: CUDA API:
...
// RUN: dpct --query-api-mapping=erfcx | FileCheck %s -check-prefix=ERFCX
// ERFCX: CUDA API:
// ERFCX-NEXT:   erfcx(d /*double*/);
// ERFCX-NEXT: Is migrated to:
// ERFCX-NEXT:   sycl::exp(d * d) * sycl::erfc(d);

// RUN: dpct --query-api-mapping=erfinv | FileCheck %s -check-prefix=ERFINV
// ERFINV: CUDA API:
...
// RUN: dpct --query-api-mapping=norm3d | FileCheck %s -check-prefix=NORM3D
// NORM3D: CUDA API:
// NORM3D-NEXT:   norm3d(d1 /*double*/, d2 /*double*/, d3 /*double*/);
// NORM3D-NEXT: Is migrated to:
// NORM3D-NEXT:   sycl::length(sycl::double3(d1, d2, d3));

// RUN: dpct --query-api-mapping=norm4d | FileCheck %s -check-prefix=NORM4D
// NORM4D: CUDA API:
...
// RUN: dpct --query-api-mapping=normcdf | FileCheck %s -check-prefix=NORMCDF
// NORMCDF: CUDA API:
...
// RUN: dpct --query-api-mapping=normcdfinv | FileCheck %s -check-prefix=NORMCDFINV
// NORMCDFINV: CUDA API:
...
// RUN: dpct --query-api-mapping=rhypot | FileCheck %s -check-prefix=RHYPOT
// RHYPOT: CUDA API:
// RHYPOT-NEXT:   rhypot(d1 /*double*/, d2 /*double*/);
// RHYPOT-NEXT: Is migrated to:
// RHYPOT-NEXT:   1 / sycl::hypot(d1, d2);

// RUN: dpct --query-api-mapping=rint | FileCheck %s -check-prefix=RINT
// RINT: CUDA API:
// RINT-NEXT:   rint(d /*double*/);
// RINT-NEXT: Is migrated to:
// RINT-NEXT:   sycl::rint(d);

// RUN: dpct --query-api-mapping=rnorm | FileCheck %s -check-prefix=RNORM
// RNORM: CUDA API:
...
// RUN: dpct --query-api-mapping=rnorm3d | FileCheck %s -check-prefix=RNORM3D
// RNORM3D: CUDA API:
...
// RUN: dpct --query-api-mapping=rnorm4d | FileCheck %s -check-prefix=RNORM4D
// RNORM4D: CUDA API:
...
// RUN: dpct --query-api-mapping=round | FileCheck %s -check-prefix=ROUND
// ROUND: CUDA API:
// ROUND-NEXT:   round(d /*double*/);
// ROUND-NEXT: Is migrated to:
// ROUND-NEXT:   sycl::round(d);

// RUN: dpct --query-api-mapping=rsqrt | FileCheck %s -check-prefix=RSQRT
// RSQRT: CUDA API:
// RSQRT-NEXT:   rsqrt(d /*double*/);
// RSQRT-NEXT: Is migrated to:
// RSQRT-NEXT:   sycl::rsqrt(d);

// RUN: dpct --query-api-mapping=scalbln | FileCheck %s -check-prefix=SCALBLN
// SCALBLN: CUDA API:
// SCALBLN-NEXT:   scalbln(d /*double*/, l /*long int*/);
// SCALBLN-NEXT: Is migrated to:
// SCALBLN-NEXT:   d*(2<<l);

// RUN: dpct --query-api-mapping=scalbn | FileCheck %s -check-prefix=SCALBN
// SCALBN: CUDA API:
// SCALBN-NEXT:   scalbn(d /*double*/, i /*int*/);
// SCALBN-NEXT: Is migrated to:
// SCALBN-NEXT:   d*(2<<i);

// RUN: dpct --query-api-mapping=sin | FileCheck %s -check-prefix=SIN
// SIN: CUDA API:
// SIN-NEXT:   sin(d /*double*/);
// SIN-NEXT: Is migrated to:
// SIN-NEXT:   sycl::sin(d);

// RUN: dpct --query-api-mapping=sincos | FileCheck %s -check-prefix=SINCOS
// SINCOS: CUDA API:
...
// RUN: dpct --query-api-mapping=sincospi | FileCheck %s -check-prefix=SINCOSPI
// SINCOSPI: CUDA API:
