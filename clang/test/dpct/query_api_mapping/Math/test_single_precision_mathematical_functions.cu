...
// RUN: dpct --query-api-mapping=cyl_bessel_i1f | FileCheck %s -check-prefix=CYL_BESSEL_I1F
...
// RUN: dpct --query-api-mapping=erfcf | FileCheck %s -check-prefix=ERFCF
// ERFCF: CUDA API:
// ERFCF-NEXT:   erfcf(f /*float*/);
// ERFCF-NEXT: Is migrated to:
// ERFCF-NEXT:   sycl::erfc(f);

// RUN: dpct --query-api-mapping=erfcinvf | FileCheck %s -check-prefix=ERFCINVF
// ERFCINVF: CUDA API:
...
// RUN: dpct --query-api-mapping=erfcxf | FileCheck %s -check-prefix=ERFCXF
// ERFCXF: CUDA API:
// ERFCXF-NEXT:   erfcxf(f /*float*/);
// ERFCXF-NEXT: Is migrated to:
// ERFCXF-NEXT:   sycl::exp(f*f)*sycl::erfc(f);

// RUN: dpct --query-api-mapping=erff | FileCheck %s -check-prefix=ERFF
// ERFF: CUDA API:
// ERFF-NEXT:   erff(f /*float*/);
// ERFF-NEXT: Is migrated to:
// ERFF-NEXT:   sycl::erf(f);

// RUN: dpct --query-api-mapping=erfinvf | FileCheck %s -check-prefix=ERFINVF
// ERFINVF: CUDA API:
...
// RUN: dpct --query-api-mapping=hypotf | FileCheck %s -check-prefix=HYPOTF
// HYPOTF: CUDA API:
// HYPOTF-NEXT:   hypotf(f1 /*float*/, f2 /*float*/);
// HYPOTF-NEXT: Is migrated to:
// HYPOTF-NEXT:   sycl::hypot(f1, f2);

// RUN: dpct --query-api-mapping=ilogbf | FileCheck %s -check-prefix=ILOGBF
// ILOGBF: CUDA API:
// ILOGBF-NEXT:   ilogbf(f /*float*/);
// ILOGBF-NEXT: Is migrated to:
// ILOGBF-NEXT:   sycl::ilogb(f);

// RUN: dpct --query-api-mapping=isfinite | FileCheck %s -check-prefix=ISFINITE
// ISFINITE: CUDA API:
...
// RUN: dpct --query-api-mapping=isinf | FileCheck %s -check-prefix=ISINF
// ISINF: CUDA API:
...
// RUN: dpct --query-api-mapping=isnan | FileCheck %s -check-prefix=ISNAN
// ISNAN: CUDA API:
...
// RUN: dpct --query-api-mapping=j0f | FileCheck %s -check-prefix=J0F
// J0F: CUDA API:
...
// RUN: dpct --query-api-mapping=j1f | FileCheck %s -check-prefix=J1F
// J1F: CUDA API:
...
// RUN: dpct --query-api-mapping=normcdfinvf | FileCheck %s -check-prefix=NORMCDFINVF
...
// RUN: dpct --query-api-mapping=norm3df | FileCheck %s -check-prefix=NORM3DF
// NORM3DF: CUDA API:
// NORM3DF-NEXT:   norm3df(f1 /*float*/, f2 /*float*/, f3 /*float*/);
// NORM3DF-NEXT: Is migrated to:
// NORM3DF-NEXT:   sycl::length(sycl::float3(f1, f2, f3));

// RUN: dpct --query-api-mapping=norm4df | FileCheck %s -check-prefix=NORM4DF
// NORM4DF: CUDA API:
...
// RUN: dpct --query-api-mapping=normf | FileCheck %s -check-prefix=NORMF
// NORMF: CUDA API:
...
// RUN: dpct --query-api-mapping=rhypotf | FileCheck %s -check-prefix=RHYPOTF
// RHYPOTF: CUDA API:
// RHYPOTF-NEXT:   rhypotf(f1 /*float*/, f2 /*float*/);
// RHYPOTF-NEXT: Is migrated to:
// RHYPOTF-NEXT:   1 / sycl::hypot(f1, f2);

// RUN: dpct --query-api-mapping=rintf | FileCheck %s -check-prefix=RINTF
// RINTF: CUDA API:
// RINTF-NEXT:   rintf(f /*float*/);
// RINTF-NEXT: Is migrated to:
// RINTF-NEXT:   sycl::rint(f);

// RUN: dpct --query-api-mapping=rnorm3df | FileCheck %s -check-prefix=RNORM3DF
// RNORM3DF: CUDA API:
...
// RUN: dpct --query-api-mapping=rnorm4df | FileCheck %s -check-prefix=RNORM4DF
// RNORM4DF: CUDA API:
...
// RUN: dpct --query-api-mapping=rnormf | FileCheck %s -check-prefix=RNORMF
// RNORMF: CUDA API:
...
// RUN: dpct --query-api-mapping=roundf | FileCheck %s -check-prefix=ROUNDF
// ROUNDF: CUDA API:
// ROUNDF-NEXT:   roundf(f /*float*/);
// ROUNDF-NEXT: Is migrated to:
// ROUNDF-NEXT:   sycl::round(f);

// RUN: dpct --query-api-mapping=rsqrtf | FileCheck %s -check-prefix=RSQRTF
// RSQRTF: CUDA API:
// RSQRTF-NEXT:   rsqrtf(f /*float*/);
// RSQRTF-NEXT: Is migrated to:
// RSQRTF-NEXT:   sycl::rsqrt(f);

// RUN: dpct --query-api-mapping=scalblnf | FileCheck %s -check-prefix=SCALBLNF
// SCALBLNF: CUDA API:
// SCALBLNF-NEXT:   scalblnf(f /*float*/, l /*long int*/);
// SCALBLNF-NEXT: Is migrated to:
// SCALBLNF-NEXT:   f*(2<<l);

// RUN: dpct --query-api-mapping=scalbnf | FileCheck %s -check-prefix=SCALBNF
// SCALBNF: CUDA API:
// SCALBNF-NEXT:   scalbnf(f /*float*/, i /*int*/);
// SCALBNF-NEXT: Is migrated to:
// SCALBNF-NEXT:   f*(2<<i);

// RUN: dpct --query-api-mapping=signbit | FileCheck %s -check-prefix=SIGNBIT
// SIGNBIT: CUDA API:
...
// RUN: dpct --query-api-mapping=sincosf | FileCheck %s -check-prefix=SINCOSF
// SINCOSF: CUDA API:
...
// RUN: dpct --query-api-mapping=sincospif | FileCheck %s -check-prefix=SINCOSPIF
// SINCOSPIF: CUDA API:
...
// RUN: dpct --query-api-mapping=y1f | FileCheck %s -check-prefix=Y1F
// Y1F: CUDA API:
