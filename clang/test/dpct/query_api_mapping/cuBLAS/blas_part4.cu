...
// RUN: dpct --query-api-mapping=cublasZgerc | FileCheck %s -check-prefix=cublasZgerc
...
// RUN: dpct --query-api-mapping=cublasStbsv | FileCheck %s -check-prefix=cublasStbsv
...
// RUN: dpct --query-api-mapping=cublasStrsm | FileCheck %s -check-prefix=cublasStrsm
...
// RUN: dpct --query-api-mapping=cublasCdotc | FileCheck %s -check-prefix=cublasCdotc
...
// cublasCdotc-NEXT:   if(sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::device && sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::shared) {
// cublasCdotc-NEXT:     res_temp_ptr_ct{{[0-9]+}} = sycl::malloc_shared<sycl::float2>(1, dpct::get_in_order_queue());
// cublasCdotc-NEXT:   }
// cublasCdotc-NEXT:   oneapi::mkl::blas::column_major::dotc(handle->get_queue(), n, (std::complex<float>*)x, incx, (std::complex<float>*)y, incy, (std::complex<float>*)res_temp_ptr_ct{{[0-9]+}});
// cublasCdotc-NEXT:   if(sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::device && sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::shared) {
// cublasCdotc-NEXT:     handle->get_queue().wait();
// cublasCdotc-NEXT:     *res = *res_temp_ptr_ct{{[0-9]+}};
// cublasCdotc-NEXT:     sycl::free(res_temp_ptr_ct{{[0-9]+}}, dpct::get_in_order_queue());
// cublasCdotc-NEXT:   }

// RUN: dpct --query-api-mapping=cublasSgeam | FileCheck %s -check-prefix=cublasSgeam
...
// RUN: dpct --query-api-mapping=cublasCgeru | FileCheck %s -check-prefix=cublasCgeru
...
// RUN: dpct --query-api-mapping=cublasCgemm | FileCheck %s -check-prefix=cublasCgemm
...
// RUN: dpct --query-api-mapping=cublasCgemm3m | FileCheck %s -check-prefix=cublasCgemm3m
...
// cublasCgemm3m-NEXT:   oneapi::mkl::blas::column_major::gemm(handle->get_queue(), transa, transb, m, n, k, dpct::get_value(alpha, handle->get_queue()), (std::complex<float>*)a, lda, (std::complex<float>*)b, ldb, dpct::get_value(beta, handle->get_queue()), (std::complex<float>*)c, ldc);

// RUN: dpct --query-api-mapping=cublasDtbsv | FileCheck %s -check-prefix=cublasDtbsv
...
// RUN: dpct --query-api-mapping=cublasZaxpy | FileCheck %s -check-prefix=cublasZaxpy
...
// RUN: dpct --query-api-mapping=cublasZsymv | FileCheck %s -check-prefix=cublasZsymv
...
// RUN: dpct --query-api-mapping=cublasSswap | FileCheck %s -check-prefix=cublasSswap
...
// RUN: dpct --query-api-mapping=cublasZtrmm | FileCheck %s -check-prefix=cublasZtrmm
...
// cublasZtrmm-NEXT:   dpct::trmm(handle->get_queue(), left_right, upper_lower, transa, unit_diag, m, n, alpha, a, lda, b, ldb, c, ldc);

// RUN: dpct --query-api-mapping=cublasStrmm | FileCheck %s -check-prefix=cublasStrmm
...
// cublasStrmm-NEXT:   dpct::trmm(handle->get_queue(), left_right, upper_lower, transa, unit_diag, m, n, alpha, a, lda, b, ldb, c, ldc);

// RUN: dpct --query-api-mapping=cublasDnrm2 | FileCheck %s -check-prefix=cublasDnrm2
...
// cublasDnrm2-NEXT:   if(sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::device && sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::shared) {
// cublasDnrm2-NEXT:     res_temp_ptr_ct{{[0-9]+}} = sycl::malloc_shared<double>(1, dpct::get_in_order_queue());
// cublasDnrm2-NEXT:   }
// cublasDnrm2-NEXT:   oneapi::mkl::blas::column_major::nrm2(handle->get_queue(), n, x, incx, res_temp_ptr_ct{{[0-9]+}});
// cublasDnrm2-NEXT:   if(sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::device && sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::shared) {
// cublasDnrm2-NEXT:     handle->get_queue().wait();
// cublasDnrm2-NEXT:     *res = *res_temp_ptr_ct{{[0-9]+}};
// cublasDnrm2-NEXT:     sycl::free(res_temp_ptr_ct{{[0-9]+}}, dpct::get_in_order_queue());
// cublasDnrm2-NEXT:   }

// RUN: dpct --query-api-mapping=cublasDrot | FileCheck %s -check-prefix=cublasDrot
// cublasDrot: CUDA API:
...
// RUN: dpct --query-api-mapping=cublasDznrm2 | FileCheck %s -check-prefix=cublasDznrm2
...
// cublasDznrm2-NEXT:   if(sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::device && sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::shared) {
// cublasDznrm2-NEXT:     res_temp_ptr_ct{{[0-9]+}} = sycl::malloc_shared<double>(1, dpct::get_in_order_queue());
// cublasDznrm2-NEXT:   }
// cublasDznrm2-NEXT:   oneapi::mkl::blas::column_major::nrm2(handle->get_queue(), n, (std::complex<double>*)x, incx, res_temp_ptr_ct{{[0-9]+}});
// cublasDznrm2-NEXT:   if(sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::device && sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::shared) {
// cublasDznrm2-NEXT:     handle->get_queue().wait();
// cublasDznrm2-NEXT:     *res = *res_temp_ptr_ct{{[0-9]+}};
// cublasDznrm2-NEXT:     sycl::free(res_temp_ptr_ct{{[0-9]+}}, dpct::get_in_order_queue());
// cublasDznrm2-NEXT:   }

// RUN: dpct --query-api-mapping=cublasSgbmv | FileCheck %s -check-prefix=cublasSgbmv
...
// RUN: dpct --query-api-mapping=cublasSetVectorAsync | FileCheck %s -check-prefix=cublasSetVectorAsync
...
// RUN: dpct --query-api-mapping=cublasSsymv | FileCheck %s -check-prefix=cublasSsymv
...
// RUN: dpct --query-api-mapping=cublasCtrsv | FileCheck %s -check-prefix=cublasCtrsv
...
// RUN: dpct --query-api-mapping=cublasDasum | FileCheck %s -check-prefix=cublasDasum
...
// cublasDasum-NEXT:   if(sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::device && sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::shared) {
// cublasDasum-NEXT:     res_temp_ptr_ct{{[0-9]+}} = sycl::malloc_shared<double>(1, dpct::get_in_order_queue());
// cublasDasum-NEXT:   }
// cublasDasum-NEXT:   oneapi::mkl::blas::column_major::asum(handle->get_queue(), n, x, incx, res_temp_ptr_ct{{[0-9]+}});
// cublasDasum-NEXT:   if(sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::device && sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::shared) {
// cublasDasum-NEXT:     handle->get_queue().wait();
// cublasDasum-NEXT:     *res = *res_temp_ptr_ct{{[0-9]+}};
// cublasDasum-NEXT:     sycl::free(res_temp_ptr_ct{{[0-9]+}}, dpct::get_in_order_queue());
// cublasDasum-NEXT:   }

// RUN: dpct --query-api-mapping=cublasSsbmv | FileCheck %s -check-prefix=cublasSsbmv
...
// RUN: dpct --query-api-mapping=cublasIzamin | FileCheck %s -check-prefix=cublasIzamin
...
// RUN: dpct --query-api-mapping=cublasNrm2Ex | FileCheck %s -check-prefix=cublasNrm2Ex
