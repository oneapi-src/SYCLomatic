...
// RUN: dpct --query-api-mapping=cublasDgeam | FileCheck %s -check-prefix=cublasDgeam
...
// RUN: dpct --query-api-mapping=cublasDotcEx | FileCheck %s -check-prefix=cublasDotcEx
...
// cublasDotcEx-NEXT:   dpct::dotc(handle->get_queue(), n, x, xtype, incx, y, ytype, incy, res, restype);

// RUN: dpct --query-api-mapping=cublasSsyr2 | FileCheck %s -check-prefix=cublasSsyr2
...
// RUN: dpct --query-api-mapping=cublasChemv | FileCheck %s -check-prefix=cublasChemv
...
// RUN: dpct --query-api-mapping=cublasDsyrkx | FileCheck %s -check-prefix=cublasDsyrkx
...
// RUN: dpct --query-api-mapping=cublasZtbmv | FileCheck %s -check-prefix=cublasZtbmv
...
// RUN: dpct --query-api-mapping=cublasHgemmStridedBatched | FileCheck %s -check-prefix=cublasHgemmStridedBatched
...
// RUN: dpct --query-api-mapping=cublasDzasum | FileCheck %s -check-prefix=cublasDzasum
...
// cublasDzasum-NEXT:   if(sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::device && sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::shared) {
// cublasDzasum-NEXT:     res_temp_ptr_ct{{[0-9]+}} = sycl::malloc_shared<double>(1, dpct::get_in_order_queue());
// cublasDzasum-NEXT:   }
// cublasDzasum-NEXT:   oneapi::mkl::blas::column_major::asum(handle->get_queue(), n, (std::complex<double>*)x, incx, res_temp_ptr_ct{{[0-9]+}});
// cublasDzasum-NEXT:   if(sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::device && sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::shared) {
// cublasDzasum-NEXT:     handle->get_queue().wait();
// cublasDzasum-NEXT:     *res = *res_temp_ptr_ct{{[0-9]+}};
// cublasDzasum-NEXT:     sycl::free(res_temp_ptr_ct{{[0-9]+}}, dpct::get_in_order_queue());
// cublasDzasum-NEXT:   }

// RUN: dpct --query-api-mapping=cublasZgemm3m | FileCheck %s -check-prefix=cublasZgemm3m
...
// RUN: dpct --query-api-mapping=cublasSspmv | FileCheck %s -check-prefix=cublasSspmv
...
// RUN: dpct --query-api-mapping=cublasCsrot | FileCheck %s -check-prefix=cublasCsrot
...
// RUN: dpct --query-api-mapping=cublasGetVersion | FileCheck %s -check-prefix=cublasGetVersion
...
// RUN: dpct --query-api-mapping=cublasIcamin | FileCheck %s -check-prefix=cublasIcamin
...
// RUN: dpct --query-api-mapping=cublasIcamax | FileCheck %s -check-prefix=cublasIcamax
...
// RUN: dpct --query-api-mapping=cublasDdgmm | FileCheck %s -check-prefix=cublasDdgmm
...
// RUN: dpct --query-api-mapping=cublasStrmv | FileCheck %s -check-prefix=cublasStrmv
...
// RUN: dpct --query-api-mapping=cublasCrot | FileCheck %s -check-prefix=cublasCrot
// cublasCrot: CUDA API:
...
// RUN: dpct --query-api-mapping=cublasCreate | FileCheck %s -check-prefix=cublasCreate
...
// RUN: dpct --query-api-mapping=cublasChpmv | FileCheck %s -check-prefix=cublasChpmv
...
// RUN: dpct --query-api-mapping=cublasSnrm2 | FileCheck %s -check-prefix=cublasSnrm2
...
// cublasSnrm2-NEXT:   if(sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::device && sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::shared) {
// cublasSnrm2-NEXT:     res_temp_ptr_ct{{[0-9]+}} = sycl::malloc_shared<float>(1, dpct::get_in_order_queue());
// cublasSnrm2-NEXT:   }
// cublasSnrm2-NEXT:   oneapi::mkl::blas::column_major::nrm2(handle->get_queue(), n, x, incx, res_temp_ptr_ct{{[0-9]+}});
// cublasSnrm2-NEXT:   if(sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::device && sycl::get_pointer_type(res, handle->get_queue().get_context())!=sycl::usm::alloc::shared) {
// cublasSnrm2-NEXT:     handle->get_queue().wait();
// cublasSnrm2-NEXT:     *res = *res_temp_ptr_ct{{[0-9]+}};
// cublasSnrm2-NEXT:     sycl::free(res_temp_ptr_ct{{[0-9]+}}, dpct::get_in_order_queue());
// cublasSnrm2-NEXT:   }

// RUN: dpct --query-api-mapping=cublasZgemmStridedBatched | FileCheck %s -check-prefix=cublasZgemmStridedBatched
...
// RUN: dpct --query-api-mapping=cublasZsyr | FileCheck %s -check-prefix=cublasZsyr
// cublasZsyr: CUDA API:
...
// RUN: dpct --query-api-mapping=cublasDtpsv | FileCheck %s -check-prefix=cublasDtpsv
...
// RUN: dpct --query-api-mapping=cublasDtrsv | FileCheck %s -check-prefix=cublasDtrsv
...
// RUN: dpct --query-api-mapping=cublasDgbmv | FileCheck %s -check-prefix=cublasDgbmv
