...
__device__ void foo() {
  ...
  // CHECK: item_ct1.get_group().get_group_linear_range();
  cg::thread_block::size();
  // CHECK: item_ct1.get_group().get_group_linear_range();
  cg::thread_block::num_threads();

  // X.num_threads()
  // CHECK-COUNT-5: item_ct1.get_group().get_group_linear_range();
  ...
  // CHECK: atile16.get_group_linear_range();
  atile16.num_threads();
  // CHECK: ctile16.get_group_linear_range();
  ctile16.num_threads();
  // CHECK: tile16.get_group_linear_range();
  tile16.num_threads();
}
