// RUN: dpct --enable-profiling=0  --format-range=none -out-root %T/tm-usm-restricted %s --cuda-include-path="%cuda-path/include" --sycl-named-lambda -- -std=c++14 -x cuda --cuda-host-only
