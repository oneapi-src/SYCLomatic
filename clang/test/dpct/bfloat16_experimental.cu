...
// RUN: dpct --format-range=none --use-experimental-features=bfloat16_math_functions -out-root %T/bfloat16_experimental %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
...
__global__ void kernelFuncBfloat16Arithmetic() {
  ...
  // CHECK: bf16 = dpct::clamp(sycl::ext::oneapi::experimental::fma(bf16_1, bf16_2, bf16_3), 0.f, 1.0f);
