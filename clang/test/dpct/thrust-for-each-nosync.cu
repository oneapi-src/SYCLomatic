...
// RUN: dpct --enable-profiling=0  --format-range=none --use-experimental-features=dpl-experimental-api -out-root %T/thrust-for-each-nosync %s --cuda-include-path="%cuda-path/include" -- -std=c++14 -x cuda --cuda-host-only -fno-delayed-template-parsing -ferror-limit=50
