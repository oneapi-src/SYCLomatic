...
// UNSUPPORTED: v8.0
// RUN: dpct --no-dpcpp-extensions=free-function-queries --format-range=none -out-root %T/warp_mask %s --use-experimental-features=masked-sub-group-operation --cuda-include-path="%cuda-path/include" -- -std=c++14 -x cuda --cuda-host-only
