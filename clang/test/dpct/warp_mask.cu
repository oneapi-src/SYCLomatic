...
// UNSUPPORTED: v8.0
// RUN: dpct --format-range=none -out-root %T/warp_mask %s --use-experimental-features=masked_sub_group_operation --cuda-include-path="%cuda-path/include" -- -std=c++14 -x cuda --cuda-host-only
