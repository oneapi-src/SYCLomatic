...
// CHECK-NEXT: /*
// CHECK-NEXT: DPCT1128:{{[0-9]+}}: This type is used in kernel but it is not device copyable. Below part(s) is(are) not meet the requirements: copy constructor. Please rewrite the code.
// CHECK-NEXT: */
...
//      CHECK: DPCT1128:{{[0-9]+}}: This type is used in kernel but it is not device copyable. Below part(s) is(are) not meet the requirements: copy constructor. Please rewrite the code.
// CHECK-NEXT: */
...
// CHECK-NEXT: };
// CHECK-NEXT: /*
// CHECK-NEXT: DPCT1128:{{[0-9]+}}: This type is used in kernel but it is not device copyable. Below part(s) is(are) not meet the requirements: copy constructor, copy assignment, move constructor, move assignment, virtual method "m", destructor, virtual base class "B1", non trivially copyable base class "B2" and non trivially copyable field "f". Please rewrite the code.
// CHECK-NEXT: */
...
void test11() {
  UserStruct16 us16;
  //      CHECK: /*
  // CHECK-NEXT: DPCT1129:{{[0-9]+}}: The type "UserStruct16" is used in kernel but it is not device copyable. The tool has added the sycl::is_device_copyable specialization for this type.
  // CHECK-NEXT: */
  ...
  UserStruct17 us17;
  //      CHECK: /*
  // CHECK-NEXT: DPCT1129:{{[0-9]+}}: The type "UserStruct17" is used in kernel but it is not device copyable. The tool has added the sycl::is_device_copyable specialization for this type.
  // CHECK-NEXT: */
  // CHECK-NEXT: q_ct1.parallel_for(
  // CHECK-NEXT:   sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)), 
  // CHECK-NEXT:   [=](sycl::nd_item<3> item_ct1) {
