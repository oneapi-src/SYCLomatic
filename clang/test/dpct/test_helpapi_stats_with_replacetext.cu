...
// RUN: c2s --format-range=none -out-root %T/test_helpapi_stats_with_replacetext %s --cuda-include-path="%cuda-path/include"  --report-type=stats -- -x cuda --cuda-host-only > stats.txt
