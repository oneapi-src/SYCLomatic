...
__global__ void simple_wmma_gemm(half *a, half *b, float *c, float *d, int m_ld,
  ...
  // CHECK: dpct::experimental::matrix::joint_matrix_fill(item_ct1.get_sub_group(), acc_frag, 0.0f);
  ...
  for (int i = 0; i < k_ld; i += WMMA_K) {
    ...
    if (aRow < m_ld && aCol < k_ld && bRow < k_ld && bCol < n_ld) {
      // Load the inputs
      // CHECK: dpct::experimental::matrix::joint_matrix_load(item_ct1.get_sub_group(), a_frag, a + aCol + aRow * lda, lda);
      nvcuda::wmma::load_matrix_sync(a_frag, a + aCol + aRow * lda, lda);
      // CHECK: dpct::experimental::matrix::joint_matrix_load(item_ct1.get_sub_group(), b_frag, b + bRow + bCol * ldb, ldb);
      nvcuda::wmma::load_matrix_sync(b_frag, b + bRow + bCol * ldb, ldb);

      // Perform the matrix multiplication
      // CHECK: dpct::experimental::matrix::joint_matrix_mad(item_ct1.get_sub_group(), acc_frag, a_frag, b_frag, acc_frag);
