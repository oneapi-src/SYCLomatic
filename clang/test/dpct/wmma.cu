...
// RUN: dpct --format-range=none --use-experimental-features=oneapi-joint-matrix -out-root %T/wmma %s --cuda-include-path="%cuda-path/include" -- -std=c++14 -x cuda --cuda-host-only
