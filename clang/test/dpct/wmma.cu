...
__global__ void simple_wmma_gemm(half *a, half *b, float *c, float *d, int m_ld,
  ...
  if (cRow < m_ld && cCol < n_ld) {
    // CHECK: dpct::experimental::matrix::joint_matrix_load(c_frag, c + cCol + cRow * ldc, ldc, dpct::experimental::matrix::layout_t::row_major);
    ...
    // Store the output
    // CHECK: dpct::experimental::matrix::joint_matrix_store(d + cCol + cRow * ldc, c_frag, ldc, dpct::experimental::matrix::layout_t::col_major);
