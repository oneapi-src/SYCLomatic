// RUN: dpct --use-experimental-features=l0-ipc --format-range=none --usm-level=none -out-root %T/share_mem_exp_option %s --cuda-include-path="%cuda-path/include" --sycl-named-lambda -- -x cuda --cuda-host-only
