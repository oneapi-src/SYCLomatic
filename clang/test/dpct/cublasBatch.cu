...
int main() {
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (dpct::getrs_batch_wrapper(*handle, dpct::get_transpose(trans0), n, nrhs, Aarray_Sc, lda, PivotArray, Barray_S, ldb, infoArray, batchSize), 0);
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (dpct::getrs_batch_wrapper(*handle, dpct::get_transpose(trans1), n, nrhs, Aarray_Dc, lda, PivotArray, Barray_D, ldb, infoArray, batchSize), 0);
  ...
  // CHECK-NEXT: DPCT1003:{{[0-9]+}}: Migrated API does not return error code. (*, 0) is inserted. You may need to rewrite this code.
  // CHECK-NEXT: */
  // CHECK-NEXT: status = (dpct::getrs_batch_wrapper(*handle, dpct::get_transpose(trans2), n, nrhs, Aarray_Cc, lda, PivotArray, Barray_C, ldb, infoArray, batchSize), 0);
