...
int main() {
  ...
  //CHECK: status = CHECK_SYCL_ERROR(dpct::syrk(*handle, fill0 == 0 ? oneapi::mkl::uplo::lower : oneapi::mkl::uplo::upper, dpct::get_transpose(trans0), n, k, &alpha_s, A_s, lda, B_s, ldb, &beta_s, C_s, ldc));
  ...
  //CHECK: status = CHECK_SYCL_ERROR(dpct::syrk(*handle, oneapi::mkl::uplo::lower, oneapi::mkl::transpose::nontrans, n, k, &alpha_d, A_d, lda, B_d, ldb, &beta_d, C_d, ldc));
