...
int main() {
  ...
  // CHECK: /*
  // CHECK-NEXT: DPCT1123:{{[0-9]+}}: The kernel function pointer cannot be deduced correctly. You need cast "(kernel_array[10])" to a function pointer. According to the kernel function definition, adjusting the kernel function arguments and the dimension of the sycl::nd_item may be also required.
  // CHECK-NEXT: */
  // CHECK-NEXT: q_ct1.parallel_for(
  // CHECK-NEXT:   sycl::nd_range<3>(sycl::range<3>(1, 1, 16) * sycl::range<3>(1, 1, 16), sycl::range<3>(1, 1, 16)), 
  // CHECK-NEXT:   [=](sycl::nd_item<3> item_ct1) {
