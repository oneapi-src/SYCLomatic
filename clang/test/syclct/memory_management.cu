...
void fooo() {
    ...
    // CHECK: syclct::sycl_memcpy(d_A, h_A, size, syclct::host_to_device);
    cudaMemcpy(d_A, h_A, size, cudaMemcpyHostToDevice);
    // CHECK: syclct::sycl_memcpy(h_A, d_A, size, syclct::device_to_host);
...
///////// Host to host
// CHECK:  syclct::sycl_memcpy(d_A, h_A, size, syclct::host_to_host);
  cudaMemcpy(d_A, h_A, size, cudaMemcpyHostToHost);
// CHECK:/*
// CHECK-NEXT:SYCLCT1003: Translated api does not return error code. (*, 0) is inserted. You may want to rewrite this code
// CHECK-NEXT:*/
// CHECK-NEXT:  err = (syclct::sycl_memcpy(d_A, h_A, size, syclct::host_to_host), 0);
  err = cudaMemcpy(d_A, h_A, size, cudaMemcpyHostToHost);
// CHECK:/*
// CHECK-NEXT:SYCLCT1003: Translated api does not return error code. (*, 0) is inserted. You may want to rewrite this code
// CHECK-NEXT:*/
// CHECK-NEXT:  checkError((syclct::sycl_memcpy(d_A, h_A, size, syclct::host_to_host), 0));
...
// CHECK-NEXT:SYCLCT1003: Translated api does not return error code. (*, 0) is inserted. You may want to rewrite this code
// CHECK-NEXT:*/
// CHECK-NEXT:  err = (syclct::sycl_memcpy(d_A, h_A, size, syclct::host_to_device), 0);
  err = cudaMemcpy(d_A, h_A, size, cudaMemcpyHostToDevice);
// CHECK:/*
// CHECK-NEXT:SYCLCT1003: Translated api does not return error code. (*, 0) is inserted. You may want to rewrite this code
// CHECK-NEXT:*/
// CHECK-NEXT:  checkError((syclct::sycl_memcpy(d_A, h_A, size, syclct::host_to_device), 0));
...
// CHECK-NEXT:SYCLCT1003: Translated api does not return error code. (*, 0) is inserted. You may want to rewrite this code
// CHECK-NEXT:*/
// CHECK-NEXT:  err = (syclct::sycl_memcpy(h_A, d_A, size, syclct::device_to_host), 0);
  err = cudaMemcpy(h_A, d_A, size, cudaMemcpyDeviceToHost);
// CHECK:/*
// CHECK-NEXT:SYCLCT1003: Translated api does not return error code. (*, 0) is inserted. You may want to rewrite this code
// CHECK-NEXT:*/
// CHECK-NEXT:  checkError((syclct::sycl_memcpy(h_A, d_A, size, syclct::device_to_host), 0));
  checkError(cudaMemcpy(h_A, d_A, size, cudaMemcpyDeviceToHost));

///////// Device to Device
// CHECK:  syclct::sycl_memcpy(h_A, d_A, size, syclct::device_to_device);
  cudaMemcpy(h_A, d_A, size, cudaMemcpyDeviceToDevice);
// CHECK:/*
// CHECK-NEXT:SYCLCT1003: Translated api does not return error code. (*, 0) is inserted. You may want to rewrite this code
// CHECK-NEXT:*/
// CHECK-NEXT:  err = (syclct::sycl_memcpy(h_A, d_A, size, syclct::device_to_device), 0);
  err = cudaMemcpy(h_A, d_A, size, cudaMemcpyDeviceToDevice);
// CHECK:/*
// CHECK-NEXT:SYCLCT1003: Translated api does not return error code. (*, 0) is inserted. You may want to rewrite this code
// CHECK-NEXT:*/
// CHECK-NEXT:  checkError((syclct::sycl_memcpy(h_A, d_A, size, syclct::device_to_device), 0));
  checkError(cudaMemcpy(h_A, d_A, size, cudaMemcpyDeviceToDevice));

///////// Default
// CHECK:  syclct::sycl_memcpy(h_A, d_A, size, syclct::automatic);
  cudaMemcpy(h_A, d_A, size, cudaMemcpyDefault);
// CHECK:/*
// CHECK-NEXT:SYCLCT1003: Translated api does not return error code. (*, 0) is inserted. You may want to rewrite this code
// CHECK-NEXT:*/
// CHECK-NEXT:  err = (syclct::sycl_memcpy(h_A, d_A, size, syclct::automatic), 0);
  err = cudaMemcpy(h_A, d_A, size, cudaMemcpyDefault);
// CHECK:/*
// CHECK-NEXT:SYCLCT1003: Translated api does not return error code. (*, 0) is inserted. You may want to rewrite this code
// CHECK-NEXT:*/
// CHECK-NEXT:  checkError((syclct::sycl_memcpy(h_A, d_A, size, syclct::automatic), 0));
