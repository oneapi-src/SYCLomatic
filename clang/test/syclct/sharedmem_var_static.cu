...
#include <stdio.h>
#define SIZE 64
// CHECK: void nonTypeTemplateReverse(int *d, int n, cl::sycl::nd_item<3> [[ITEM:item_ct1]], syclct::syclct_accessor<int, syclct::shared, 1> s) {
...
// CHECK: void staticReverse(int *d, int n, cl::sycl::nd_item<3> [[ITEM:item_ct1]], syclct::syclct_accessor<int, syclct::shared, 1> s) {
...
// CHECK-NEXT: void templateReverse(TData *d, TData n, cl::sycl::nd_item<3> [[ITEM:item_ct1]], syclct::syclct_accessor<TData, syclct::shared, 2> s) {
...
void testTemplate() {
  ...
  // CHECK-NEXT:      syclct::shared_memory<T, 2> s(64/*size * 2*/, 128/*size * 4*/);
  ...
  // CHECK-NEXT:          templateReverse<T>(d_d, n, [[ITEM]], syclct::syclct_accessor<T, syclct::shared, 2>(s_acc_ct1, s_range_ct1));
  // CHECK-NEXT:      });
  // CHECK-NEXT:    });
  // CHECK-NEXT:}
...
int main(void) {
  ...
  // CHECK-NEXT:  syclct::get_default_queue().submit(
  // CHECK-NEXT:    [&](cl::sycl::handler &cgh) {
  // CHECK-NEXT:      syclct::shared_memory<int, 1> s(64/*size*/);
  // CHECK-NEXT:      auto s_range_ct1 = s.get_range();
  // CHECK-NEXT:      auto s_acc_ct1 = s.get_access(cgh);
  // CHECK-NEXT:      auto d_d_acc = d_d_buf.first.get_access<cl::sycl::access::mode::read_write>(cgh);
  ...
  // CHECK-NEXT:          staticReverse(d_d, n, [[ITEM]], syclct::syclct_accessor<int, syclct::shared, 1>(s_acc_ct1, s_range_ct1));
  // CHECK-NEXT:        });
  // CHECK-NEXT:    });
  // CHECK-NEXT:}
  ...
  // CHECK-NEXT:  syclct::get_default_queue().submit(
  // CHECK-NEXT:    [&](cl::sycl::handler &cgh) {
  // CHECK-NEXT:      syclct::shared_memory<int, 2> s(64/*size * 2*/, 128/*size * 4*/);
  // CHECK-NEXT:      auto s_range_ct1 = s.get_range();
  // CHECK-NEXT:      auto s_acc_ct1 = s.get_access(cgh);
  // CHECK-NEXT:      auto d_d_acc = d_d_buf.first.get_access<cl::sycl::access::mode::read_write>(cgh);
  ...
  // CHECK-NEXT:        [=](cl::sycl::nd_item<3> [[ITEM:item_ct1]]) {
  // CHECK-NEXT:          int *d_d = (int*)(&d_d_acc[0] + d_d_offset);
  // CHECK-NEXT:          templateReverse<int>(d_d, n, [[ITEM]], syclct::syclct_accessor<int, syclct::shared, 2>(s_acc_ct1, s_range_ct1));
  // CHECK-NEXT:        });
  // CHECK-NEXT:    });
  // CHECK-NEXT:}
  ...
  // CHECK-NEXT:  syclct::get_default_queue().submit(
  // CHECK-NEXT:    [&](cl::sycl::handler &cgh) {
  // CHECK-NEXT:      syclct::shared_memory<int, 1> s(2*SIZE*SIZE);
  // CHECK-NEXT:      auto s_range_ct1 = s.get_range();
  // CHECK-NEXT:      auto s_acc_ct1 = s.get_access(cgh);
  // CHECK-NEXT:      auto d_d_acc = d_d_buf.first.get_access<cl::sycl::access::mode::read_write>(cgh);
  // CHECK-NEXT:      cgh.parallel_for<syclct_kernel_name<class nonTypeTemplateReverse_{{[a-f0-9]+}}, syclct_kernel_scalar<SIZE>>>(
  // CHECK-NEXT:        cl::sycl::nd_range<3>((cl::sycl::range<3>(1, 1, 1) * cl::sycl::range<3>(n, 1, 1)), cl::sycl::range<3>(n, 1, 1)),
  // CHECK-NEXT:        [=](cl::sycl::nd_item<3> [[ITEM:item_ct1]]) {
  // CHECK-NEXT:          int *d_d = (int*)(&d_d_acc[0] + d_d_offset);
  // CHECK-NEXT:          nonTypeTemplateReverse<SIZE>(d_d, n, [[ITEM]], syclct::syclct_accessor<int, syclct::shared, 1>(s_acc_ct1, s_range_ct1));
  // CHECK-NEXT:        });
  // CHECK-NEXT:    });
  // CHECK-NEXT:  }
