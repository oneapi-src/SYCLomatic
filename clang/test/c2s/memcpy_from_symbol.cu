// RUN: c2s --format-range=none -usm-level=none -out-root %T/memcpy_from_symbol %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
...
// CHECK: c2s::global_memory<int, 0> a;
__device__ int a;
__global__ void kernel1() { a = 10; }

// CHECK: c2s::global_memory<int, 1> b(k_num_elements);
...
int main() {
  int h_a = 0;
  kernel1<<<1, 1>>>();
  // CHECK: c2s::c2s_memcpy(&h_a, a.get_ptr(), sizeof(h_a));
  cudaMemcpyFromSymbol(&h_a, a, sizeof(h_a));
  CHECK(h_a == 10);

  int h_b[k_num_elements] = {0};
  kernel2<<<1, k_num_elements>>>();
  // CHECK: c2s::c2s_memcpy(&h_b, b.get_ptr(), sizeof(h_b));
