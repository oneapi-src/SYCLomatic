// RUN: c2s --format-range=none --usm-level=none -out-root %T/devicemem %s --cuda-include-path="%cuda-path/include" --sycl-named-lambda -- -x cuda --cuda-host-only
...
int main() {
  ...
  // CHECK-NEXT:     auto d_out_acc_ct0 = c2s::get_access(d_out, cgh);
  // CHECK-EMPTY:
