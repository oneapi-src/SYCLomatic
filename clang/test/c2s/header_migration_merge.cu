...
// RUN: c2s --format-range=none -out-root %T/header_migration_merge %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only
// RUN: c2s --format-range=none -out-root %T/header_migration_merge %s --cuda-include-path="%cuda-path/include" -- -x cuda --cuda-host-only -DMACROA
