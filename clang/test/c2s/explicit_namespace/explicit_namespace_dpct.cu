// RUN: c2s --format-range=none -in-root %S -out-root %T/explicit_namespace_c2s %S/explicit_namespace_dpct.cu --cuda-include-path="%cuda-path/include" --use-explicit-namespace=dpct --sycl-named-lambda -- -x cuda --cuda-host-only
// RUN: FileCheck --input-file %T/explicit_namespace_c2s/explicit_namespace_dpct.dp.cpp --match-full-lines %s
