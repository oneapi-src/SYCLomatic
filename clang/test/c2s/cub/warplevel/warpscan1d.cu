...
// RUN: c2s --assume-nd-range-dim=1 -in-root %S -out-root %T/warplevel/warpscan1d %S/warpscan1d.cu --cuda-include-path="%cuda-path/include" -- -std=c++14 -x cuda --cuda-host-only
